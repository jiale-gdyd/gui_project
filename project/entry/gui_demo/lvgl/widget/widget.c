...
static void analytics_create(lv_obj_t *parent)
{
    ...
    indic = lv_meter_add_scale_lines(meter3, lv_palette_darken(LV_PALETTE_RED, 3), lv_palette_darken(LV_PALETTE_RED, 3), true, 0);
    lv_meter_set_indicator_start_value(meter3, indic, 0);
    lv_meter_set_indicator_end_value(meter3, indic, 20);
    ...
    indic = lv_meter_add_scale_lines(meter3, lv_palette_darken(LV_PALETTE_BLUE, 3), lv_palette_darken(LV_PALETTE_BLUE, 3), true, 0);
    lv_meter_set_indicator_start_value(meter3, indic, 20);
    lv_meter_set_indicator_end_value(meter3, indic, 40);
    ...
    indic = lv_meter_add_scale_lines(meter3, lv_palette_darken(LV_PALETTE_GREEN, 3), lv_palette_darken(LV_PALETTE_GREEN, 3), true, 0);
    lv_meter_set_indicator_start_value(meter3, indic, 40);
    lv_meter_set_indicator_end_value(meter3, indic, 60);
...
static void color_event_cb(lv_event_t *e)
{
    ...
    if (code == LV_EVENT_FOCUSED) {
        ...
        lv_palette_t palette_secondary = (*palette_primary) + 3;
...
static void shop_chart_event_cb(lv_event_t *e)
{
    ...
    if (code == LV_EVENT_DRAW_PART_END) {
        ...
        if (dsc->part == LV_PART_ITEMS) {
            ...
            a.y2 = a.y1 + 4 + (devices[dsc->id] * h) / 100;
...
static void meter2_timer_cb(lv_timer_t *timer)
{
    ...
    lv_meter_indicator_t **indics = (lv_meter_indicator_t **)timer->user_data;
