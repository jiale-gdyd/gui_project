...
int lvgl_demo_printer(int argc, char *argv[])
{
    ...
    lv_theme_t *th = lv_demo_printer_theme_init(LV_COLOR_BLACK, LV_COLOR_BLACK, 0, &lv_font_montserrat_14, &lv_font_montserrat_22, &lv_font_montserrat_28, &lv_font_montserrat_32);
    lv_theme_set_act(th);
...
static void home_open(uint32_t delay)
{
    ...
    lv_obj_set_style_local_value_str(box, LV_CONT_PART_MAIN, LV_STATE_DEFAULT, "http://www.xin-dun.com");
    ...
    lv_obj_set_style_local_bg_color(bar, LV_BAR_PART_INDIC, LV_STATE_DEFAULT, lv_color_hex(0xe600e6));
    lv_obj_set_size(bar, 25, 50);
    lv_obj_align_origo(bar, NULL, LV_ALIGN_IN_LEFT_MID, 3 * (box_w - 20) / 8 + 10, 0);
    lv_bar_set_value(bar, 30, LV_ANIM_ON);

    bar = lv_bar_create(box, NULL);
    lv_obj_set_style_local_bg_color(bar, LV_BAR_PART_INDIC, LV_STATE_DEFAULT, lv_color_hex(0xefef01));
    lv_obj_set_size(bar, 25, 50);
    lv_obj_align_origo(bar, NULL, LV_ALIGN_IN_LEFT_MID, 5 * (box_w - 20) / 8 + 10, 0);
    lv_bar_set_value(bar, 80, LV_ANIM_ON);

    bar = lv_bar_create(box, NULL);
    lv_obj_set_style_local_bg_color(bar, LV_BAR_PART_INDIC, LV_STATE_DEFAULT, lv_color_hex(0x1d1d25));
    lv_obj_set_size(bar, 25, 50);
...
static void loader_anim_cb(void *arc, lv_anim_value_t v)
{
    ...
    lv_obj_set_style_local_value_str((lv_obj_t *)arc, LV_ARC_PART_BG, LV_STATE_DEFAULT, buf);
}
    ...
    while (child) {
        if ((child != scan_img) && (child != bg_top) && (child != bg_bottom) && (child != lv_scr_act())) {
            ...
            if (lv_obj_get_y(child) < 80) {
                lv_anim_set_values(&a, lv_obj_get_y(child), lv_obj_get_y(child) - LV_DEMO_PRINTER_ANIM_Y);
            } else {
                lv_anim_set_values(&a, lv_obj_get_y(child), lv_obj_get_y(child) + LV_DEMO_PRINTER_ANIM_Y);
