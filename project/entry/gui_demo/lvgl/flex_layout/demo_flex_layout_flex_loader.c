    ...
    do {                                                                                        \
        ...
        for (i = 0; i < ARRAY_SIZE(flex_align_map); i++) {                                      \
            if (flex_align == flex_align_map[i]) {                                              \
                lv_dropdown_set_selected(ui->ctrl_pad.tab.align.ddlist_align_##item, i);        \
                break;                                                                          \
            }                                                                                   \
        }                                                                                       \
    } while(0)
...
#define FLEX_ALIGN_EVENT_ATTACH(item)                                                           \
    do {                                                                                        \
        lv_obj_add_event(ui->ctrl_pad.tab.align.ddlist_align_##item, flex_align_##item##_event_handler, LV_EVENT_VALUE_CHANGED, ui);    \
    } while(0)
...
{
    lv_obj_add_event(ui->ctrl_pad.tab.flex.ddlist_flow, flex_flow_event_handler, LV_EVENT_VALUE_CHANGED, ui);
