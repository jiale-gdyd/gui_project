...
void ctrl_pad_attach(view_t *ui)
{
    ...
    lv_obj_add_event(ui->ctrl_pad.tab.flex.checkbox_scrl, ctrl_pad_checkbox_event_handler, LV_EVENT_VALUE_CHANGED, ui);
}
...
static void ctrl_pad_btn_event_attach(view_t *ui)
{
    lv_obj_add_event(ui->ctrl_pad.btn.add, ctrl_pad_btn_add_event_handler, LV_EVENT_CLICKED, ui);
    lv_obj_add_event(ui->ctrl_pad.btn.remove, ctrl_pad_btn_remove_event_handler, LV_EVENT_CLICKED, ui);
}
