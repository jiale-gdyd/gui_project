...
static const void *get_imgfont_path(const lv_font_t *font, uint32_t unicode, uint32_t unicode_next, lv_coord_t *offset_y, void *user_data)
{
    LV_UNUSED(user_data);
...
int lvgl_demo_multilang(int argc, char *argv[])
{
...
static void scroll_event_cb(lv_event_t * e)
{
    ...
    if (lv_indev_get_scroll_obj(indev) != cont) {
        return;
    }
