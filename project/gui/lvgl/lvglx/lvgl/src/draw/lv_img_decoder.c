...
lv_res_t lv_img_decoder_get_info(const void * src, lv_img_header_t * header)
{
    ...
    for (d = (lv_img_decoder_t *)_lv_ll_get_head(&LV_GC_ROOT(_lv_img_decoder_ll)); d != NULL; d = (lv_img_decoder_t *)_lv_ll_get_next(&LV_GC_ROOT(_lv_img_decoder_ll), d)) {
        if(d->info_cb) {
...
lv_res_t lv_img_decoder_open(lv_img_decoder_dsc_t * dsc, const void * src, lv_color_t color, int32_t frame_id)
{
    ...
    if(dsc->src_type == LV_IMG_SRC_FILE) {
        size_t fnlen = strlen((const char *)src);
        ...
        strcpy((char *)dsc->src, (const char *)src);
    }
    else {
        dsc->src = src;
    }
    ...
    for (decoder = (lv_img_decoder_t *)_lv_ll_get_head(&LV_GC_ROOT(_lv_img_decoder_ll)); decoder != NULL; decoder = (lv_img_decoder_t *)_lv_ll_get_next(&LV_GC_ROOT(_lv_img_decoder_ll), decoder)) {
...
{
    lv_img_decoder_t * decoder;
    decoder = (lv_img_decoder_t *)_lv_ll_ins_head(&LV_GC_ROOT(_lv_img_decoder_ll));
    LV_ASSERT_MALLOC(decoder);
...
lv_res_t lv_img_decoder_built_in_info(lv_img_decoder_t * decoder, const void * src, lv_img_header_t * header)
{
    ...
    if(src_type == LV_IMG_SRC_VARIABLE) {
        ...
        if(strcmp(lv_fs_get_ext((const char *)src), "bin")) return LV_RES_INV;

        lv_fs_file_t f;
        lv_fs_res_t res = lv_fs_open(&f, (const char *)src, LV_FS_MODE_RD);
        if(res == LV_FS_RES_OK) {
            uint32_t rn;
...
lv_res_t lv_img_decoder_built_in_open(lv_img_decoder_t * decoder, lv_img_decoder_dsc_t * dsc)
{
    ...
    if(dsc->src_type == LV_IMG_SRC_FILE) {
        /*Support only "*.bin" files*/
        if(strcmp(lv_fs_get_ext((const char *)dsc->src), "bin")) return LV_RES_INV;

        lv_fs_file_t f;
        lv_fs_res_t res = lv_fs_open(&f, (const char *)dsc->src, LV_FS_MODE_RD);
        if(res != LV_FS_RES_OK) {
        ...
        lv_img_decoder_built_in_data_t * user_data = (lv_img_decoder_built_in_data_t *)dsc->user_data;
    ...
    /*Process indexed images. Build a palette*/
        ...
        lv_img_decoder_built_in_data_t * user_data = (lv_img_decoder_built_in_data_t *)dsc->user_data;
        user_data->palette = (lv_color_t *)lv_malloc(palette_size * sizeof(lv_color_t));
        LV_ASSERT_MALLOC(user_data->palette);
        user_data->opa = (lv_opa_t *)lv_malloc(palette_size * sizeof(lv_opa_t));
...
#error "Invalid LV_COLOR_DEPTH. Check it in lv_conf.h"
    ...
    lv_img_decoder_built_in_data_t * user_data = (lv_img_decoder_built_in_data_t *)dsc->user_data;
    uint8_t * fs_buf = (uint8_t *)lv_malloc(w);
