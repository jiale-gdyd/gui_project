...
static gd_GIF * gif_open(gd_GIF * gif_base)
{
    ...
    gif = (gd_GIF *)lv_malloc(sizeof(gd_GIF) + 5 * width * height);
#elif LV_COLOR_DEPTH == 16
    gif = (gd_GIF *)lv_malloc(sizeof(gd_GIF) + 4 * width * height);
#elif LV_COLOR_DEPTH == 8 || LV_COLOR_DEPTH == 1
    gif = (gd_GIF *)lv_malloc(sizeof(gd_GIF) + 3 * width * height);
#endif
...
new_table(int key_size)
{
    ...
    Table *table = (Table *)lv_malloc(sizeof(*table) + sizeof(Entry) * init_bulk);
    if (table) {
...
add_entry(Table **tablep, uint16_t length, uint16_t prefix, uint8_t suffix)
{
    ...
    if (table->nentries == table->bulk) {
        table->bulk *= 2;
        table = (Table *)lv_realloc(table, sizeof(*table) + sizeof(Entry) * table->bulk);
        if (!table) return -1;
        table->entries = (Entry *) &table[1];
        *tablep = table;
    }
...
static bool f_gif_open(gd_GIF * gif, const void * path, bool is_file)
{
    ...
    if(is_file) {
        lv_fs_res_t res = lv_fs_open(&gif->fd, (const char *)path, LV_FS_MODE_RD);
...
{
    if(gif->is_file) {
        lv_fs_seek(&gif->fd, pos, (lv_fs_whence_t)k);
        uint32_t x;
        lv_fs_tell(&gif->fd, &x);
        return x;
    } else {
