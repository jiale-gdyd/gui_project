...
lv_res_t lv_qrcode_update(lv_obj_t * obj, const void * data, uint32_t data_len)
{
    ...
    lv_img_dsc_t * imgdsc = lv_canvas_get_img((lv_obj_t *)qrcode);
    ...
    uint8_t * qr0 = (uint8_t *)lv_malloc(qrcodegen_BUFFER_LEN_FOR_VERSION(qr_version));
    LV_ASSERT_MALLOC(qr0);
    uint8_t * data_tmp = (uint8_t *)lv_malloc(qrcodegen_BUFFER_LEN_FOR_VERSION(qr_version));
