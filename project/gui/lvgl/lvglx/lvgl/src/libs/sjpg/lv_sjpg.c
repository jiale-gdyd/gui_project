...
{
    io_source_t *io = (io_source_t *)jd->device;
...
static lv_res_t decoder_open( lv_img_decoder_t * decoder, lv_img_decoder_dsc_t * dsc )
{
    ...
    if(dsc->src_type == LV_IMG_SRC_VARIABLE) {
        ...
        if( sjpeg == NULL ) {
            sjpeg =  (SJPEG *)lv_malloc( sizeof( SJPEG ) );
        ...
        if( !strncmp( ( char * ) sjpeg->sjpeg_data, "_SJPG__", strlen("_SJPG__") ) ) {
            ...
            sjpeg->frame_base_array = (uint8_t **)lv_malloc( sizeof(uint8_t *) * sjpeg->sjpeg_total_frames );
            ...
            sjpeg->frame_cache = (uint8_t *)lv_malloc( sjpeg->sjpeg_x_res * sjpeg->sjpeg_single_frame_height * 3/*2*/ );
            ...
            sjpeg->workb =  (uint8_t *)lv_malloc( TJPGD_WORKBUFF_SIZE );
            if( ! sjpeg->workb ) {
                lv_sjpg_cleanup( sjpeg );
                sjpeg = NULL;
                return LV_RES_INV;
            ...
            sjpeg->tjpeg_jd = (JDEC *)lv_malloc( sizeof( JDEC ) );
            if( ! sjpeg->tjpeg_jd ) {
                lv_sjpg_cleanup( sjpeg );
                sjpeg = NULL;
                return LV_RES_INV;
            ...
            if(rc == JDR_OK ) {
                ...
                sjpeg->sjpeg_cache_frame_index = -1;
                sjpeg->frame_cache = (uint8_t *)lv_malloc( sjpeg->sjpeg_x_res * sjpeg->sjpeg_single_frame_height * 3 );
                if( ! sjpeg->frame_cache ) {
                    lv_sjpg_cleanup( sjpeg );
                    sjpeg = NULL;
                    return LV_RES_INV;
        ...
        if(strcmp(lv_fs_get_ext(fn), "sjpg") == 0) {
            ...
            if(strcmp((char *)buff, "_SJPG__") == 0 ) {
                ...
                sjpeg->frame_base_offset = (int *)lv_malloc( sizeof(int) * sjpeg->sjpeg_total_frames );
            ...
            if(rc == JDR_OK ) {
                ...
                sjpeg->frame_base_offset =  (int *)lv_malloc( sizeof(uint8_t *) * sjpeg->sjpeg_total_frames );
                if( ! sjpeg->frame_base_offset ) {
                    lv_fs_close(&lv_file);
                    lv_sjpg_cleanup(sjpeg);
                    return LV_RES_INV;
