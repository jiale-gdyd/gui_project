...
lv_span_t * lv_spangroup_new_span(lv_obj_t * obj)
{
    ...
    lv_span_t * span = (lv_span_t *)_lv_ll_ins_tail(&spans->child_ll);
...
void lv_spangroup_del_span(lv_obj_t * obj, lv_span_t * span)
{
    ...
    for (cur_span = (lv_span_t *)_lv_ll_get_head(&spans->child_ll); cur_span != NULL; cur_span = (lv_span_t *)_lv_ll_get_next(&spans->child_ll, cur_span)) {
        if(cur_span == span) {
...
void lv_span_set_text(lv_span_t * span, const char * text)
{
    ...
    if(span->txt == NULL || span->static_flag == 1) {
        span->txt = (char *)lv_malloc(strlen(text) + 1);
    }
    else {
        span->txt = (char *)lv_realloc(span->txt, strlen(text) + 1);
    }
    span->static_flag = 0;
...
lv_coord_t lv_spangroup_get_expand_height(lv_obj_t * obj, lv_coord_t width)
{
    ...
    lv_span_t * cur_span = (lv_span_t *)_lv_ll_get_head(&spans->child_ll);
    const char * cur_txt = cur_span->txt;
    span_text_check(&cur_txt);
    ...
    while(cur_span) {
        ...
        while(1) {
            ...
            if(cur_txt[cur_txt_ofs] == '\0') {
                cur_span = (lv_span_t *)_lv_ll_get_next(&spans->child_ll, cur_span);
...
static void lv_spangroup_destructor(const lv_obj_class_t * class_p, lv_obj_t * obj)
{
    ...
    while(cur_span) {
        ...
        lv_style_reset(&cur_span->style);
        lv_free(cur_span);
        cur_span = (lv_span_t *)_lv_ll_get_head(&spans->child_ll);
    }
}
    ...
    while(cur_span) {
        ...
        /* Whether the current line is the end line and does overflow processing */
        {
            ...
            if(last_snippet->txt[last_snippet->bytes] == '\0') {
                next_line_h = 0;
                lv_span_t * next_span = (lv_span_t *)_lv_ll_get_next(&spans->child_ll, last_snippet->span);
