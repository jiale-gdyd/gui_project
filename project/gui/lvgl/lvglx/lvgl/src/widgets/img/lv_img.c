...
void lv_img_set_src(lv_obj_t * obj, const void * src)
{
    ...
    if(src_type == LV_IMG_SRC_VARIABLE) {
        ...
        if(img->src != src) {
            ...
            char * new_str = (char *)lv_malloc(strlen((const char *)src) + 1);
            LV_ASSERT_MALLOC(new_str);
            if(new_str == NULL) return;
            strcpy(new_str, (const char *)src);
            img->src = new_str;
    ...
    if(src_type == LV_IMG_SRC_SYMBOL) {
        ...
        lv_point_t size;
        lv_txt_get_size(&size, (const char *)src, font, letter_space, line_space, LV_COORD_MAX, LV_TEXT_FLAG_NONE);
        header.w = size.x;
        header.h = size.y;
    }
    ...
    if(code == LV_EVENT_COVER_CHECK) {
        ...
        if(code == LV_EVENT_DRAW_MAIN) {
            ...
            if(img->src_type == LV_IMG_SRC_FILE || img->src_type == LV_IMG_SRC_VARIABLE) {
                ...
                lv_draw_label(draw_ctx, &label_dsc, &obj->coords, (const char *)img->src, NULL);
            }
