...
void lv_dropdown_set_options(lv_obj_t * obj, const char * options)
{
    ...
    dropdown->options = (char *)lv_malloc(len);
...
void lv_dropdown_add_option(lv_obj_t * obj, const char * option, uint32_t pos)
{
    ...
    dropdown->options        = (char *)lv_realloc(dropdown->options, new_len + 1);
    LV_ASSERT_MALLOC(dropdown->options);
    if(dropdown->options == NULL) return;
    ...
    char * ins_buf = (char *)lv_malloc(ins_len + 2); /*+ 2 for terminating NULL and possible \n*/
...
const char * lv_dropdown_get_symbol(lv_obj_t * obj)
{
    LV_ASSERT_OBJ(obj, MY_CLASS);
    lv_dropdown_t * dropdown = (lv_dropdown_t *)obj;
    return (const char *)dropdown->symbol;
}
...
static void draw_main(lv_event_t * e)
{
    ...
    if(dropdown->symbol) {
        ...
        if(symbol_type == LV_IMG_SRC_SYMBOL) {
            lv_point_t size;
            lv_txt_get_size(&size, (const char *)dropdown->symbol, symbol_dsc.font, symbol_dsc.letter_space, symbol_dsc.line_space, LV_COORD_MAX,
                            symbol_dsc.flag);
            symbol_w = size.x;
            symbol_h = size.y;
        }
        ...
        if(symbol_type == LV_IMG_SRC_SYMBOL) {
            ...
            lv_draw_label(draw_ctx, &symbol_dsc, &symbol_area, (const char *)dropdown->symbol, NULL);
        }
        else {
...
static lv_res_t list_release_handler(lv_obj_t * list_obj)
{
    ...
    if(lv_indev_get_type(indev) == LV_INDEV_TYPE_ENCODER) {
        dropdown->sel_opt_id_orig = dropdown->sel_opt_id;
        lv_group_t * g      = (lv_group_t *)lv_obj_get_group(dropdown_obj);
