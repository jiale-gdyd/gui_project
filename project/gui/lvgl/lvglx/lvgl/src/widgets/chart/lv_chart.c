...
void lv_chart_set_type(lv_obj_t * obj, lv_chart_type_t type)
{
    ...
    if(chart->type == LV_CHART_TYPE_SCATTER) {
        ...
        for (ser = (lv_chart_series_t *)_lv_ll_get_tail(&chart->series_ll); ser != NULL; ser = (lv_chart_series_t *)_lv_ll_get_prev(&chart->series_ll, ser)) {
    ...
    if(type == LV_CHART_TYPE_SCATTER) {
        ...
        //_LV_LL_READ_BACK(&chart->series_ll, ser) {
        for (ser = (lv_chart_series_t *)_lv_ll_get_tail(&chart->series_ll); ser != NULL; ser = (lv_chart_series_t *)_lv_ll_get_prev(&chart->series_ll, ser)) {
            ser->x_points = (lv_coord_t *)lv_malloc(sizeof(lv_point_t) * chart->point_cnt);
...
void lv_chart_get_point_pos_by_id(lv_obj_t * obj, lv_chart_series_t * ser, uint16_t id, lv_point_t * p_out)
{
    ...
    if(chart->type == LV_CHART_TYPE_LINE) {
        ...
        for (ser_i = (lv_chart_series_t *)_lv_ll_get_tail(&chart->series_ll); ser_i != NULL; ser_i = (lv_chart_series_t *)_lv_ll_get_prev(&chart->series_ll, ser_i)) {
            if(ser_i == ser) break;
            p_out->x += col_w;
        }
...
lv_chart_series_t * lv_chart_add_series(lv_obj_t * obj, lv_color_t color, lv_chart_axis_t axis)
{
    ...
    lv_chart_t * chart    = (lv_chart_t *)obj;
    lv_chart_series_t * ser = (lv_chart_series_t *)_lv_ll_ins_head(&chart->series_ll);
    ...
    ser->color  = color;
    ser->y_points = (lv_coord_t *)lv_malloc(sizeof(lv_coord_t) * chart->point_cnt);
    LV_ASSERT_MALLOC(ser->y_points);

    if(chart->type == LV_CHART_TYPE_SCATTER) {
        ser->x_points = (lv_coord_t *)lv_malloc(sizeof(lv_coord_t) * chart->point_cnt);
        LV_ASSERT_MALLOC(ser->x_points);
    }
...
lv_chart_series_t * lv_chart_get_series_next(const lv_obj_t * obj, const lv_chart_series_t * ser)
{
    ...
    lv_chart_t * chart  = (lv_chart_t *)obj;
    if(ser == NULL) return (lv_chart_series_t *)_lv_ll_get_head(&chart->series_ll);
    else return (lv_chart_series_t *)_lv_ll_get_next(&chart->series_ll, ser);
}
...
lv_chart_cursor_t  * lv_chart_add_cursor(lv_obj_t * obj, lv_color_t color, lv_dir_t dir)
{
    ...
    lv_chart_t * chart  = (lv_chart_t *)obj;
    lv_chart_cursor_t * cursor = (lv_chart_cursor_t *)_lv_ll_ins_head(&chart->cursor_ll);
...
static void lv_chart_destructor(const lv_obj_class_t * class_p, lv_obj_t * obj)
{
    ...
    while(chart->series_ll.head) {
        ser = (lv_chart_series_t *)_lv_ll_get_head(&chart->series_ll);
    ...
    while (chart->cursor_ll.head) {
        cur = (lv_chart_cursor_t *)_lv_ll_get_head(&chart->cursor_ll);
...
static void draw_cursors(lv_obj_t * obj, lv_draw_ctx_t * draw_ctx)
{
    ...
    for (cursor = (lv_chart_cursor_t *)_lv_ll_get_tail(&chart->cursor_ll); cursor != NULL; cursor = (lv_chart_cursor_t *)_lv_ll_get_prev(&chart->cursor_ll, cursor)) {
...
static void new_points_alloc(lv_obj_t * obj, lv_chart_series_t * ser, uint32_t cnt, lv_coord_t ** a)
{
    ...
    if(ser->start_point != 0) {
        lv_coord_t * new_points = (lv_coord_t *)lv_malloc(sizeof(lv_coord_t) * cnt);
        ...
        (*a) = (lv_coord_t *)lv_realloc((*a), sizeof(lv_coord_t) * cnt);
        LV_ASSERT_MALLOC((*a));
        if((*a) == NULL) return;
