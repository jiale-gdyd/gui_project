...
lv_font_t *lv_font_load(const char *font_name)
{
    ...
    if (font) {
        memset(font, 0x00, sizeof(lv_font_t));
...
static int read_label(lv_fs_file_t *fp, int start, const char *label)
{
    ...
    if ((lv_fs_read(fp, &length, 4, NULL) != LV_FS_RES_OK) || (lv_fs_read(fp, buf, 4, NULL) != LV_FS_RES_OK) || (memcmp(label, buf, 4) != 0)) {
...
static bool load_cmaps_tables(lv_fs_file_t *fp, lv_font_fmt_txt_dsc_t *font_dsc, uint32_t cmaps_start, cmap_table_bin_t *cmap_table)
{
    ...
    for (unsigned int i = 0; i < font_dsc->cmap_num; ++i) {
        ...
        switch (cmap_table[i].format_type) {
            case LV_FONT_FMT_TXT_CMAP_FORMAT0_FULL: {
                    ...
                    uint8_t *glyph_id_ofs_list = (uint8_t *)lv_malloc(ids_size);
            ...
            case LV_FONT_FMT_TXT_CMAP_SPARSE_TINY: {
                    ...
                    if (cmap_table[i].format_type == LV_FONT_FMT_TXT_CMAP_SPARSE_FULL) {
                        uint16_t *buf = (uint16_t *)lv_malloc(sizeof(uint16_t) * cmap->list_length);
...
static int32_t load_cmaps(lv_fs_file_t *fp, lv_font_fmt_txt_dsc_t *font_dsc, uint32_t cmaps_start)
{
    ...
    lv_font_fmt_txt_cmap_t *cmaps = (lv_font_fmt_txt_cmap_t *)lv_malloc(cmaps_subtables_count * sizeof(lv_font_fmt_txt_cmap_t));
    ...
    cmap_table_bin_t *cmaps_tables = (cmap_table_bin_t *)lv_malloc(sizeof(cmap_table_bin_t) * font_dsc->cmap_num);
...
static int32_t load_glyph(lv_fs_file_t *fp, lv_font_fmt_txt_dsc_t *font_dsc, uint32_t start, uint32_t *glyph_offset, uint32_t loca_count, font_header_bin_t *header)
{
    ...
    lv_font_fmt_txt_glyph_dsc_t * glyph_dsc = (lv_font_fmt_txt_glyph_dsc_t *)lv_malloc(loca_count * sizeof(lv_font_fmt_txt_glyph_dsc_t));
...
static bool lvgl_load_font(lv_fs_file_t *fp, lv_font_t *font)
{
    ...
    bool failed = false;
    uint32_t *glyph_offset = (uint32_t *)lv_malloc(sizeof(uint32_t) * (loca_count + 1));
...
int32_t load_kern(lv_fs_file_t *fp, lv_font_fmt_txt_dsc_t *font_dsc, uint8_t format, uint32_t start)
{
    ...
    if ((lv_fs_read(fp, &kern_format_type, sizeof(uint8_t), NULL) != LV_FS_RES_OK) || (lv_fs_read(fp, &padding, 3 * sizeof(uint8_t), NULL) != LV_FS_RES_OK)) {
        return -1;
    }

    if (0 == kern_format_type) {
        lv_font_fmt_txt_kern_pair_t *kern_pair = (lv_font_fmt_txt_kern_pair_t *)lv_malloc(sizeof(lv_font_fmt_txt_kern_pair_t));
        ...
        uint8_t *glyph_ids = (uint8_t *)lv_malloc(ids_size);
        int8_t *values = (int8_t *)lv_malloc(glyph_entries);
        ...
        lv_font_fmt_txt_kern_classes_t *kern_classes = (lv_font_fmt_txt_kern_classes_t *)lv_malloc(sizeof(lv_font_fmt_txt_kern_classes_t));
        ...
        uint8_t *kern_left = (uint8_t *)lv_malloc(kern_class_mapping_length);
        uint8_t *kern_right = (uint8_t *)lv_malloc(kern_class_mapping_length);
        int8_t *kern_values = (int8_t *)lv_malloc(kern_values_length);
