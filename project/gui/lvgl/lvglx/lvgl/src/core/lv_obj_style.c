...
void lv_obj_add_style(lv_obj_t *obj, const lv_style_t *style, lv_style_selector_t selector)
{
    ...
    // 现在`i`是第一个普通样式。在此之前插入新样式

    // 为新样式分配空间并将其余样式移到最后
    obj->style_cnt++;
    LV_ASSERT(obj->style_cnt != 0);
    obj->styles = (_lv_obj_style_t *)lv_realloc(obj->styles, obj->style_cnt * sizeof(_lv_obj_style_t));
...
bool lv_obj_replace_style(struct _lv_obj_t *obj, const lv_style_t *old_style, const lv_style_t *new_style, lv_style_selector_t selector)
{
...
void lv_obj_remove_style(lv_obj_t *obj, const lv_style_t *style, lv_style_selector_t selector)
{
    ...
    while (i < obj->style_cnt) {
        lv_state_t state_act = lv_obj_style_get_selector_state(obj->styles[i].selector);
        lv_part_t part_act = lv_obj_style_get_selector_part(obj->styles[i].selector);
        if (((state != LV_STATE_ANY) && (state_act != state)) || ((part != LV_PART_ANY) && (part_act != part)) || ((style != NULL) && (style != obj->styles[i].style))) {
            i++;
            continue;
        }
...
void lv_obj_set_local_style_prop_meta(lv_obj_t *obj, lv_style_prop_t prop, uint16_t meta, lv_style_selector_t selector)
{
    lv_style_t *style = get_local_style(obj, selector);
...
void _lv_obj_style_create_transition(lv_obj_t *obj, lv_part_t part, lv_state_t prev_state, lv_state_t new_state, const _lv_obj_style_transition_dsc_t *tr_dsc)
{
    ...
    lv_style_set_prop((lv_style_t *)style_trans->style, tr_dsc->prop, v1);   // 确保`trans_style`具有有效值
    ...
    tr = (trans_t *)_lv_ll_ins_head(&LV_GC_ROOT(_lv_obj_style_trans_ll));
    LV_ASSERT_MALLOC(tr);
    if (tr == NULL) {
        return;
    }
...
static lv_style_t *get_local_style(lv_obj_t *obj,  lv_style_selector_t selector)
{
    ...
    for (i = obj->style_cnt - 1; i > 0 ; i--) {
        // 仅复制普通样式(不是本地和过渡)。新的本地样式将添加为最后的本地样式
    ...
    lv_memzero(&obj->styles[i], sizeof(_lv_obj_style_t));
    obj->styles[i].style = (lv_style_t *)lv_malloc(sizeof(lv_style_t));
...
static _lv_obj_style_t *get_trans_style(lv_obj_t *obj,  lv_style_selector_t selector)
{
    ...
    obj->styles[0].style = (lv_style_t *)lv_malloc(sizeof(lv_style_t));
...
static lv_style_res_t get_prop_core(const lv_obj_t *obj, lv_part_t part, lv_style_prop_t prop, lv_style_value_t *v)
{
    ...
    for (; i < obj->style_cnt; i++) {
        ...
        // 确保样式没有指定请求以外的其他状态。例如对于HOVER+PRESS对象状态，只有HOVER样式是可以的，但HOVER+FOCUS样式不是
...
static bool trans_del(lv_obj_t *obj, lv_part_t part, lv_style_prop_t prop, trans_t * tr_limit)
{
    ...
    tr = (trans_t *)_lv_ll_get_tail(&LV_GC_ROOT(_lv_obj_style_trans_ll));
    while (tr != NULL) {
        if (tr == tr_limit) {
            break;
        }

        // 'tr'可能会被删除，因此在'tr'有效时获取下一个对象
        tr_prev = (trans_t *)_lv_ll_get_prev(&LV_GC_ROOT(_lv_obj_style_trans_ll), tr);
...
static void trans_anim_start_cb(lv_anim_t *a)
{
    ...
    lv_style_set_prop((lv_style_t *)style_trans->style, tr->prop, tr->start_value);       // 确保`trans_style`具有有效值
}
...
static void trans_anim_ready_cb(lv_anim_t *a)
{
    ...
    // 从trans中删除transitioned属性。如果此属性没有更多过渡，则样式。它允许通过正常样式更改它
    bool running = false;
    trans_t *tr_i;
    // _LV_LL_READ(&LV_GC_ROOT(_lv_obj_style_trans_ll), tr_i) {
    for (tr_i = (trans_t *)_lv_ll_get_head(&LV_GC_ROOT(_lv_obj_style_trans_ll)); tr_i != NULL; tr_i = (trans_t *)_lv_ll_get_next(&LV_GC_ROOT(_lv_obj_style_trans_ll), tr_i)) {
...
static lv_layer_type_t calculate_layer_type(lv_obj_t *obj)
{
    if (lv_obj_get_style_transform_angle(obj, 0) != 0) {
        return LV_LAYER_TYPE_TRANSFORM;
    }

    if (lv_obj_get_style_transform_zoom(obj, 0) != 256) {
        return LV_LAYER_TYPE_TRANSFORM;
    }

    if (lv_obj_get_style_opa(obj, 0) != LV_OPA_COVER) {
...
static void fade_anim_cb(void *obj, int32_t v)
{
    lv_obj_set_style_opa((struct _lv_obj_t *)obj, v, 0);
}
