...
#define SCROLL_ANIM_TIME_MIN                200
#define SCROLL_ANIM_TIME_MAX                400
...
lv_coord_t lv_obj_get_scroll_left(lv_obj_t *obj)
{
    ...
    // 通常无法在左侧滚动对象。 所以只需将当前滚动位置用作"left size"
...
lv_coord_t lv_obj_get_scroll_right(lv_obj_t *obj)
{
    ...
    // 使用 RTL 基础目录无法滚动到右侧的对象。 所以只需将当前滚动位置用作"right size"
    if (lv_obj_get_style_base_dir(obj, LV_PART_MAIN) == LV_BASE_DIR_RTL) {
        if (obj->spec_attr == NULL) {
            return 0;
        }
    ...
    // 其他基本方向(LTR)向右滚动是正常的，所以找到最右边的坐标
    lv_coord_t child_res = LV_COORD_MIN;
    uint32_t i;
...
void lv_obj_get_scrollbar_area(lv_obj_t *obj, lv_area_t *hor_area, lv_area_t *ver_area)
{
    ...
    // 如果没有indev滚动这个对象但是 `mode==active`返回
    lv_indev_t *indev = lv_indev_get_next(NULL);
    ...
    if ((lv_obj_get_style_bg_opa(obj, LV_PART_SCROLLBAR) < LV_OPA_MIN) && (lv_obj_get_style_border_opa(obj, LV_PART_SCROLLBAR) < LV_OPA_MIN)) {
        return;
    }

    // 如果模式为ON或可以沿此方向滚动，则绘制垂直滚动条
    ...
    if (ver_draw && content_h) {
        ...
        rem = (obj_h - top_space - bottom_space - hor_req_space) - sb_h;    // 滚动条轨道的剩余大小不是滚动条本身
        lv_coord_t scroll_h = content_h - obj_h;                            // 可以真正滚动的内容的大小
    ...
    if (hor_draw && content_w) {
        ...
        rem = (obj_w - left_space - right_space - ver_reg_space) - sb_w;    // 滚动条轨道的剩余大小不是滚动条本身
        lv_coord_t scroll_w = content_w - obj_w;                            // 可以真正滚动的内容的大小
...
static void scroll_x_anim(void *obj, int32_t v)
{
    _lv_obj_scroll_by_raw((lv_obj_t *)obj, v + lv_obj_get_scroll_x((const lv_obj_t *)obj), 0);
}

static void scroll_y_anim(void *obj, int32_t v)
{
    _lv_obj_scroll_by_raw((lv_obj_t *)obj, 0, v + lv_obj_get_scroll_y((const lv_obj_t *)obj));
}

static void scroll_anim_ready_cb(lv_anim_t *a)
{
    lv_event_send((struct _lv_obj_t *)a->var, LV_EVENT_SCROLL_END, NULL);
}
