...
static void pinyin_k9_init_data(lv_obj_t * obj)
{
    ...
    uint16_t py_str_i = 0;
    uint16_t btnm_i = 0;
    for(btnm_i = 19; btnm_i < (LV_IME_PINYIN_K9_CAND_TEXT_NUM + 21); btnm_i++) {    // 19 20 21 22 23
...
static void pinyin_k9_get_legal_py(lv_obj_t * obj, char * k9_input, const char * py9_map[])
{
    ...
    ll_index = (ime_pinyin_k9_py_str_t *)_lv_ll_get_head(&pinyin_ime->k9_legal_py_ll);

    while(index != -1) {
        if(index == len) {
            if(pinyin_k9_is_valid_py(obj, py_comp)) {
                if((count >= ll_len) || (ll_len == 0)) {
                    ll_index = (ime_pinyin_k9_py_str_t *)_lv_ll_ins_tail(&pinyin_ime->k9_legal_py_ll);
                    strcpy(ll_index->py_str, py_comp);
                }
                else if((count < ll_len)) {
                    strcpy(ll_index->py_str, py_comp);
                    ll_index = (ime_pinyin_k9_py_str_t *)_lv_ll_get_next(&pinyin_ime->k9_legal_py_ll, ll_index);
                }
                count++;
            }
            index--;
...
static void pinyin_k9_fill_cand(lv_obj_t * obj)
{
    ...
    while(ll_index) {
        ...
        ll_index = (ime_pinyin_k9_py_str_t *)_lv_ll_get_next(&pinyin_ime->k9_legal_py_ll, ll_index); /*Find the next list*/
        index++;
    }
...
static void pinyin_k9_cand_page_proc(lv_obj_t * obj, uint16_t dir)
{
    ...
    if((ll_len > LV_IME_PINYIN_K9_CAND_TEXT_NUM) && (pinyin_ime->k9_legal_py_count > LV_IME_PINYIN_K9_CAND_TEXT_NUM)) {
        ...
        // previous page
            ...
            ll_index = (ime_pinyin_k9_py_str_t *)_lv_ll_get_prev(&pinyin_ime->k9_legal_py_ll, ll_index);
            while(ll_index) {
                if(count < 0)  break;

                strcpy(lv_pinyin_k9_cand_str[count], ll_index->py_str);
                ll_index = (ime_pinyin_k9_py_str_t *)_lv_ll_get_prev(&pinyin_ime->k9_legal_py_ll, ll_index); /*Find the previous list*/
                count--;
            }
