...
{
    lv_fragment_manager_t * instance = (lv_fragment_manager_t *)lv_malloc(sizeof(lv_fragment_manager_t));
...
void lv_fragment_manager_del(lv_fragment_manager_t * manager)
{
    ...
    for (states = (lv_fragment_managed_states_t *)_lv_ll_get_tail(&manager->attached); states != NULL; states = (lv_fragment_managed_states_t *)_lv_ll_get_prev(&manager->attached, states)) {
        item_del_obj(states);
...
{
    LV_ASSERT_NULL(manager);
    lv_fragment_stack_item_t * top = (lv_fragment_stack_item_t *)_lv_ll_get_tail(&manager->stack);
    lv_fragment_managed_states_t * states = NULL;
    //_LV_LL_READ(&manager->attached, states) {
    for (states = (lv_fragment_managed_states_t *)_lv_ll_get_head(&manager->attached); states != NULL; states = (lv_fragment_managed_states_t *)_lv_ll_get_next(&manager->attached, states)) {
...
void lv_fragment_manager_remove(lv_fragment_manager_t * manager, lv_fragment_t * fragment)
{
    ...
    if(states->in_stack) {
        ...
        for (item = (lv_fragment_stack_item_t *)_lv_ll_get_tail(&manager->stack); item != NULL; item = (lv_fragment_stack_item_t *)_lv_ll_get_prev(&manager->stack, item)) {
...
void lv_fragment_manager_push(lv_fragment_manager_t * manager, lv_fragment_t * fragment, lv_obj_t * const * container)
{
    ...
    lv_fragment_stack_item_t * item = (lv_fragment_stack_item_t *)_lv_ll_ins_tail(&manager->stack);
...
bool lv_fragment_manager_send_event(lv_fragment_manager_t * manager, int code, void * userdata)
{
    ...
    for (p = (lv_fragment_managed_states_t *)_lv_ll_get_tail(&manager->attached); p!= NULL; p = (lv_fragment_managed_states_t *)_lv_ll_get_prev(&manager->attached, p)) {
                                                      ...
                                                      lv_obj_t * const * container)
{
    ...
    lv_fragment_managed_states_t * states = (lv_fragment_managed_states_t *)_lv_ll_ins_tail(&manager->attached);
