...
LV_ATTRIBUTE_TIMER_HANDLER uint32_t lv_timer_handler(void)
{
    ...
    do {
        timer_deleted             = false;
        timer_created             = false;
        LV_GC_ROOT(_lv_timer_act) = (lv_timer_t *)_lv_ll_get_head(&LV_GC_ROOT(_lv_timer_ll));
        while(LV_GC_ROOT(_lv_timer_act)) {
            /*The timer might be deleted if it runs only once ('repeat_count = 1')
             *So get next element until the current is surely valid*/
            next = (lv_timer_t *)_lv_ll_get_next(&LV_GC_ROOT(_lv_timer_ll), LV_GC_ROOT(_lv_timer_act));
    ...
    next = (lv_timer_t *)_lv_ll_get_head(&LV_GC_ROOT(_lv_timer_ll));
    while(next) {
        if(!next->paused) {
        ...
        next = (lv_timer_t *)_lv_ll_get_next(&LV_GC_ROOT(_lv_timer_ll), next); /*Find the next timer*/
    }
...
lv_timer_t * lv_timer_create(lv_timer_cb_t timer_xcb, uint32_t period, void * user_data)
{
    ...
    new_timer = (lv_timer_t *)_lv_ll_ins_head(&LV_GC_ROOT(_lv_timer_ll));
...
{
    if(timer == NULL) return (lv_timer_t *)_lv_ll_get_head(&LV_GC_ROOT(_lv_timer_ll));
    else return (lv_timer_t *)_lv_ll_get_next(&LV_GC_ROOT(_lv_timer_ll), timer);
}
