...
lv_fs_res_t lv_fs_open(lv_fs_file_t * file_p, const char * path, lv_fs_mode_t mode)
{
    ...
    if(drv->cache_size) {
        file_p->cache = (lv_fs_file_cache_t *)lv_malloc(sizeof(lv_fs_file_cache_t));
...
static lv_fs_res_t lv_fs_read_cached(lv_fs_file_t * file_p, char * buf, uint32_t btr, uint32_t * br)
{
    ...
    char * buffer = (char *)file_p->cache->buffer;
    ...
    if(start <= file_position && file_position < end) {
        ...
        if(btr > buffer_size) {
            ...
            if(buffer == NULL) {
                ...
                buffer = (char *)file_p->cache->buffer;
            }
...
void lv_fs_drv_register(lv_fs_drv_t * drv_p)
{
    /*Save the new driver*/
    lv_fs_drv_t ** new_drv;
    new_drv = (lv_fs_drv_t **)_lv_ll_ins_head(&LV_GC_ROOT(_lv_fsdrv_ll));
...
lv_fs_drv_t * lv_fs_get_drv(char letter)
{
    ...
    for (drv = (lv_fs_drv_t **)_lv_ll_get_head(&LV_GC_ROOT(_lv_fsdrv_ll)); drv != NULL; drv = (lv_fs_drv_t **)_lv_ll_get_next(&LV_GC_ROOT(_lv_fsdrv_ll), drv)) {
