...
void * _lv_ll_ins_head(lv_ll_t * ll_p)
{
    ...
    n_new = (lv_ll_node_t *)lv_malloc(ll_p->n_size + LL_NODE_META_SIZE);
...
void * _lv_ll_ins_prev(lv_ll_t * ll_p, void * n_act)
{
    ...
    if(_lv_ll_get_head(ll_p) == n_act) {
        n_new = (lv_ll_node_t *)_lv_ll_ins_head(ll_p);
        ...
        lv_ll_node_t * n_prev;
        n_prev = (lv_ll_node_t *)_lv_ll_get_prev(ll_p, n_act);
        node_set_next(ll_p, n_prev, n_new);
        node_set_prev(ll_p, n_new, n_prev);
        node_set_prev(ll_p, (lv_ll_node_t *)n_act, (lv_ll_node_t *)n_new);
        node_set_next(ll_p, n_new, (lv_ll_node_t *)n_act);
    }
...
void _lv_ll_remove(lv_ll_t * ll_p, void * node_p)
{
    ...
    if(_lv_ll_get_head(ll_p) == node_p) {
        ...
        ll_p->head = (lv_ll_node_t *)_lv_ll_get_next(ll_p, node_p);
        if(ll_p->head == NULL) {
            ll_p->tail = NULL;
        }
        else {
        ...
        ll_p->tail = (lv_ll_node_t *)_lv_ll_get_prev(ll_p, node_p);
        if(ll_p->tail == NULL) {
            ll_p->head = NULL;
        }
        else {
        ...
        lv_ll_node_t * n_prev = (lv_ll_node_t *)_lv_ll_get_prev(ll_p, node_p);
        lv_ll_node_t * n_next = (lv_ll_node_t *)_lv_ll_get_next(ll_p, node_p);
...
void _lv_ll_chg_list(lv_ll_t * ll_ori_p, lv_ll_t * ll_new_p, void * node, bool head)
{
    ...
    if(head) {
        /*Set node as head*/
        node_set_prev(ll_new_p, (lv_ll_node_t *)node, NULL);
        node_set_next(ll_new_p, (lv_ll_node_t *)node, ll_new_p->head);

        if(ll_new_p->head != NULL) { /*If there is old head then before it goes the new*/
            node_set_prev(ll_new_p, ll_new_p->head, (lv_ll_node_t *)node);
        }

        ll_new_p->head = (lv_ll_node_t *)node;       /*Set the new head in the dsc.*/
        if(ll_new_p->tail == NULL) { /*If there is no tail (first node) set the tail too*/
            ll_new_p->tail = (lv_ll_node_t *)node;
        }
    }
    else {
        /*Set node as tail*/
        node_set_prev(ll_new_p, (lv_ll_node_t *)node, ll_new_p->tail);
        node_set_next(ll_new_p, (lv_ll_node_t *)node, NULL);

        if(ll_new_p->tail != NULL) { /*If there is old tail then after it goes the new*/
            node_set_next(ll_new_p, ll_new_p->tail, (lv_ll_node_t *)node);
        }

        ll_new_p->tail = (lv_ll_node_t *)node;       /*Set the new tail in the dsc.*/
        if(ll_new_p->head == NULL) { /*If there is no head (first node) set the head too*/
            ll_new_p->head = (lv_ll_node_t *)node;
        }
    }
}
...
void * _lv_ll_get_next(const lv_ll_t * ll_p, const void * n_act)
{
    ...
    const lv_ll_node_t * n_act_d = (const lv_ll_node_t *)n_act;
...
void _lv_ll_move_before(lv_ll_t * ll_p, void * n_act, void * n_after)
{
    ...
    node_set_next(ll_p, (lv_ll_node_t *)n_before, (lv_ll_node_t *)n_act);
    node_set_prev(ll_p, (lv_ll_node_t *)n_act, (lv_ll_node_t *)n_before);
    node_set_prev(ll_p, (lv_ll_node_t *)n_after, (lv_ll_node_t *)n_act);
    node_set_next(ll_p, (lv_ll_node_t *)n_act, (lv_ll_node_t *)n_after);

    /*If `n_act` was moved before NULL then it become the new tail*/
    if(n_after == NULL) ll_p->tail = (lv_ll_node_t *)n_act;

    /*If `n_act` was moved before `NULL` then it's the new head*/
    if(n_before == NULL) ll_p->head = (lv_ll_node_t *)n_act;
}
