...
static void flex_update(lv_obj_t * cont, void * user_data)
{
    ...
    lv_coord_t abs_y = cont->coords.y1 + lv_obj_get_style_space_top(cont, LV_PART_MAIN) - lv_obj_get_scroll_y(cont);
    lv_coord_t abs_x = cont->coords.x1 + lv_obj_get_style_space_left(cont, LV_PART_MAIN) - lv_obj_get_scroll_x(cont);
    ...
    while(item) {
        ...
        if(!lv_obj_has_flag_any(item, LV_OBJ_FLAG_IGNORE_LAYOUT | LV_OBJ_FLAG_HIDDEN | LV_OBJ_FLAG_FLOATING)) {
            ...
            if(grow_value) {
                ...
                if(t->grow_dsc_calc) {
                    grow_dsc_t * new_dsc = (grow_dsc_t *)lv_realloc(t->grow_dsc, sizeof(grow_dsc_t) * (t->grow_item_cnt));
