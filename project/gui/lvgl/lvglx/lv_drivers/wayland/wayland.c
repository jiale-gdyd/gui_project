...
{
    struct application *app = data;
    const lv_indev_state_t lv_state = (state == WL_POINTER_BUTTON_STATE_PRESSED) ? LV_INDEV_STATE_PR : LV_INDEV_STATE_REL;
...
static void keyboard_handle_key(void *data, struct wl_keyboard *keyboard, uint32_t serial, uint32_t time, uint32_t key, uint32_t state)
{
    ...
    const lv_indev_state_t lv_state = (state == WL_KEYBOARD_KEY_STATE_PRESSED) ? LV_INDEV_STATE_PR : LV_INDEV_STATE_REL;
...
static bool initialize_buffer(struct window *window, struct buffer_hdl *buffer_hdl, int width, int height)
{
    int ret;
    ...
    LV_LOG_TRACE("initializing buffer %dx%d (alloc size: %d)", width, height, buffer_hdl->size);

    if (allocator->shm_file_free_size < buffer_hdl->size)  {
        do {
            ret = ftruncate(allocator->shm_mem_fd, allocator->shm_mem_size + (buffer_hdl->size - allocator->shm_file_free_size));
    ...
    buffer_hdl->base = mmap(NULL, buffer_hdl->size, PROT_READ | PROT_WRITE, MAP_SHARED, allocator->shm_mem_fd, allocator->shm_mem_size - allocator->shm_file_free_size);
    ...
    if (!allocator->shm_pool) {
        allocator->shm_pool = wl_shm_create_pool(app->shm, allocator->shm_mem_fd, allocator->shm_mem_size + allocated_size);
        if (!allocator->shm_pool) {
            LV_LOG_ERROR("cannot create shm pool");
            goto err_unmap;
        }
    } else if (allocated_size > 0) {
        wl_shm_pool_resize(allocator->shm_pool, allocator->shm_mem_size + allocated_size);
    }

    buffer_hdl->wl_buffer = wl_shm_pool_create_buffer(allocator->shm_pool, allocator->shm_mem_size - allocator->shm_file_free_size, width, height, width * window->bits_per_pixel / 8, app->format);
...
static struct graphic_object *create_graphic_obj(struct application *app, struct window *window, enum object_type type, struct graphic_object *parent)
{
...
#if LV_WAYLAND_CLIENT_SIDE_DECORATIONS
static bool create_decoration(struct window *window, struct graphic_object * decoration, int window_width, int window_height)
{
    int x, y;
    ...
    switch (decoration->type) {
        case OBJECT_TITLEBAR:
            lv_color_fill((lv_color_t *)buffer->base, lv_color_make(0x66, 0x66, 0x66), (decoration->width * decoration->height));
            break;

        case OBJECT_BUTTON_CLOSE:
            lv_color_fill((lv_color_t *)buffer->base, lv_color_make(0xCC, 0xCC, 0xCC), (decoration->width * decoration->height));
...
static bool attach_decoration(struct window *window, struct graphic_object *decoration, struct graphic_object * parent)
{
    int x, y;
    int pos_x, pos_y;
    struct buffer_hdl *buffer = &decoration->buffer;
    ...
    decoration->subsurface = wl_subcompositor_get_subsurface(window->application->subcompositor, decoration->surface, parent->surface);
...
static void detach_decoration(struct window *window, struct graphic_object *decoration)
{
    if (decoration->subsurface) {
...
static bool resize_window(struct window *window, int width, int height)
{
    ...
    if (!window->application->opt_disable_decorations && !window->fullscreen) {
        for (b = 0; b < NUM_DECORATIONS; b++) {
            if (!create_decoration(window, window->decoration[b], window->body->width, window->body->height)) {
...
static void _lv_wayland_flush(lv_disp_drv_t *disp_drv, const lv_area_t *area, lv_color_t *color_p)
{
...
#elif (LV_COLOR_DEPTH == 1)
                uint8_t *const buf = (uint8_t *)buffer->base + offset;
                *buf = ((0x07 * color_p->ch.red) << 5) | ((0x07 * color_p->ch.green) << 2) | ((0x03 * color_p->ch.blue)  << 0);
#endif
                color_p++;
            }
        }
    ...
    wl_surface_damage(window->body->surface, area->x1, area->y1, (area->x2 - area->x1 + 1), (area->y2 - area->y1 + 1));
