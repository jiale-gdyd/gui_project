...
void lv_rlottie_set_play_mode(lv_obj_t *obj, const lv_rlottie_ctrl_t ctrl)
{
    ...
    if (rlottie->task && (rlottie->dest_frame != rlottie->current_frame || (rlottie->play_ctrl & LV_RLOTTIE_CTRL_PAUSE) == LV_RLOTTIE_CTRL_PLAY)) {
...
static void lv_rlottie_constructor(const lv_obj_class_t *class_p, lv_obj_t *obj)
{
    ...
    rlottie->allocated_buf = (uint32_t *)lv_malloc(allocaled_buf_size);
    if (rlottie->allocated_buf != NULL) {
        rlottie->allocated_buffer_size = allocaled_buf_size;
        memset(rlottie->allocated_buf, 0x00, allocaled_buf_size);
    }
    ...
    rlottie->imgdsc.data = (const uint8_t *)rlottie->allocated_buf;
    rlottie->imgdsc.data_size = allocaled_buf_size;

    lv_img_set_src(obj, &rlottie->imgdsc);

    rlottie->play_ctrl = (lv_rlottie_ctrl_t)(LV_RLOTTIE_CTRL_FORWARD | LV_RLOTTIE_CTRL_PLAY | LV_RLOTTIE_CTRL_LOOP);
    rlottie->dest_frame = rlottie->total_frames;
...
static void next_frame_task_cb(lv_timer_t *t)
{
    ...
    lottie_animation_render(rlottie->animation, rlottie->current_frame, rlottie->allocated_buf, rlottie->imgdsc.header.w, rlottie->imgdsc.header.h, rlottie->scanline_width);
