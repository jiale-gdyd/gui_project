                                                            ...
                                                            lv_coord_t len)
{
    ...
    while(m->param) {
        dsc = (_lv_draw_mask_common_dsc_t *)m->param;
                                                                ...
                                                                lv_coord_t len, const int16_t *ids, int16_t ids_count)
{
    ...
    for (int i = 0; i < ids_count; i++) {
        ...
        dsc = (_lv_draw_mask_common_dsc_t *)LV_GC_ROOT(_lv_draw_mask_list[id]).param;
        if (!dsc) continue;
        lv_draw_mask_res_t res = LV_DRAW_MASK_RES_FULL_COVER;
...
void * lv_draw_mask_remove_id(int16_t id)
{
    ...
    if(id != LV_MASK_ID_INV) {
        p = (_lv_draw_mask_common_dsc_t *)LV_GC_ROOT(_lv_draw_mask_list[id]).param;
...
void * lv_draw_mask_remove_custom(void * custom_id)
{
    ...
    for(i = 0; i < _LV_MASK_MAX_NUM; i++) {
        if(LV_GC_ROOT(_lv_draw_mask_list[i]).custom_id == custom_id) {
            p = (_lv_draw_mask_common_dsc_t *)LV_GC_ROOT(_lv_draw_mask_list[i]).param;
...
{
    _lv_draw_mask_common_dsc_t * pdsc = (_lv_draw_mask_common_dsc_t *)p;
...
bool lv_draw_mask_is_any(const lv_area_t * a)
{
    ...
    uint8_t i;
    for(i = 0; i < _LV_MASK_MAX_NUM; i++) {
        _lv_draw_mask_common_dsc_t * comm_param = (_lv_draw_mask_common_dsc_t *)LV_GC_ROOT(_lv_draw_mask_list[i]).param;
        if(comm_param == NULL) continue;
        if(comm_param->type == LV_DRAW_MASK_TYPE_RADIUS) {
            lv_draw_mask_radius_param_t * radius_param = (lv_draw_mask_radius_param_t *)LV_GC_ROOT(_lv_draw_mask_list[i]).param;
...
void lv_draw_mask_radius_init(lv_draw_mask_radius_param_t * param, const lv_area_t * rect, lv_coord_t radius, bool inv)
{
    ...
    if(!entry) {
        entry = (_lv_draw_mask_radius_circle_dsc_t *)lv_malloc(sizeof(_lv_draw_mask_radius_circle_dsc_t));
...
void lv_draw_mask_polygon_init(lv_draw_mask_polygon_param_t * param, const lv_point_t * points, uint16_t point_cnt)
{
    ...
    lv_point_t * p = (lv_point_t *)lv_malloc(point_cnt * sizeof(lv_point_t));
    if(p == NULL) return;
    uint16_t i;
    uint16_t pcnt = 0;
    p[0] = points[0];
...
static void circ_calc_aa4(_lv_draw_mask_radius_circle_dsc_t * c, lv_coord_t radius)
{
    ...
    c->buf = (uint8_t *)lv_malloc(radius * 6 + 6);  /*Use uint16_t for opa_start_on_y and x_start_on_y*/
    ...
    lv_coord_t * cir_x = (lv_coord_t *)lv_malloc((radius + 1) * 2 * 2 * sizeof(lv_coord_t));
