...
static void draw_bg_img(lv_draw_ctx_t * draw_ctx, const lv_draw_rect_dsc_t * dsc, const lv_area_t * coords)
{
    ...
    if(src_type == LV_IMG_SRC_SYMBOL) {
        lv_point_t size;
        lv_txt_get_size(&size, (const char *)dsc->bg_img_src, (const lv_font_t *)dsc->bg_img_symbol_font, 0, 0, LV_COORD_MAX, LV_TEXT_FLAG_NONE);
        lv_area_t a;
        ...
        label_draw_dsc.font = (const lv_font_t *)dsc->bg_img_symbol_font;
        label_draw_dsc.color = dsc->bg_img_recolor;
        label_draw_dsc.opa = dsc->bg_img_opa;
        lv_draw_label(draw_ctx, &label_draw_dsc, &a, (const char *)dsc->bg_img_src, NULL);
    }
    else {
                                              ...
                                              const lv_area_t * coords)
{
...
#else
    sh_buf = (lv_opa_t *)lv_malloc(corner_size * corner_size * sizeof(uint16_t));
    shadow_draw_corner_buf(&core_area, (uint16_t *)sh_buf, dsc->shadow_width, r_sh);
#endif
    ...
    lv_opa_t * mask_buf = (lv_opa_t *)lv_malloc(lv_area_get_width(&shadow_area));
    lv_area_t blend_area;
                                                         ...
                                                         lv_coord_t r)
{
    ...
    int32_t y;
    lv_opa_t * mask_line = (lv_opa_t *)lv_malloc(size);
...
LV_ATTRIBUTE_FAST_MEM static void shadow_blur_corner(lv_coord_t size, lv_coord_t sw, uint16_t * sh_ups_buf)
{
    ...
    /*Horizontal blur*/
    uint16_t * sh_ups_blur_buf = (uint16_t *)lv_malloc(size * sizeof(uint16_t));
                         ...
                         lv_coord_t rout, lv_coord_t rin, lv_color_t color, lv_opa_t opa, lv_blend_mode_t blend_mode)
{
    ...
    lv_draw_sw_blend_dsc_t blend_dsc;
    lv_memzero(&blend_dsc, sizeof(blend_dsc));
    blend_dsc.mask_buf = (lv_opa_t *)lv_malloc(draw_area_w);
