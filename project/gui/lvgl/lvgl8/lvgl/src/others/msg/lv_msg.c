...
{
    sub_dsc_t * s = (sub_dsc_t *)_lv_ll_ins_tail(&LV_GC_ROOT(_subs_ll));
    LV_ASSERT_MALLOC(s);
...
{
    sub_dsc_t * s = (sub_dsc_t *)lv_msg_subscribe(msg_id, obj_notify_cb, user_data);
    if(s == NULL) return NULL;
    s->_priv_data = obj;

    /*If not added yet, add a delete event cb which automatically unsubcribes the object*/
    sub_dsc_t * s_first = (sub_dsc_t *)lv_obj_get_event_user_data(obj, obj_delete_event_cb);
    if(s_first == NULL) {
...
lv_msg_t * lv_event_get_msg(lv_event_t * e)
{
    if(e->code == LV_EVENT_MSG_RECEIVED) {
        return (lv_msg_t *)lv_event_get_param(e);
    }
    else {
...
static void notify(lv_msg_t * m)
{
    ...
    for (s = (sub_dsc_t *)_lv_ll_get_head(&LV_GC_ROOT(_subs_ll)); s != NULL; s = (sub_dsc_t *)_lv_ll_get_next(&LV_GC_ROOT(_subs_ll), s)) {
...
static void obj_notify_cb(lv_msg_t * m)
{
    lv_event_send((lv_obj_t *)m->_priv_data, LV_EVENT_MSG_RECEIVED, m);
}
...
static void obj_delete_event_cb(lv_event_t * e)
{
    ...
    sub_dsc_t * s = (sub_dsc_t *)_lv_ll_get_head(&LV_GC_ROOT(_subs_ll));
    sub_dsc_t * s_next;
    while(s) {
        /*On unsubscribe the list changes s becomes invalid so get next item while it's surely valid*/
        s_next = (sub_dsc_t *)_lv_ll_get_next(&LV_GC_ROOT(_subs_ll), s);
        if(s->_priv_data == obj) {
