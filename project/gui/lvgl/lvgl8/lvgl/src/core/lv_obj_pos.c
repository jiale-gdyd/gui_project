...
bool lv_obj_refr_size(lv_obj_t *obj)
{
    ...
    // calc_auto_size将滚动x/y设置为0，以便恢复原始值
    if (w_is_content || h_is_content) {
        lv_obj_scroll_to(obj, sl_ori, st_ori, LV_ANIM_OFF);
    }

    // 如果大小没有改变，则什么都不做。非常重要，否则可以在不改变大小的情况下进行递归调整大小
    ...
    // 如果对象不在父对象之外，则新滚动条区域也会失效。如果它不是在父之外而是现在，也使滚动条无效
...
{
    layout_cnt++;
    LV_GC_ROOT(_lv_layout_list) = (lv_layout_dsc_t *)lv_realloc(LV_GC_ROOT(_lv_layout_list), layout_cnt * sizeof(lv_layout_dsc_t));
...
void lv_obj_move_to(lv_obj_t *obj, lv_coord_t x, lv_coord_t y)
{
    ...
    // 如果位置没有改变，则什么都不做。非常重要，否则可以在不改变位置的情况下进行递归定位
...
void lv_obj_transform_point(const lv_obj_t *obj, lv_point_t *p, bool recursive, bool inv)
{
    if (obj) {
        ...
        if (inv) {
            if (recursive) {
                lv_obj_transform_point(lv_obj_get_parent(obj), p, recursive, inv);
            }
...
bool lv_obj_area_is_visible(const lv_obj_t *obj, lv_area_t *area)
{
    ...
    while (par != NULL) {
        ...
        if (!lv_obj_has_flag_any(par, LV_OBJ_FLAG_OVERFLOW_VISIBLE)) {
            ...
            if (!_lv_area_intersect(area, area, &par_area)) {
                return false;
            }
        }
