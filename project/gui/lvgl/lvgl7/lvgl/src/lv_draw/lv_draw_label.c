                                         ...
                                         lv_draw_label_hint_t * hint)
{
    ...
    while(txt[line_start] != '\0') {
        ...
        i         = 0;
#if LV_USE_BIDI
        char * bidi_txt = (char *)_lv_mem_buf_get(line_end - line_start + 1);
                                                     ...
                                                     const uint8_t * map_p, lv_color_t color, lv_opa_t opa, lv_blend_mode_t blend_mode)
{
    ...
    lv_opa_t * mask_buf = (lv_opa_t *)_lv_mem_buf_get(mask_buf_size);
    int32_t mask_p = 0;
                              ...
                              const uint8_t * map_p, lv_color_t color, lv_opa_t opa, lv_blend_mode_t blend_mode)
{
    ...
    lv_color_t * color_buf = (lv_color_t *)_lv_mem_buf_get(mask_buf_size * sizeof(lv_color_t));

    lv_disp_t * disp    = _lv_refr_get_disp_refreshing();
    lv_disp_buf_t * vdb = lv_disp_get_buf(disp);

    int32_t vdb_width     = lv_area_get_width(&vdb->area);
    lv_color_t * vdb_buf_tmp = (lv_color_t *)vdb->buf_act;
