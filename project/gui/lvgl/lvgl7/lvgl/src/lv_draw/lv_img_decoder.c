...
lv_res_t lv_img_decoder_get_info(const char * src, lv_img_header_t * header)
{
    ...
    for (d = (lv_img_decoder_t *)_lv_ll_get_head(&LV_GC_ROOT(_lv_img_defoder_ll)); d != NULL; d = (lv_img_decoder_t *)_lv_ll_get_next(&LV_GC_ROOT(_lv_img_defoder_ll), d)) {
        if(d->info_cb) {
...
{
    lv_img_decoder_t * decoder;
    decoder = (lv_img_decoder_t *)_lv_ll_ins_head(&LV_GC_ROOT(_lv_img_defoder_ll));
    LV_ASSERT_MEM(decoder);
...
lv_res_t lv_img_decoder_built_in_info(lv_img_decoder_t * decoder, const void * src, lv_img_header_t * header)
{
    ...
    if(src_type == LV_IMG_SRC_VARIABLE) {
        ...
        lv_fs_res_t res;
        uint32_t rn;
        res = lv_fs_open(&file, (const char *)src, LV_FS_MODE_RD);
        if(res == LV_FS_RES_OK) {
...
lv_res_t lv_img_decoder_built_in_open(lv_img_decoder_t * decoder, lv_img_decoder_dsc_t * dsc)
{
    ...
    /*Process indexed images. Build a palette*/
        ...
        lv_img_decoder_built_in_data_t * user_data = (lv_img_decoder_built_in_data_t *)dsc->user_data;
        user_data->palette                         = (lv_color_t *)lv_mem_alloc(palette_size * sizeof(lv_color_t));
        LV_ASSERT_MEM(user_data->palette);
        user_data->opa                             = (lv_opa_t *)lv_mem_alloc(palette_size * sizeof(lv_opa_t));
...
#if LV_USE_FILESYSTEM
    lv_img_decoder_built_in_data_t * user_data = (lv_img_decoder_built_in_data_t *)dsc->user_data;
    uint8_t * fs_buf = (uint8_t *)_lv_mem_buf_get(w);
