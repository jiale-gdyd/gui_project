                                          ...
                                          const lv_draw_rect_dsc_t * dsc)
{
    ...
    lv_opa_t * mask_buf = (lv_opa_t *)_lv_mem_buf_get(draw_area_w);
    ...
    /*More complex case: there is a radius, gradient or other mask.*/
        ...
        if(grad_dir == LV_GRAD_DIR_HOR) {
            grad_map = (lv_color_t *)_lv_mem_buf_get(coords_w * sizeof(lv_color_t));
                                              ...
                                              const lv_draw_rect_dsc_t * dsc)
{
    ...
    if(sh_cache_size == corner_size && sh_cache_r == r_sh) {
        ...
        sh_buf = (lv_opa_t *)_lv_mem_buf_get(corner_size * corner_size);
        _lv_memcpy(sh_buf, sh_cache, corner_size * corner_size);
    }
    else {
        /*A larger buffer is required for calculation */
        sh_buf = (lv_opa_t *)_lv_mem_buf_get(corner_size * corner_size * sizeof(uint16_t));
    ...
    /*Create a mask*/
    lv_draw_mask_res_t mask_res;
    lv_opa_t * mask_buf = (lv_opa_t *)_lv_mem_buf_get(lv_area_get_width(&sh_area));
                                                         ...
                                                         lv_coord_t r)
{
    ...
    int32_t y;
    lv_opa_t * mask_line = (lv_opa_t *)_lv_mem_buf_get(size);
...
LV_ATTRIBUTE_FAST_MEM static void shadow_blur_corner(lv_coord_t size, lv_coord_t sw, uint16_t * sh_ups_buf)
{
    ...
    /*Horizontal blur*/
    uint16_t * sh_ups_blur_buf = (uint16_t *)_lv_mem_buf_get(size * sizeof(uint16_t));
...
static void draw_pattern(const lv_area_t * coords, const lv_area_t * clip, const lv_draw_rect_dsc_t * dsc)
{
    ...
    if(src_type == LV_IMG_SRC_FILE || src_type == LV_IMG_SRC_VARIABLE) {
        ...
        lv_res_t res = lv_img_decoder_get_info((const char *)dsc->pattern_image, &header);
        if(res != LV_RES_OK) {
        ...
        lv_point_t s;
        _lv_txt_get_size(&s, (const char *)dsc->pattern_image, label_dsc.font, label_dsc.letter_space, label_dsc.line_space, LV_COORD_MAX,
                         LV_TXT_FLAG_NONE);
        img_w = s.x;
        img_h = s.y;
    ...
    if(dsc->pattern_repeat) {
        ...
        for(; coords_tmp.y1 <= coords->y2; coords_tmp.y1 += img_h, coords_tmp.y2 += img_h) {
            ...
            for(; coords_tmp.x1 <= coords->x2; coords_tmp.x1 += img_w, coords_tmp.x2 += img_w) {
                if(src_type == LV_IMG_SRC_SYMBOL)  lv_draw_label(&coords_tmp, clip, &label_dsc, (const char *)dsc->pattern_image, NULL);
...
static void draw_value_str(const lv_area_t * coords, const lv_area_t * clip, const lv_draw_rect_dsc_t * dsc)
{
    ...
    char * str = (char *)_lv_mem_buf_get(str_len + 1);
