...
LV_ATTRIBUTE_TASK_HANDLER uint32_t lv_task_handler(void)
{
    ...
    do {
        ...
        LV_GC_ROOT(_lv_task_act) = (lv_task_t *)_lv_ll_get_head(&LV_GC_ROOT(_lv_task_ll));
        while(LV_GC_ROOT(_lv_task_act)) {
            /* The task might be deleted if it runs only once ('once = 1')
             * So get next element until the current is surely valid*/
            next = (lv_task_t *)_lv_ll_get_next(&LV_GC_ROOT(_lv_task_ll), LV_GC_ROOT(_lv_task_act));
    ...
    next = (lv_task_t *)_lv_ll_get_head(&LV_GC_ROOT(_lv_task_ll));
    while(next && next->prio != LV_TASK_PRIO_OFF) {
        uint32_t delay = lv_task_time_remaining(next);
        if(delay < time_till_next)
            time_till_next = delay;

        next = (lv_task_t *)_lv_ll_get_next(&LV_GC_ROOT(_lv_task_ll), next); /*Find the next task*/
    }
...
lv_task_t * lv_task_create(lv_task_cb_t task_xcb, uint32_t period, lv_task_prio_t prio, void * user_data)
{
    ...
    tmp = (lv_task_t *)_lv_ll_get_head(&LV_GC_ROOT(_lv_task_ll));

    /*It's the first task*/
    if(NULL == tmp) {
        new_task = (lv_task_t *)_lv_ll_ins_head(&LV_GC_ROOT(_lv_task_ll));
    ...
    /*Insert the new task to proper place according to its priority*/
        ...
        do {
            if(tmp->prio <= prio) {
                new_task = (lv_task_t *)_lv_ll_ins_prev(&LV_GC_ROOT(_lv_task_ll), tmp);
                LV_ASSERT_MEM(new_task);
                if(new_task == NULL) return NULL;
                break;
            }
            tmp = (lv_task_t *)_lv_ll_get_next(&LV_GC_ROOT(_lv_task_ll), tmp);
        } while(tmp != NULL);

        /*Only too high priority tasks were found. Add the task to the end*/
        if(tmp == NULL) {
            new_task = (lv_task_t *)_lv_ll_ins_tail(&LV_GC_ROOT(_lv_task_ll));
            LV_ASSERT_MEM(new_task);
            if(new_task == NULL) return NULL;
        }
    }
...
void lv_task_set_prio(lv_task_t * task, lv_task_prio_t prio)
{
    ...
    for (i = (lv_task_t *)_lv_ll_get_head(&LV_GC_ROOT(_lv_task_ll)); i != NULL; i = (lv_task_t *)_lv_ll_get_next(&LV_GC_ROOT(_lv_task_ll), i)) {
        if(i->prio <= prio) {
...
{
    if(task == NULL) return (lv_task_t *)_lv_ll_get_head(&LV_GC_ROOT(_lv_task_ll));
    else return (lv_task_t *)_lv_ll_get_next(&LV_GC_ROOT(_lv_task_ll), task);
}
