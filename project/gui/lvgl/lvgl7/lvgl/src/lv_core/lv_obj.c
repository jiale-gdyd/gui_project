...
lv_obj_t * lv_obj_create(lv_obj_t * parent, const lv_obj_t * copy)
{
    ...
    if(parent == NULL) {
        ...
        new_obj = (lv_obj_t *)_lv_ll_ins_head(&disp->scr_ll);
    ...
    /*Create a normal object*/
        ...
        new_obj = (lv_obj_t *)_lv_ll_ins_head(&parent->child_ll);
        LV_ASSERT_MEM(new_obj);
        if(new_obj == NULL) return NULL;
    ...
    if(copy != NULL) {
        ...
        if(copy->group_p != NULL) {
            lv_group_add_obj((lv_group_t *)copy->group_p, new_obj);
        }
#endif
...
void lv_obj_set_size(lv_obj_t * obj, lv_coord_t w, lv_coord_t h)
{
    ...
    for (i = (lv_obj_t *)_lv_ll_get_head(&obj->child_ll); i != NULL; i = (lv_obj_t *)_lv_ll_get_next(&obj->child_ll, i)) {
...
void lv_obj_report_style_mod(lv_style_t * style)
{
    ...
    while(d) {
        ...
        for (i = (lv_obj_t *)_lv_ll_get_head(&d->scr_ll); i != NULL; i = (lv_obj_t *)_lv_ll_get_next(&d->scr_ll, i)) {
...
void lv_obj_finish_transitions(lv_obj_t * obj, uint8_t part)
{
    ...
    for (tr = (lv_style_trans_t *)_lv_ll_get_tail(&LV_GC_ROOT(_lv_obj_style_trans_ll)); tr != NULL; tr = (lv_style_trans_t *)_lv_ll_get_prev(&LV_GC_ROOT(_lv_obj_style_trans_ll), tr)) {
...
void lv_event_send_refresh_recursive(lv_obj_t * obj)
{
    if(obj == NULL) {
        ...
        lv_disp_t * d = lv_disp_get_next(NULL);
        while(d) {
            lv_obj_t * scr = (lv_obj_t *)_lv_ll_get_head(&d->scr_ll);
            while(scr) {
                lv_event_send_refresh_recursive(scr);
                scr = (lv_obj_t *)_lv_ll_get_next(&d->scr_ll, scr);
            }
...
lv_disp_t * lv_obj_get_disp(const lv_obj_t * obj)
{
    ...
    for (d = (lv_disp_t *)_lv_ll_get_head(&LV_GC_ROOT(_lv_disp_ll)); d != NULL; d = (lv_disp_t *)_lv_ll_get_next(&LV_GC_ROOT(_lv_disp_ll), d)) {
        ...
        for (s = (lv_obj_t *)_lv_ll_get_head(&d->scr_ll); s != NULL; s = (lv_obj_t *)_lv_ll_get_next(&d->scr_ll, s)) {
            if(s == scr) return d;
        }
    }
...
lv_obj_t * lv_obj_get_child(const lv_obj_t * obj, const lv_obj_t * child)
{
    ...
    if(child == NULL) {
        result = (lv_obj_t *)_lv_ll_get_head(&obj->child_ll);
    }
    else {
        result = (lv_obj_t *)_lv_ll_get_next(&obj->child_ll, child);
    }
...
lv_obj_t * lv_obj_get_child_back(const lv_obj_t * obj, const lv_obj_t * child)
{
    ...
    if(child == NULL) {
        result = (lv_obj_t *)_lv_ll_get_tail(&obj->child_ll);
    }
    else {
        result = (lv_obj_t *)_lv_ll_get_prev(&obj->child_ll, child);
    }
...
#if LV_USE_GROUP
    if(obj->group_p) {
        if(lv_group_get_focused((const lv_group_t *)obj->group_p) == obj) return true;
    }
    return false;
#else
    LV_UNUSED(obj);
...
bool lv_debug_check_obj_valid(const lv_obj_t * obj)
{
    ...
    while(disp) {
        ...
        for (scr = (lv_obj_t *)_lv_ll_get_head(&disp->scr_ll); scr != NULL; scr = (lv_obj_t *)_lv_ll_get_next(&disp->scr_ll, scr)) {
...
static void obj_del_core(lv_obj_t * obj)
{
    ...
    lv_obj_t * i;
    i = (lv_obj_t *)_lv_ll_get_head(&(obj->child_ll));
    while(i != NULL) {
...
static lv_design_res_t lv_obj_design(lv_obj_t * obj, const lv_area_t * clip_area, lv_design_mode_t mode)
{
    if(mode == LV_DESIGN_COVER_CHK) {
        ...
        if(lv_obj_get_style_clip_corner(obj, LV_OBJ_PART_MAIN)) {
            lv_draw_mask_radius_param_t * mp = (lv_draw_mask_radius_param_t *)_lv_mem_buf_get(sizeof(lv_draw_mask_radius_param_t));
...
static lv_res_t lv_obj_signal(lv_obj_t * obj, lv_signal_t sign, void * param)
{
    if(sign == LV_SIGNAL_GET_STYLE) {
        lv_get_style_info_t * info = (lv_get_style_info_t *)param;
        if(info->part == LV_OBJ_PART_MAIN) info->result = &obj->style_list;
        else info->result = NULL;
        return LV_RES_OK;
    }
    else if(sign == LV_SIGNAL_GET_TYPE) return lv_obj_handle_get_type_signal((lv_obj_type_t *)param, LV_OBJX_NAME);
                                       ...
                                       lv_state_t new_state)
{
    ...
    if((prop & 0xF) < LV_STYLE_ID_COLOR) { /*Int*/
        ...
        tr = (lv_style_trans_t *)_lv_ll_ins_head(&LV_GC_ROOT(_lv_obj_style_trans_ll));
        LV_ASSERT_MEM(tr);
...
static void trans_del(lv_obj_t * obj, uint8_t part, lv_style_property_t prop, lv_style_trans_t * tr_limit)
{
    ...
    tr = (lv_style_trans_t *)_lv_ll_get_tail(&LV_GC_ROOT(_lv_obj_style_trans_ll));
    while(tr != NULL) {
        if(tr == tr_limit) break;

        /*'tr' might be deleted, so get the next object while 'tr' is valid*/
        tr_prev = (lv_style_trans_t *)_lv_ll_get_prev(&LV_GC_ROOT(_lv_obj_style_trans_ll), tr);
...
static void trans_anim_start_cb(lv_anim_t * a)
{
    lv_style_trans_t * tr = (lv_style_trans_t *)a->var;
...
static void trans_anim_ready_cb(lv_anim_t * a)
{
    ...
    for (tr_i = (lv_style_trans_t *)_lv_ll_get_head(&LV_GC_ROOT(_lv_obj_style_trans_ll)); tr_i != NULL; tr_i = (lv_style_trans_t *)_lv_ll_get_next(&LV_GC_ROOT(_lv_obj_style_trans_ll), tr_i)) {
...
static bool obj_valid_child(const lv_obj_t * parent, const lv_obj_t * obj_to_find)
{
    ...
    for (child = (lv_obj_t *)_lv_ll_get_head(&parent->child_ll); child != NULL; child = (lv_obj_t *)_lv_ll_get_next(&parent->child_ll, child)) {
