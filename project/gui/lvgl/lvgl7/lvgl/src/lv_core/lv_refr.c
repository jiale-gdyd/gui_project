...
void _lv_disp_refr_task(lv_task_t * task)
{
    ...
    disp_refr = (lv_disp_t *)task->user_data;
    ...
    if(disp_refr->inv_p != 0) {
        ...
        if(lv_disp_is_true_double_buf(disp_refr)) {
            if(disp_refr->driver.set_px_cb) {
                ...
                LV_UNUSED(copy_buf);
#else
                copy_buf = (lv_color_t *)_lv_mem_buf_get(disp_refr->driver.hor_res * sizeof(lv_color_t));
#endif

                uint8_t * buf_act = (uint8_t *)vdb->buf_act;
                uint8_t * buf_ina = (uint8_t *)vdb->buf_act == (uint8_t *)vdb->buf1 ? (uint8_t *)vdb->buf2 : (uint8_t *)vdb->buf1;
...
static void lv_refr_area_part(const lv_area_t * area_p)
{
    ...
    if(top_act_scr == NULL && top_prev_scr == NULL) {
        if(disp_refr->bg_img) {
            ...
            lv_res_t res;
            res = lv_img_decoder_get_info((const char *)disp_refr->bg_img, &header);
            if(res == LV_RES_OK) {
                lv_area_t a;
...
static void lv_refr_obj_and_children(lv_obj_t * top_p, const lv_area_t * mask_p)
{
    ...
    while(par != NULL) {
        ...
        lv_obj_t * i = (lv_obj_t *)_lv_ll_get_prev(&(par->child_ll), border_p);

        while(i != NULL) {
            /*Refresh the objects*/
            lv_refr_obj(i, mask_p);
            i = (lv_obj_t *)_lv_ll_get_prev(&(par->child_ll), i);
        }
...
static void lv_refr_obj(lv_obj_t * obj, const lv_area_t * mask_ori_p)
{
    ...
    if(union_ok != false) {
        ...
        if(union_ok != false) {
            ...
            for (child_p = (lv_obj_t *)_lv_ll_get_tail(&obj->child_ll); child_p != NULL; child_p = (lv_obj_t *)_lv_ll_get_prev(&obj->child_ll, child_p)) {
    ...
    if(drv->rotated == LV_DISP_ROT_180) {
        ...
        while(row < area_h) {
            ...
            if((row == 0) && (area_h >= area_w)) {
                ...
                if(rot_buf == NULL)
                    rot_buf = (lv_color_t *)_lv_mem_buf_get(LV_DISP_ROT_MAX_BUF);
...
{
    lv_disp_buf_t * vdb = lv_disp_get_buf(disp_refr);
    lv_color_t * color_p = (lv_color_t *)vdb->buf_act;
    ...
    if(disp->driver.flush_cb) {
        ...
        if(disp->driver.rotated != LV_DISP_ROT_NONE && disp->driver.sw_rotate) {
            lv_refr_vdb_rotate(&vdb->area, (lv_color_t *)vdb->buf_act);
        }
        else {
