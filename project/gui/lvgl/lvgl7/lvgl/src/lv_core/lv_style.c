...
void lv_style_copy(lv_style_t * style_dest, const lv_style_t * style_src)
{
    ...
    style_dest->map = (uint8_t *)lv_mem_alloc(size);
    if(style_dest->map)
...
void lv_style_list_copy(lv_style_list_t * list_dest, const lv_style_list_t * list_src)
{
    ...
    if(list_src->has_local == 0) {
        if(list_src->has_trans) {
            list_dest->style_list = (lv_style_t **)lv_mem_alloc((list_src->style_cnt - 1) * sizeof(lv_style_t *));
            ...
            list_dest->style_list = (lv_style_t **)lv_mem_alloc(list_src->style_cnt * sizeof(lv_style_t *));
            if(list_dest->style_list) {
        ...
        if(list_src->has_trans) {
            list_dest->style_list = (lv_style_t **)lv_mem_alloc((list_src->style_cnt - 2) * sizeof(lv_style_t *));
            if(list_dest->style_list) {
...
void _lv_style_list_add_style(lv_style_list_t * list, lv_style_t * style)
{
    ...
    if(list->style_cnt == 0) new_styles = (lv_style_t **)lv_mem_alloc(sizeof(lv_style_t *));
    else new_styles = (lv_style_t **)lv_mem_realloc(list->style_list, sizeof(lv_style_t *) * (list->style_cnt + 1));
...
void _lv_style_list_remove_style(lv_style_list_t * list, lv_style_t * style)
{
    ...
    lv_style_t ** new_styles = (lv_style_t **)lv_mem_alloc(sizeof(lv_style_t *) * (list->style_cnt - 1));
    LV_ASSERT_MEM(new_styles);
    if(new_styles == NULL) {
...
lv_style_t * _lv_style_list_add_trans_style(lv_style_list_t * list)
{
    ...
    lv_style_t * trans_style = (lv_style_t *)lv_mem_alloc(sizeof(lv_style_t));
...
static lv_style_t * get_alloc_local_style(lv_style_list_t * list)
{
    ...
    lv_style_t * local_style = (lv_style_t *)lv_mem_alloc(sizeof(lv_style_t));
...
static inline bool style_resize(lv_style_t * style, size_t sz)
{
    uint8_t * new_map = (uint8_t *)lv_mem_realloc(style->map, sz);
