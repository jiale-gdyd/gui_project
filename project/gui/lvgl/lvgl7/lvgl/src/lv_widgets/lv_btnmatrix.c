...
lv_obj_t * lv_btnmatrix_create(lv_obj_t * par, const lv_obj_t * copy)
{
    ...
    lv_btnmatrix_ext_t * ext = (lv_btnmatrix_ext_t *)lv_obj_allocate_ext_attr(btnm, sizeof(lv_btnmatrix_ext_t));
    LV_ASSERT_MEM(ext);
    if(ext == NULL) {
        lv_obj_del(btnm);
        return NULL;
    ...
    /*Copy an existing object*/
        ...
        lv_btnmatrix_ext_t * copy_ext = (lv_btnmatrix_ext_t *)lv_obj_get_ext_attr(copy);
...
static lv_design_res_t lv_btnmatrix_design(lv_obj_t * btnm, const lv_area_t * clip_area, lv_design_mode_t mode)
{
    ...
    /*Draw the object*/
        ...
        char * txt_ap = (char *)_lv_mem_buf_get(txt_ap_size);
#endif
...
static lv_res_t lv_btnmatrix_signal(lv_obj_t * btnm, lv_signal_t sign, void * param)
{
    ...
    if(sign == LV_SIGNAL_CLEANUP) {
        ...
        if(lv_obj_get_width(btnm) != lv_area_get_width((const lv_area_t *)param) || lv_obj_get_height(btnm) != lv_area_get_height((const lv_area_t *)param)) {
            lv_btnmatrix_set_map(btnm, ext->map_p);
        }
    }
        ...
        if(indev_type == LV_INDEV_TYPE_POINTER || indev_type == LV_INDEV_TYPE_BUTTON) {
            ...
            lv_indev_get_point((const lv_indev_t *)param, &p);
                ...
                if(button_is_inactive(ext->ctrl_bits[btn_pr]) == false &&
                   button_is_hidden(ext->ctrl_bits[btn_pr]) == false) {
                    ...
                    invalidate_button_area(btnm, ext->btn_id_pr); /*Invalidate the new area*/
                                                       ...
                                                       lv_group_get_editing((const lv_group_t *)lv_obj_get_group(btnm)))) {
        ...
        if(btn_pr != ext->btn_id_pr) {
            ...
            lv_indev_reset_long_press((lv_indev_t *)param); /*Start the log press time again on the new button*/
            if(btn_pr != LV_BTNMATRIX_BTN_NONE &&
        ...
        if(indev_type == LV_INDEV_TYPE_ENCODER) {
            ...
            if(lv_group_get_editing((const lv_group_t *)lv_obj_get_group(btnm))) {
...
static void allocate_btn_areas_and_controls(const lv_obj_t * btnm, const char ** map)
{
    ...
    ext->button_areas = (lv_area_t *)lv_mem_alloc(sizeof(lv_area_t) * btn_cnt);
    LV_ASSERT_MEM(ext->button_areas);
    ext->ctrl_bits = (lv_btnmatrix_ctrl_t *)lv_mem_alloc(sizeof(lv_btnmatrix_ctrl_t) * btn_cnt);
