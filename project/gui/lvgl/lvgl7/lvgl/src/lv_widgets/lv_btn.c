...
lv_obj_t * lv_btn_create(lv_obj_t * par, const lv_obj_t * copy)
{
    ...
    lv_btn_ext_t * ext = (lv_btn_ext_t *)lv_obj_allocate_ext_attr(btn, sizeof(lv_btn_ext_t));
    LV_ASSERT_MEM(ext);
    if(ext == NULL) {
        lv_obj_del(btn);
        return NULL;
    ...
    /*Copy 'copy'*/
        ...
        lv_btn_ext_t * copy_ext = (lv_btn_ext_t *)lv_obj_get_ext_attr(copy);
...
void lv_btn_set_checkable(lv_obj_t * btn, bool tgl)
{
    ...
    lv_btn_ext_t * ext = (lv_btn_ext_t *)lv_obj_get_ext_attr(btn);
...
static lv_res_t lv_btn_signal(lv_obj_t * btn, lv_signal_t sign, void * param)
{
    ...
    if(sign == LV_SIGNAL_RELEASED) {
        ...
        if(lv_indev_is_dragging((const lv_indev_t *)param) == false && tgl) {
            uint32_t toggled = 0;
            if(lv_obj_get_state(btn, LV_BTN_PART_MAIN) & LV_STATE_CHECKED) {
