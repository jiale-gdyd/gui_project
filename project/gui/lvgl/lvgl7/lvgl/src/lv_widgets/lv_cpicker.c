...
lv_obj_t * lv_cpicker_create(lv_obj_t * par, const lv_obj_t * copy)
{
    ...
    lv_cpicker_ext_t * ext = (lv_cpicker_ext_t *)lv_obj_allocate_ext_attr(cpicker, sizeof(lv_cpicker_ext_t));
    LV_ASSERT_MEM(ext);
    if(ext == NULL) {
        lv_obj_del(cpicker);
        return NULL;
    ...
    /*Copy 'copy'*/
        ...
        lv_cpicker_ext_t * copy_ext = (lv_cpicker_ext_t *)lv_obj_get_ext_attr(copy);
...
static lv_res_t lv_cpicker_signal(lv_obj_t * cpicker, lv_signal_t sign, void * param)
{
    ...
    if(sign == LV_SIGNAL_CLEANUP) {
        ...
        if(lv_obj_get_width(cpicker) != lv_area_get_width((const lv_area_t *)param) ||
           lv_obj_get_height(cpicker) != lv_area_get_height((const lv_area_t *)param)) {
            lv_obj_refresh_ext_draw_pad(cpicker);
            refr_knob_pos(cpicker);
            lv_obj_invalidate(cpicker);
        }
