...
lv_obj_t * lv_dropdown_create(lv_obj_t * par, const lv_obj_t * copy)
{
    ...
    lv_dropdown_ext_t * ext = (lv_dropdown_ext_t *)lv_obj_allocate_ext_attr(ddlist, sizeof(lv_dropdown_ext_t));
    LV_ASSERT_MEM(ext);
    if(ext == NULL) {
        lv_obj_del(ddlist);
        return NULL;
    ...
    /*Copy an existing drop down list*/
        ...
        lv_dropdown_ext_t * copy_ext = (lv_dropdown_ext_t *)lv_obj_get_ext_attr(copy);
...
void lv_dropdown_set_text(lv_obj_t * ddlist, const char * txt)
{
    ...
    lv_dropdown_ext_t * ext = (lv_dropdown_ext_t *)lv_obj_get_ext_attr(ddlist);
...
void lv_dropdown_set_options(lv_obj_t * ddlist, const char * options)
{
    ...
    ext->options = (char *)lv_mem_alloc(len);
...
void lv_dropdown_add_option(lv_obj_t * ddlist, const char * option, uint32_t pos)
{
    ...
    ext->options        = (char *)lv_mem_realloc(ext->options, new_len + 1);
    LV_ASSERT_MEM(ext->options);
    if(ext->options == NULL) return;
    ...
    char * ins_buf = (char *)_lv_mem_buf_get(ins_len + 2); /* + 2 for terminating NULL and possible \n */
...
void lv_dropdown_open(lv_obj_t * ddlist)
{
    ...
    lv_dropdown_page_ext_t * page_ext = (lv_dropdown_page_ext_t *)lv_obj_allocate_ext_attr(ext->page, sizeof(lv_dropdown_page_ext_t));
...
static lv_design_res_t lv_dropdown_design(lv_obj_t * ddlist, const lv_area_t * clip_area, lv_design_mode_t mode)
{
    ...
    /*Draw the object*/
        ...
        if(ext->show_selected) {
            char * buf = (char *)_lv_mem_buf_get(128);
...
static lv_design_res_t lv_dropdown_page_design(lv_obj_t * page, const lv_area_t * clip_area, lv_design_mode_t mode)
{
    ...
    /*Draw the object*/
        ...
        lv_dropdown_page_ext_t * page_ext = (lv_dropdown_page_ext_t *)lv_obj_get_ext_attr(page);
    ...
    if(sign == LV_SIGNAL_GET_STYLE) {
...
#if LV_USE_GROUP
        lv_group_t * g             = (lv_group_t *)lv_obj_get_group(ddlist);
        ...
        if(lv_indev_is_dragging(indev) == false) {
            if(ext->page) {
                ...
                if(indev_type == LV_INDEV_TYPE_ENCODER) {
                    lv_group_set_editing((lv_group_t *)lv_obj_get_group(ddlist), false);
                }
#endif
            }
            else {
