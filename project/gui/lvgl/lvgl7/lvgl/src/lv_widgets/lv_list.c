...
lv_obj_t * lv_list_create(lv_obj_t * par, const lv_obj_t * copy)
{
    ...
    lv_list_ext_t * ext = (lv_list_ext_t *)lv_obj_allocate_ext_attr(list, sizeof(lv_list_ext_t));
    LV_ASSERT_MEM(ext);
    if(ext == NULL) {
        lv_obj_del(list);
        return NULL;
...
lv_obj_t * lv_list_add_btn(lv_obj_t * list, const void * img_src, const char * txt)
{
     ...
     * focused, select it */
    {
        lv_group_t * g = (lv_group_t *)lv_obj_get_group(list);
...
bool lv_list_remove(const lv_obj_t * list, uint16_t index)
{
    ...
    lv_list_ext_t * ext = (lv_list_ext_t *)lv_obj_get_ext_attr(list);
    uint16_t count = 0;
    ...
    if(sign == LV_SIGNAL_RELEASED || sign == LV_SIGNAL_PRESSED || sign == LV_SIGNAL_PRESSING ||
       sign == LV_SIGNAL_LONG_PRESS || sign == LV_SIGNAL_LONG_PRESS_REP) {
        ...
        if(indev_type == LV_INDEV_TYPE_KEYPAD ||
           (indev_type == LV_INDEV_TYPE_ENCODER && lv_group_get_editing((const lv_group_t *)lv_obj_get_group(list)))) {
            lv_list_ext_t * ext = (lv_list_ext_t *)lv_obj_get_ext_attr(list);
...
static lv_res_t lv_list_btn_signal(lv_obj_t * btn, lv_signal_t sign, void * param)
{
    ...
    if((sign == LV_SIGNAL_COORD_CHG && (lv_obj_get_width(btn) != lv_area_get_width((const lv_area_t *)param) ||
                                        lv_obj_get_height(btn) != lv_area_get_height((const lv_area_t *)param))) ||
