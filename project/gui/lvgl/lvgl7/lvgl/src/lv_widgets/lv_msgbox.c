...
lv_obj_t * lv_msgbox_create(lv_obj_t * par, const lv_obj_t * copy)
{
    ...
    lv_msgbox_ext_t * ext = (lv_msgbox_ext_t *)lv_obj_allocate_ext_attr(mbox, sizeof(lv_msgbox_ext_t));
    LV_ASSERT_MEM(ext);
    if(ext == NULL) {
        lv_obj_del(mbox);
        return NULL;
    ...
    /*Copy an existing message box*/
        ...
        lv_msgbox_ext_t * copy_ext = (lv_msgbox_ext_t *)lv_obj_get_ext_attr(copy);
...
void lv_msgbox_set_text_fmt(lv_obj_t * mbox, const char * fmt, ...)
{
    ...
    lv_msgbox_ext_t * msgbox_ext = (lv_msgbox_ext_t *)lv_obj_get_ext_attr(mbox);
    lv_label_ext_t * label_ext = (lv_label_ext_t *)lv_obj_get_ext_attr(msgbox_ext->text);
...
static lv_res_t lv_msgbox_signal(lv_obj_t * mbox, lv_signal_t sign, void * param)
{
    ...
    if(sign == LV_SIGNAL_COORD_CHG) {
        if(lv_obj_get_width(mbox) != lv_area_get_width((const lv_area_t *)param)) {
            mbox_realign(mbox);
        }
    }
        ...
        if(sign == LV_SIGNAL_FOCUS) {
            ...
            if(indev_type == LV_INDEV_TYPE_ENCODER) {
                ...
                if(lv_group_get_editing((const lv_group_t *)lv_obj_get_group(mbox))) lv_btnmatrix_set_focused_btn(ext->btnm, 0);
