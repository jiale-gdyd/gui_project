...
lv_obj_t * lv_img_create(lv_obj_t * par, const lv_obj_t * copy)
{
    ...
    lv_img_ext_t * ext = (lv_img_ext_t *)lv_obj_allocate_ext_attr(img, sizeof(lv_img_ext_t));
    LV_ASSERT_MEM(ext);
    if(ext == NULL) {
        lv_obj_del(img);
        return NULL;
    ...
    if(copy == NULL) {
        ...
        lv_img_ext_t * copy_ext = (lv_img_ext_t *)lv_obj_get_ext_attr(copy);
...
void lv_img_set_src(lv_obj_t * img, const void * src_img)
{
    ...
    lv_img_ext_t * ext    = (lv_img_ext_t *)lv_obj_get_ext_attr(img);
    ...
    lv_img_decoder_get_info((const char *)src_img, &header);
    ...
    if(src_type == LV_IMG_SRC_VARIABLE) {
        ...
        if(ext->src != src_img) {
            ...
            char * new_str = (char *)lv_mem_alloc(strlen((const char *)src_img) + 1);
            LV_ASSERT_MEM(new_str);
            if(new_str == NULL) return;
            strcpy(new_str, (const char *)src_img);
            ext->src = new_str;
    ...
    if(src_type == LV_IMG_SRC_SYMBOL) {
        ...
        lv_point_t size;
        _lv_txt_get_size(&size, (const char *)src_img, font, letter_space, line_space,
...
static lv_design_res_t lv_img_design(lv_obj_t * img, const lv_area_t * clip_area, lv_design_mode_t mode)
{
    ...
    if(mode == LV_DESIGN_COVER_CHK) {
        ...
        if(ext->src_type == LV_IMG_SRC_FILE || ext->src_type == LV_IMG_SRC_VARIABLE) {
            ...
            lv_draw_label(&img->coords, clip_area, &label_dsc, (const char *)ext->src, NULL);
        }
        else {
        ...
        if(lv_obj_get_style_clip_corner(img, LV_OBJ_PART_MAIN)) {
            lv_draw_mask_radius_param_t * param = (lv_draw_mask_radius_param_t *)lv_draw_mask_remove_custom(img + 8);
...
static lv_res_t lv_img_signal(lv_obj_t * img, lv_signal_t sign, void * param)
{
    ...
    if(sign == LV_SIGNAL_CLEANUP) {
        ...
        lv_hit_test_info_t * info = (lv_hit_test_info_t *)param;
