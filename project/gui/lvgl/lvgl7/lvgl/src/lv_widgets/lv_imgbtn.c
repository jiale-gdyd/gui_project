...
lv_obj_t * lv_imgbtn_create(lv_obj_t * par, const lv_obj_t * copy)
{
    ...
    lv_imgbtn_ext_t * ext = (lv_imgbtn_ext_t *)lv_obj_allocate_ext_attr(imgbtn, sizeof(lv_imgbtn_ext_t));
    LV_ASSERT_MEM(ext);
    if(ext == NULL) {
        lv_obj_del(imgbtn);
        return NULL;
    ...
    /*Copy an existing image button*/
        ...
        lv_imgbtn_ext_t * copy_ext = (lv_imgbtn_ext_t *)lv_obj_get_ext_attr(copy);
...
void lv_imgbtn_set_src(lv_obj_t * imgbtn, lv_btn_state_t state, const void * src)
{
    ...
    lv_imgbtn_ext_t * ext = (lv_imgbtn_ext_t *)lv_obj_get_ext_attr(imgbtn);
...
static lv_design_res_t lv_imgbtn_design(lv_obj_t * imgbtn, const lv_area_t * clip_area, lv_design_mode_t mode)
{
    ...
    /*Draw the object*/
        ...
        if(!ext->tiled) {
            ...
            if(lv_img_src_get_type(src) == LV_IMG_SRC_SYMBOL) {
                ...
                lv_draw_label(&imgbtn->coords, clip_area, &label_dsc, (const char *)src, NULL);
            }
            else {
            ...
            if(src) {
                lv_img_decoder_get_info((const char *)src, &header);
                left_w = header.w;
    ...
    /*Post draw when the children are drawn*/
        ...
        if(lv_obj_get_style_clip_corner(imgbtn, LV_OBJ_PART_MAIN)) {
            lv_draw_mask_radius_param_t * param = (lv_draw_mask_radius_param_t *)lv_draw_mask_remove_custom(imgbtn + 8);
...
static void refr_img(lv_obj_t * imgbtn)
{
    ...
    if(lv_img_src_get_type(src) == LV_IMG_SRC_SYMBOL) {
        ...
        header.w = _lv_txt_get_width((const char *)src, (uint16_t)strlen((const char *)src), font, 0, LV_TXT_FLAG_NONE);
        header.always_zero = 0;
        header.cf = LV_IMG_CF_ALPHA_1BIT;
    }
    else {
        info_res = lv_img_decoder_get_info((const char *)src, &header);
    }
