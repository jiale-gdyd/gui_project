...
lv_obj_t * lv_gauge_create(lv_obj_t * par, const lv_obj_t * copy)
{
    ...
    lv_gauge_ext_t * ext = (lv_gauge_ext_t *)lv_obj_allocate_ext_attr(gauge, sizeof(lv_gauge_ext_t));
    LV_ASSERT_MEM(ext);
    if(ext == NULL) {
        lv_obj_del(gauge);
        return NULL;
    ...
    /*Copy an existing gauge*/
        ...
        lv_gauge_ext_t * copy_ext = (lv_gauge_ext_t *)lv_obj_get_ext_attr(copy);
...
void lv_gauge_set_needle_count(lv_obj_t * gauge, uint8_t needle_cnt, const lv_color_t colors[])
{
    ...
    lv_gauge_ext_t * ext = (lv_gauge_ext_t *)lv_obj_get_ext_attr(gauge);
    ...
    if(ext->needle_count != needle_cnt) {
        ...
        ext->values = (int32_t *)lv_mem_realloc(ext->values, needle_cnt * sizeof(ext->values[0]));
...
void lv_gauge_set_value(lv_obj_t * gauge, uint8_t needle_id, int32_t value)
{
    ...
    if(ext->needle_img) {
        ...
        lv_img_decoder_get_info((const char *)ext->needle_img, &info);
        needle_w = info.h;
    }
    else {
