...
lv_obj_t * lv_keyboard_create(lv_obj_t * par, const lv_obj_t * copy)
{
    ...
    lv_keyboard_ext_t * ext = (lv_keyboard_ext_t *)lv_obj_allocate_ext_attr(kb, sizeof(lv_keyboard_ext_t));
    LV_ASSERT_MEM(ext);
    if(ext == NULL) {
        lv_obj_del(kb);
        return NULL;
    ...
    /*Copy an existing keyboard*/
        ...
        lv_keyboard_ext_t * copy_ext = (lv_keyboard_ext_t *)lv_obj_get_ext_attr(copy);
        ext->ta                = copy_ext->ta;
