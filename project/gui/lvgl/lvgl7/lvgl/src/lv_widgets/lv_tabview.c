...
lv_obj_t * lv_tabview_create(lv_obj_t * par, const lv_obj_t * copy)
{
    ...
    lv_tabview_ext_t * ext = (lv_tabview_ext_t *)lv_obj_allocate_ext_attr(tabview, sizeof(lv_tabview_ext_t));
    LV_ASSERT_MEM(ext);
    if(ext == NULL) {
        lv_obj_del(tabview);
        return NULL;
    ...
    if(copy == NULL) {
        ext->tab_name_ptr = (const char **)lv_mem_alloc(sizeof(char *));
    ...
    /*Copy an existing tab view*/
        ...
        lv_tabview_ext_t * copy_ext = (lv_tabview_ext_t *)lv_obj_get_ext_attr(copy);
        ext->point_last.x           = 0;
        ext->point_last.y           = 0;
...
lv_obj_t * lv_tabview_add_tab(lv_obj_t * tabview, const char * name)
{
    ...
    char * name_dm;
    name_dm = (char *)lv_mem_alloc(strlen(name) + 1); /*+1 for the closing '\0' */
    ...
    switch(ext->btns_pos) {
        ...
        case LV_TABVIEW_TAB_POS_BOTTOM:
            ext->tab_name_ptr = (const char **)lv_mem_realloc(ext->tab_name_ptr, sizeof(char *) * (ext->tab_cnt + 1));
        ...
        case LV_TABVIEW_TAB_POS_RIGHT:
            ext->tab_name_ptr = (const char **)lv_mem_realloc(ext->tab_name_ptr, sizeof(char *) * (ext->tab_cnt * 2));
    ...
    lv_btnmatrix_ext_t * btnm_ext = (lv_btnmatrix_ext_t *)lv_obj_get_ext_attr(ext->btns);
    btnm_ext->map_p          = NULL;
...
void lv_tabview_set_tab_name(lv_obj_t * tabview, uint16_t id, char * name)
{
    ...
    if(ext->tab_cnt > id) {
        ...
        char * str = (char *)lv_mem_realloc((void *)ext->tab_name_ptr[id], strlen(name) + 1);
        LV_ASSERT_MEM(str);
...
static lv_res_t lv_tabview_signal(lv_obj_t * tabview, lv_signal_t sign, void * param)
{
    ...
    if(sign == LV_SIGNAL_CLEANUP) {
        ...
        if(ext->content != NULL && (lv_obj_get_width(tabview) != lv_area_get_width((const lv_area_t *)param) ||
                                    lv_obj_get_height(tabview) != lv_area_get_height((const lv_area_t *)param))) {
            tabview_realign(tabview);
        }
    }
        ...
        if(indev_type == LV_INDEV_TYPE_KEYPAD ||
           (indev_type == LV_INDEV_TYPE_ENCODER && lv_group_get_editing((const lv_group_t *)lv_obj_get_group(tabview)))) {
...
static void tab_btnm_event_cb(lv_obj_t * tab_btnm, lv_event_t event)
{
    ...
    if(lv_indev_get_type(lv_indev_get_act()) == LV_INDEV_TYPE_ENCODER) {
        lv_group_set_editing((lv_group_t *)lv_obj_get_group(tabview), false);
    }
#endif
