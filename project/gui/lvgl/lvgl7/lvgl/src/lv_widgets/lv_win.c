...
lv_obj_t * lv_win_create(lv_obj_t * par, const lv_obj_t * copy)
{
    ...
    lv_win_ext_t * ext = (lv_win_ext_t *)lv_obj_allocate_ext_attr(new_win, sizeof(lv_win_ext_t));
    LV_ASSERT_MEM(ext);
    if(ext == NULL) {
        lv_obj_del(new_win);
        return NULL;
    ...
    ext->page          = NULL;
    ext->header        = NULL;
    ext->title_txt    = (char *)lv_mem_alloc(strlen(DEF_TITLE) + 1);
    ...
    /*Copy an existing object*/
        ...
        lv_win_ext_t * copy_ext = (lv_win_ext_t *)lv_obj_get_ext_attr(copy);
        /*Create the objects*/
        ext->header   = lv_obj_create(new_win, copy_ext->header);
        ext->title_txt    = (char *)lv_mem_alloc(strlen(copy_ext->title_txt) + 1);
...
void lv_win_set_title(lv_obj_t * win, const char * title)
{
    ...
    ext->title_txt    = (char *)lv_mem_realloc(ext->title_txt, len + 1);
...
static lv_res_t lv_win_signal(lv_obj_t * win, lv_signal_t sign, void * param)
{
    ...
    if(sign == LV_SIGNAL_CHILD_CHG) { /*Move children to the page*/
        ...
        if(lv_area_get_width((const lv_area_t *)param) != lv_obj_get_width(win) || lv_area_get_height((const lv_area_t *)param) != lv_obj_get_height(win)) {
            lv_win_realign(win);
        }
    }
...
static lv_obj_t * lv_win_btn_create(lv_obj_t * par, const void * img_src)
{
    ...
    lv_win_btn_ext_t * ext = (lv_win_btn_ext_t *)lv_obj_allocate_ext_attr(win_btn, sizeof(lv_win_btn_ext_t));
    LV_ASSERT_MEM(ext);
    if(ext == NULL) {
        lv_obj_del(win_btn);
        return NULL;
...
{
    lv_win_btn_ext_t * ext = (lv_win_btn_ext_t *)lv_obj_get_ext_attr(win_btn);
