...
lv_obj_t * lv_page_create(lv_obj_t * par, const lv_obj_t * copy)
{
    ...
    lv_page_ext_t * ext = (lv_page_ext_t *)lv_obj_allocate_ext_attr((lv_obj_t *)page, sizeof(lv_page_ext_t));
    LV_ASSERT_MEM(ext);
    if(ext == NULL) {
        lv_obj_del(page);
        return NULL;
    ...
    if(copy == NULL) {
        ...
        lv_page_ext_t * copy_ext = (lv_page_ext_t *)lv_obj_get_ext_attr(copy);
...
void lv_page_clean(lv_obj_t * page)
{
    ...
    lv_obj_t * scrl = (lv_obj_t *)lv_page_get_scrollable(page);
    lv_obj_clean(scrl);
}
...
void lv_page_set_scrollbar_mode(lv_obj_t * page, lv_scrollbar_mode_t sb_mode)
{
    ...
    lv_page_ext_t * ext = (lv_page_ext_t *)lv_obj_get_ext_attr(page);
...
static lv_res_t lv_page_signal(lv_obj_t * page, lv_signal_t sign, void * param)
{
    ...
    if(sign == LV_SIGNAL_GET_STYLE) {
        ...
        lv_get_state_info_t * info = (lv_get_state_info_t *)param;
    ...
    if(sign == LV_SIGNAL_GET_TYPE) return lv_obj_handle_get_type_signal((lv_obj_type_t *)param, LV_OBJX_NAME);
    ...
    if(sign == LV_SIGNAL_CLEANUP) {
        ...
        if(ext->scroll_prop) {
            ...
            lv_page_ext_t * parent_ext = (lv_page_ext_t *)lv_obj_get_ext_attr(parent_page);
    ...
    /*Automatically move children to the scrollable object*/
        ...
        if(ext->scrl != NULL && (lv_obj_get_width(page) != lv_area_get_width((const lv_area_t *)param) ||
                                 lv_obj_get_height(page) != lv_area_get_height((const lv_area_t *)param))) {
...
static lv_res_t lv_page_scrollable_signal(lv_obj_t * scrl, lv_signal_t sign, void * param)
{
    ...
    if(res != LV_RES_OK) return res;
    if(sign == LV_SIGNAL_GET_TYPE) return lv_obj_handle_get_type_signal((lv_obj_type_t *)param, "");

    lv_obj_t * page               = lv_obj_get_parent(scrl);
    lv_page_ext_t * page_ext      = (lv_page_ext_t *)lv_obj_get_ext_attr(page);
    ...
    if(sign == LV_SIGNAL_COORD_CHG) {
        ...
        if(page_ext->scroll_prop  && indev) {
            ...
            lv_page_ext_t * parent_ext = (lv_page_ext_t *)lv_obj_get_ext_attr(lv_obj_get_parent(page_parent));
        ...
        if((ext->scrlbar.hor_draw || ext->scrlbar.ver_draw) ||
           (lv_obj_get_width(scrl) != lv_area_get_width((const lv_area_t *)param) || lv_obj_get_height(scrl) != lv_area_get_height((const lv_area_t *)param))) {
            scrlbar_refresh(page);
        }
    }
    else if(sign == LV_SIGNAL_STYLE_CHG) {
        ...
        if(page_ext->scroll_prop_obj) {
            ...
            lv_page_ext_t * scroller_page_ext      = (lv_page_ext_t *)lv_obj_get_ext_attr(scroller_page);
            ...
            lv_page_ext_t * scroller_ext = (lv_page_ext_t *)lv_obj_get_ext_attr(scroller_page);
            while(scroller_ext->scroll_prop_obj) {
                ...
                scroller_ext = (lv_page_ext_t *)lv_obj_get_ext_attr(scroller_page);
            }
        }
...
static void edge_flash_anim(void * page, lv_anim_value_t v)
{
    lv_page_ext_t * ext   = (lv_page_ext_t *)lv_obj_get_ext_attr((const lv_obj_t *)page);
    ext->edge_flash.state = v;

    lv_area_t flash_area;
    get_edge_flash_area((lv_obj_t *)page, &flash_area, LV_PAGE_END_FLASH_SIZE);
    lv_obj_invalidate_area((const lv_obj_t *)page, &flash_area);
}

static void edge_flash_anim_end(lv_anim_t * a)
{
    lv_area_t flash_area;
    get_edge_flash_area((lv_obj_t *)a->var, &flash_area, LV_PAGE_END_FLASH_SIZE);
    lv_obj_invalidate_area((const lv_obj_t *)a->var, &flash_area);

    lv_page_ext_t * ext       = (lv_page_ext_t *)lv_obj_get_ext_attr((const lv_obj_t *)a->var);
    ext->edge_flash.top_ip    = 0;
    ext->edge_flash.bottom_ip = 0;
    ext->edge_flash.left_ip   = 0;
    ext->edge_flash.right_ip  = 0;
