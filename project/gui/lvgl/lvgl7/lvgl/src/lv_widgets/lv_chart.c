...
lv_obj_t * lv_chart_create(lv_obj_t * par, const lv_obj_t * copy)
{
    ...
    lv_chart_ext_t * ext = (lv_chart_ext_t *)lv_obj_allocate_ext_attr(chart, sizeof(lv_chart_ext_t));
    LV_ASSERT_MEM(ext);
    if(ext == NULL) {
        lv_obj_del(chart);
        return NULL;
    ...
    if(copy == NULL) {
        ...
        lv_chart_ext_t * ext_copy = (lv_chart_ext_t *)lv_obj_get_ext_attr(copy);
...
lv_chart_series_t * lv_chart_add_series(lv_obj_t * chart, lv_color_t color)
{
    ...
    lv_chart_series_t * ser = (lv_chart_series_t *)_lv_ll_ins_head(&ext->series_ll);
    LV_ASSERT_MEM(ser);
    ...
    ser->color  = color;
    ser->points = (lv_coord_t *)lv_mem_alloc(sizeof(lv_coord_t) * ext->point_cnt);
...
lv_chart_cursor_t  * lv_chart_add_cursor(lv_obj_t * chart, lv_color_t color, lv_cursor_direction_t axes)
{
    ...
    lv_chart_ext_t * ext    = (lv_chart_ext_t *)lv_obj_get_ext_attr(chart);
    lv_chart_cursor_t * cursor = (lv_chart_cursor_t *)_lv_ll_ins_head(&ext->cursors_ll);
...
void lv_chart_set_point_count(lv_obj_t * chart, uint16_t point_cnt)
{
    ...
    for (ser = (lv_chart_series_t *)_lv_ll_get_tail(&ext->series_ll); ser != NULL; ser = (lv_chart_series_t *)_lv_ll_get_prev(&ext->series_ll, ser)) {
        if(!ser->ext_buf_assigned) {
            if(ser->start_point != 0) {
                lv_coord_t * new_points = (lv_coord_t *)lv_mem_alloc(sizeof(lv_coord_t) * point_cnt);
                ...
                ser->points = (lv_coord_t *)lv_mem_realloc(ser->points, sizeof(lv_coord_t) * point_cnt);
                LV_ASSERT_MEM(ser->points);
...
lv_coord_t lv_chart_get_x_from_index(lv_obj_t * chart, lv_chart_series_t * ser, uint16_t id)
{
    ...
    if(ext->type & LV_CHART_TYPE_LINE) {
        ...
        for (itr_ser = (lv_chart_series_t *)_lv_ll_get_tail(&ext->series_ll); itr_ser != NULL; itr_ser = (lv_chart_series_t *)_lv_ll_get_prev(&ext->series_ll, itr_ser)) {
...
static lv_res_t lv_chart_signal(lv_obj_t * chart, lv_signal_t sign, void * param)
{
    ...
    if(sign == LV_SIGNAL_CLEANUP) {
        ...
        while(ext->series_ll.head != NULL) {
            ser = (lv_chart_series_t *)_lv_ll_get_head(&ext->series_ll);
...
static void draw_cursors(lv_obj_t * chart, const lv_area_t * series_area, const lv_area_t * clip_area)
{
    ...
    for (cursor = (lv_chart_cursor_t *)_lv_ll_get_tail(&ext->cursors_ll); cursor != NULL; cursor = (lv_chart_cursor_t *)_lv_ll_get_prev(&ext->cursors_ll, cursor)) {
    ...
    for(i = 0; i < (num_scale_ticks + 1); i++) { /* one extra loop - it may not exist in the list, empty label */
        ...
        if(num_of_labels != 0) {
            ...
            if(is_tick_with_label(i, &(ext->x_axis))) {
                ...
                lv_area_t a = {(lv_coord_t)(p2.x - size.x / 2), (lv_coord_t)(p2.y + label_dist), 
                            (lv_coord_t)(p2.x + size.x / 2),
                               (lv_coord_t)(p2.y + size.y + label_dist)
                              };
                lv_draw_label(&a, mask, &label_dsc, buf, NULL);
            }
        }
