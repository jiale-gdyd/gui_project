...
lv_obj_t * lv_spinbox_create(lv_obj_t * par, const lv_obj_t * copy)
{
    ...
    lv_spinbox_ext_t * ext = (lv_spinbox_ext_t *)lv_obj_allocate_ext_attr(spinbox, sizeof(lv_spinbox_ext_t));
    LV_ASSERT_MEM(ext);
    if(ext == NULL) {
        lv_obj_del(spinbox);
        return NULL;
    ...
    /*Copy an existing spinbox*/
        ...
        lv_spinbox_ext_t * copy_ext = (lv_spinbox_ext_t *)lv_obj_get_ext_attr(copy);
...
#if LV_USE_GROUP
            if(lv_group_get_editing((const lv_group_t *)lv_obj_get_group(spinbox))) {
                if(ext->step > 1) {
...
static lv_style_list_t * lv_spinbox_get_style(lv_obj_t * ta, uint8_t part)
{
    ...
    lv_spinbox_ext_t * ext = (lv_spinbox_ext_t * )lv_obj_get_ext_attr(ta);
