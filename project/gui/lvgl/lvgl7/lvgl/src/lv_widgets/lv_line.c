...
lv_obj_t * lv_line_create(lv_obj_t * par, const lv_obj_t * copy)
{
    ...
    lv_line_ext_t * ext = (lv_line_ext_t *)lv_obj_allocate_ext_attr(line, sizeof(lv_line_ext_t));
    LV_ASSERT_MEM(ext);
    if(ext == NULL) {
        lv_obj_del(line);
        return NULL;
    ...
    /*Copy an existing object*/
        ...
        lv_line_ext_t * copy_ext = (lv_line_ext_t *)lv_obj_get_ext_attr(copy);
...
void lv_line_set_points(lv_obj_t * line, const lv_point_t point_a[], uint16_t point_num)
{
    ...
    lv_line_ext_t * ext = (lv_line_ext_t *)lv_obj_get_ext_attr(line);
