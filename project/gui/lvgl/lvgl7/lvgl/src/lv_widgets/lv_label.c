...
lv_obj_t * lv_label_create(lv_obj_t * par, const lv_obj_t * copy)
{
    ...
    lv_label_ext_t * ext = (lv_label_ext_t *)lv_obj_get_ext_attr(new_label);
    LV_ASSERT_MEM(ext);
    if(ext == NULL) {
    ...
    /*Copy 'copy' if not NULL*/
        ...
        lv_label_ext_t * copy_ext = (lv_label_ext_t *)lv_obj_get_ext_attr(copy);
        ...
        if(copy_ext->long_mode == LV_LABEL_LONG_DOT) {
            ext->text = (char *)lv_mem_realloc(ext->text, _lv_mem_get_size(copy_ext->text));
...
void lv_label_set_text(lv_obj_t * label, const char * text)
{
    ...
    if(ext->text == text && ext->static_txt == 0) {
        ...
        ext->text = (char *)lv_mem_realloc(ext->text, len);
        LV_ASSERT_MEM(ext->text);
        if(ext->text == NULL) return;

        _lv_txt_ap_proc(ext->text, ext->text);
#else
        ext->text = (char *)lv_mem_realloc(ext->text, strlen(ext->text) + 1);
#endif
        ...
        ext->text = (char *)lv_mem_alloc(len);
        LV_ASSERT_MEM(ext->text);
        if(ext->text == NULL) return;
...
#if LV_USE_BIDI
    bidi_txt = (char *)_lv_mem_buf_get(new_line_start - line_start + 1);
...
void lv_label_ins_text(lv_obj_t * label, uint32_t pos, const char * txt)
{
    ...
    ext->text        = (char *)lv_mem_realloc(ext->text, new_len + 1);
    LV_ASSERT_MEM(ext->text);
    if(ext->text == NULL) return;
...
#if LV_USE_BIDI
    char * bidi_buf = (char *)_lv_mem_buf_get(ins_len + 1);
...
void lv_label_cut_text(lv_obj_t * label, uint32_t pos, uint32_t cnt)
{
    ...
    char * label_txt = (char *)lv_label_get_text(label);
...
static lv_res_t lv_label_signal(lv_obj_t * label, lv_signal_t sign, void * param)
{
    ...
    if(sign == LV_SIGNAL_CLEANUP) {
        ...
        if(lv_area_get_width(&label->coords) != lv_area_get_width((const lv_area_t *)param) ||
           lv_area_get_height(&label->coords) != lv_area_get_height((const lv_area_t *)param)) {
            lv_label_revert_dots(label);
            lv_label_refr_text(label);
        }
    }
...
static bool lv_label_set_dot_tmp(lv_obj_t * label, char * data, uint32_t len)
{
    ...
    if(len > sizeof(char *)) {
        ...
        ext->dot.tmp_ptr = (char *)lv_mem_alloc(len + 1);
