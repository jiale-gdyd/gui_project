...
lv_obj_t * lv_canvas_create(lv_obj_t * par, const lv_obj_t * copy)
{
    ...
    lv_canvas_ext_t * ext = (lv_canvas_ext_t *)lv_obj_allocate_ext_attr(new_canvas, sizeof(lv_canvas_ext_t));
    LV_ASSERT_MEM(ext);
    if(ext == NULL) {
...
void lv_canvas_set_buffer(lv_obj_t * canvas, void * buf, lv_coord_t w, lv_coord_t h, lv_img_cf_t cf)
{
    ...
    ext->dsc.header.w  = w;
    ext->dsc.header.h  = h;
    ext->dsc.data      = (const uint8_t *)buf;
                         ...
                         int32_t pivot_x, int32_t pivot_y, bool antialias)
{
    ...
    lv_canvas_ext_t * ext_dst = (lv_canvas_ext_t *)lv_obj_get_ext_attr(canvas);
    lv_color_t color = lv_obj_get_style_image_recolor(canvas, LV_CANVAS_PART_MAIN);
...
void lv_canvas_blur_hor(lv_obj_t * canvas, const lv_area_t * area, uint16_t r)
{
    ...
    uint8_t * line_buf = (uint8_t *)_lv_mem_buf_get(line_w);
...
void lv_canvas_blur_ver(lv_obj_t * canvas, const lv_area_t * area, uint16_t r)
{
    ...
    uint8_t * col_buf = (uint8_t *)_lv_mem_buf_get(col_w);
    lv_img_dsc_t line_img;
                        ...
                        const lv_draw_img_dsc_t * img_draw_dsc)
{
    ...
    lv_res_t res = lv_img_decoder_get_info((const char *)src, &header);
    if(res != LV_RES_OK) {
