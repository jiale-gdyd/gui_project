...
lv_obj_t * lv_arc_create(lv_obj_t * par, const lv_obj_t * copy)
{
    ...
    lv_arc_ext_t * ext = (lv_arc_ext_t *)lv_obj_allocate_ext_attr(arc, sizeof(lv_arc_ext_t));
    LV_ASSERT_MEM(ext);
    if(ext == NULL) {
        lv_obj_del(arc);
        return NULL;
    ...
    /*Copy an existing arc*/
        ...
        lv_arc_ext_t * copy_ext = (lv_arc_ext_t *)lv_obj_get_ext_attr(copy);
...
static lv_res_t lv_arc_signal(lv_obj_t * arc, lv_signal_t sign, void * param)
{
    ...
    if(sign == LV_SIGNAL_PRESSING) {
        ...
        lv_group_t * g             = (lv_group_t *)lv_obj_get_group(arc);
