...
lv_obj_t * lv_table_create(lv_obj_t * par, const lv_obj_t * copy)
{
    ...
    lv_obj_t * table = (lv_obj_t *)lv_obj_create(par, copy);
    LV_ASSERT_MEM(table);
    if(table == NULL) return NULL;

    /*Allocate the table type specific extended data*/
    lv_table_ext_t * ext = (lv_table_ext_t *)lv_obj_allocate_ext_attr(table, sizeof(lv_table_ext_t));
    LV_ASSERT_MEM(ext);
    if(ext == NULL) {
        lv_obj_del(table);
        return NULL;
    ...
    /*Copy an existing table*/
        ...
        lv_table_ext_t * copy_ext = (lv_table_ext_t *)lv_obj_get_ext_attr(copy);
        for(i = 0; i < LV_TABLE_CELL_STYLE_CNT; i++) {
...
void lv_table_set_cell_value(lv_obj_t * table, uint16_t row, uint16_t col, const char * txt)
{
    ...
    lv_table_ext_t * ext = (lv_table_ext_t *)lv_obj_get_ext_attr(table);
    if(col >= ext->col_cnt) {
    ...
    ext->cell_data[cell] = (char *)lv_mem_realloc(ext->cell_data[cell], len_ap + 1);
    LV_ASSERT_MEM(ext->cell_data[cell]);
    if(ext->cell_data[cell] == NULL) return;

    _lv_txt_ap_proc(txt, &ext->cell_data[cell][1]);
#else
    ext->cell_data[cell] = (char *)lv_mem_realloc(ext->cell_data[cell], strlen(txt) + 2); /*+1: trailing '\0; +1: format byte*/
    LV_ASSERT_MEM(ext->cell_data[cell]);
    if(ext->cell_data[cell] == NULL) return;
...
void lv_table_set_cell_value_fmt(lv_obj_t * table, uint16_t row, uint16_t col, const char * fmt, ...)
{
    ...
    char * raw_txt = (char *)_lv_mem_buf_get(len + 1);
...
#else
    ext->cell_data[cell] = (char *)lv_mem_realloc(ext->cell_data[cell], len + 2); /*+1: trailing '\0; +1: format byte*/
    LV_ASSERT_MEM(ext->cell_data[cell]);
    if(ext->cell_data[cell] == NULL) {
        va_end(ap2);
        return;
...
void lv_table_set_row_cnt(lv_obj_t * table, uint16_t row_cnt)
{
    ...
    if(ext->row_cnt > 0) {
        ext->row_h = (lv_coord_t *)lv_mem_realloc(ext->row_h, ext->row_cnt * sizeof(ext->row_h[0]));
    ...
    if(ext->row_cnt > 0 && ext->col_cnt > 0) {
        ...
        ext->cell_data = (char **)lv_mem_realloc(ext->cell_data, ext->row_cnt * ext->col_cnt * sizeof(char *));
...
void lv_table_set_cell_align(lv_obj_t * table, uint16_t row, uint16_t col, lv_label_align_t align)
{
    ...
    if(ext->cell_data[cell] == NULL) {
        ext->cell_data[cell]    = (char *)lv_mem_alloc(2); /*+1: trailing '\0; +1: format byte*/
        LV_ASSERT_MEM(ext->cell_data[cell]);
        if(ext->cell_data[cell] == NULL) return;
