...
lv_obj_t * lv_roller_create(lv_obj_t * par, const lv_obj_t * copy)
{
    ...
    lv_roller_ext_t * ext = (lv_roller_ext_t *)lv_obj_allocate_ext_attr(roller, sizeof(lv_roller_ext_t));
    LV_ASSERT_MEM(ext);
    if(ext == NULL) {
        lv_obj_del(roller);
        return NULL;
    ...
    /*Copy an existing roller*/
        ...
        lv_roller_ext_t * copy_ext = (lv_roller_ext_t *)lv_obj_get_ext_attr(copy);
...
void lv_roller_set_options(lv_obj_t * roller, const char * options, lv_roller_mode_t mode)
{
    ...
    lv_roller_ext_t * ext = (lv_roller_ext_t *)lv_obj_get_ext_attr(roller);
    ...
    if(mode == LV_ROLLER_MODE_NORMAL) {
        ...
        char * opt_extra = (char *)_lv_mem_buf_get(opt_len * LV_ROLLER_INF_PAGES);
        uint8_t i;
...
static lv_res_t lv_roller_signal(lv_obj_t * roller, lv_signal_t sign, void * param)
{
    ...
    if(sign == LV_SIGNAL_STYLE_CHG) {
        ...
        if(lv_obj_get_width(roller) != lv_area_get_width((const lv_area_t *)param) ||
           lv_obj_get_height(roller) != lv_area_get_height((const lv_area_t *)param)) {
#if LV_USE_ANIMATION
...
#if LV_USE_GROUP
        lv_group_t * g             = (lv_group_t *)lv_obj_get_group(roller);
