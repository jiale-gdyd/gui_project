...
lv_obj_t * lv_textarea_create(lv_obj_t * par, const lv_obj_t * copy)
{
    ...
    lv_textarea_ext_t * ext = (lv_textarea_ext_t *)lv_obj_allocate_ext_attr(ta, sizeof(lv_textarea_ext_t));
    LV_ASSERT_MEM(ext);
    if(ext == NULL) {
        lv_obj_del(ta);
        return NULL;
    ...
    /*Copy an existing object*/
        ...
        lv_obj_set_design_cb(ext->page.scrl, lv_textarea_scrollable_design);
        lv_textarea_ext_t * copy_ext = (lv_textarea_ext_t *)lv_obj_get_ext_attr(copy);
        ...
        if(copy_ext->pwd_tmp) {
            ...
            ext->pwd_tmp = (char *)lv_mem_alloc(len);
...
void lv_textarea_add_char(lv_obj_t * ta, uint32_t c)
{
    ...
    if(ext->pwd_mode != 0) {
        ext->pwd_tmp = (char *)lv_mem_realloc(ext->pwd_tmp, strlen(ext->pwd_tmp) + strlen(letter_buf) + 1); /*+2: the new char + \0 */
        LV_ASSERT_MEM(ext->pwd_tmp);
...
void lv_textarea_add_text(lv_obj_t * ta, const char * txt)
{
    ...
    if(ext->pwd_mode != 0) {
        ext->pwd_tmp = (char *)lv_mem_realloc(ext->pwd_tmp, strlen(ext->pwd_tmp) + strlen(txt) + 1);
        LV_ASSERT_MEM(ext->pwd_tmp);
        if(ext->pwd_tmp == NULL) return;
...
void lv_textarea_del_char(lv_obj_t * ta)
{
    ...
    if(ext->pwd_mode != 0) {
        ...
        ext->pwd_tmp = (char *)lv_mem_realloc(ext->pwd_tmp, strlen(ext->pwd_tmp) + 1);
        LV_ASSERT_MEM(ext->pwd_tmp);
        if(ext->pwd_tmp == NULL) return;
    }
...
void lv_textarea_set_text(lv_obj_t * ta, const char * txt)
{
    ...
    if(ext->pwd_mode != 0) {
        ext->pwd_tmp = (char *)lv_mem_realloc(ext->pwd_tmp, strlen(txt) + 1);
        LV_ASSERT_MEM(ext->pwd_tmp);
        if(ext->pwd_tmp == NULL) return;
...
void lv_textarea_set_placeholder_text(lv_obj_t * ta, const char * txt)
{
    ...
    if(txt_len == 0) {
        ...
        if(ext->placeholder_txt == NULL) {
            ext->placeholder_txt = (char *)lv_mem_alloc(txt_len + 1);
        }
        else {
            ext->placeholder_txt = (char *)lv_mem_realloc(ext->placeholder_txt, txt_len + 1);
...
void lv_textarea_set_pwd_mode(lv_obj_t * ta, bool en)
{
    ...
    if(en != false) {
        ...
        ext->pwd_tmp = (char *)lv_mem_alloc(len + 1);
        LV_ASSERT_MEM(ext->pwd_tmp);
        if(ext->pwd_tmp == NULL) return;
...
static lv_res_t lv_textarea_signal(lv_obj_t * ta, lv_signal_t sign, void * param)
{
    ...
    if(sign == LV_SIGNAL_CLEANUP) {
        ...
        if(ext->label) {
            if(lv_obj_get_width(ta) != lv_area_get_width((const lv_area_t *)param) || lv_obj_get_height(ta) != lv_area_get_height((const lv_area_t *)param)) {
                lv_obj_set_width(ext->label, lv_page_get_width_fit(ta));
...
static lv_res_t lv_textarea_scrollable_signal(lv_obj_t * scrl, lv_signal_t sign, void * param)
{
    ...
    if(sign == LV_SIGNAL_REFR_EXT_DRAW_PAD) {
        ...
        /*Set the label width according to the text area width*/
        if(ext->label) {
            if(lv_obj_get_width(scrl) != lv_area_get_width((const lv_area_t *)param) ||
               lv_obj_get_height(scrl) != lv_area_get_height((const lv_area_t *)param)) {
...
static void pwd_char_hider(lv_obj_t * ta)
{
    ...
    if(ext->pwd_mode != 0) {
        ...
        char * txt_tmp = (char *)_lv_mem_buf_get(enc_len * bullet_len + 1);
        int32_t i;
        for(i = 0; i < enc_len; i++) {
...
#if LV_LABEL_TEXT_SEL
    lv_label_ext_t * ext_label = (lv_label_ext_t *)lv_obj_get_ext_attr(ext->label);
