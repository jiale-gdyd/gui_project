...
lv_obj_t * lv_cont_create(lv_obj_t * par, const lv_obj_t * copy)
{
    ...
    lv_cont_ext_t * ext = (lv_cont_ext_t *)lv_obj_get_ext_attr(cont);
    if(ext == NULL) {
        lv_obj_del(cont);
        return NULL;
    }
    ...
    /*Copy an existing object*/
        ...
        lv_cont_ext_t * copy_ext = (lv_cont_ext_t *)lv_obj_get_ext_attr(copy);
...
static lv_res_t lv_cont_signal(lv_obj_t * cont, lv_signal_t sign, void * param)
{
    ...
    if(sign == LV_SIGNAL_STYLE_CHG) { /*Recalculate the padding if the style changed*/
        ...
        if(lv_obj_get_width(cont) != lv_area_get_width((const lv_area_t *)param) || lv_obj_get_height(cont) != lv_area_get_height((const lv_area_t *)param)) {
            lv_cont_refr_layout(cont);
            lv_cont_refr_autofit(cont);
        }
    }
...
static void lv_cont_layout_col(lv_obj_t * cont)
{
    ...
    for (child = (lv_obj_t *)_lv_ll_get_tail(&cont->child_ll); child != NULL; child = (lv_obj_t *)_lv_ll_get_prev(&cont->child_ll, child)) {
...
static void lv_cont_layout_center(lv_obj_t * cont)
{
    ...
    for (child = (lv_obj_t *)_lv_ll_get_head(&cont->child_ll); child != NULL; child = (lv_obj_t *)_lv_ll_get_next(&cont->child_ll, child)) {
        if(lv_obj_get_hidden(child) != false || lv_obj_is_protected(child, LV_PROTECT_POS) != false) continue;
...
static void lv_cont_layout_pretty(lv_obj_t * cont)
{
    ...
    child_rs = (lv_obj_t *)_lv_ll_get_tail(&cont->child_ll); /*Set the row starter child*/
    ...
    while(child_rs != NULL) {
        ...
        do {
            if(lv_obj_get_hidden(child_rc) == false && lv_obj_is_protected(child_rc, LV_PROTECT_POS) == false) {
                ...
                if(w_row + w > w_obj) {
                    ...
                    if(child_rc != NULL && obj_num != 0) {
                        child_rc = (lv_obj_t *)_lv_ll_get_next(&cont->child_ll, child_rc);
                    }
                    break;
                }
            ...
            child_rc = (lv_obj_t *)_lv_ll_get_prev(&cont->child_ll, child_rc); /*Load the next object*/
            if(obj_num == 0)
        ...
        /* Align the children (from child_rs to child_rc)*/
            ...
            while(child_tmp != NULL) {
                ...
                child_tmp = (lv_obj_t *)_lv_ll_get_prev(&cont->child_ll, child_tmp);
            }
        }

        if(child_rc == NULL) break;
        act_y += pinner + h_row;           /*y increment*/
        child_rs = (lv_obj_t *)_lv_ll_get_prev(&cont->child_ll, child_rc); /*Go to the next object*/
        child_rc = child_rs;
    }
    lv_obj_clear_protect(cont, LV_PROTECT_CHILD_CHG);
}
...
static void lv_cont_refr_autofit(lv_obj_t * cont)
{
    ...
    if(recursion_level <= LV_LAYOUT_MAX_RECURSION) {
        ...
        if(has_children) {
            ...
            for (child_i = (lv_obj_t *)_lv_ll_get_head(&cont->child_ll); child_i != NULL; child_i = (lv_obj_t *)_lv_ll_get_next(&cont->child_ll, child_i)) {
