...
static gd_GIF * gif_open(gd_GIF * gif_base)
{
    ...
    gif = (gd_GIF *)lv_mem_alloc(sizeof(gd_GIF) + 5 * width * height);
#elif LV_COLOR_DEPTH == 16
    gif = (gd_GIF *)lv_mem_alloc(sizeof(gd_GIF) + 4 * width * height);
#elif LV_COLOR_DEPTH == 8
    gif = (gd_GIF *)lv_mem_alloc(sizeof(gd_GIF) + 3 * width * height);
#endif
...
new_table(int key_size)
{
    ...
    Table *table = (Table *)lv_mem_alloc(sizeof(*table) + sizeof(Entry) * init_bulk);
    if (table) {
...
add_entry(Table **tablep, uint16_t length, uint16_t prefix, uint8_t suffix)
{
    ...
    if (table->nentries == table->bulk) {
        table->bulk *= 2;
        table = (Table *)lv_mem_realloc(table, sizeof(*table) + sizeof(Entry) * table->bulk);
        if (!table) return -1;
        table->entries = (Entry *) &table[1];
        *tablep = table;
    }
...
{
    lv_fs_file_t * file_p = (lv_fs_file_t *)lv_mem_alloc(sizeof(lv_fs_file_t));
...
#   if LVGL_VERSION_MAJOR > 7
        gif->fd = lv_fs_open((const char *)path, LV_FS_MODE_RD);
#   else
        gif->fd = (lv_fs_file_t *)lv_fs_open_to_old((const char *)path, LV_FS_MODE_RD);
#   endif
