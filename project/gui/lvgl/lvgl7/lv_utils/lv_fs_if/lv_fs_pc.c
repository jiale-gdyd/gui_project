...
static lv_fs_res_t fs_open (lv_fs_drv_t * drv, void * file_p, const char * path, lv_fs_mode_t mode)
{
	...
	file_t * fp = (file_t *)file_p;        /*Just avoid the confusing casings*/
	*fp = f;
...
{
	(void) drv;		/*Unused*/
	static char new_[512];
	static char old_[512];

	sprintf(old_, LV_FS_PC_PATH "/%s", oldname);
	sprintf(new_, LV_FS_PC_PATH "/%s", newname);

	int r = rename(old_, new_);
...
static lv_fs_res_t fs_dir_open (lv_fs_drv_t * drv, void * dir_p, const char *path)
{
	...
	if ((d = opendir(buf)) == NULL) {
		 ...
		 * we need to store our file descriptor here*/
		dir_t * dp = (dir_t *)dir_p;        /*Just avoid the confusing casings*/
		*dp = d;
	}
#else
