...
{
    lv_font_fmt_ft_dsc_t * dsc = (lv_font_fmt_ft_dsc_t *)lv_mem_alloc(sizeof(lv_font_fmt_ft_dsc_t));
    if(dsc == NULL) return false;

    dsc->font = (lv_font_t *)lv_mem_alloc(sizeof(lv_font_t));
    if(dsc->font == NULL) {
        lv_mem_free(dsc);
        return false;
    }
    ...
    if (face == NULL) {
        ...
        face_info = (lv_face_info_t *)lv_mem_alloc(sizeof(lv_face_info_t) + strlen(info->name) + 1);
        if(face_info == NULL) {
            lv_mem_free(dsc->font);
            lv_mem_free(dsc);
            return false;
...
static FT_Face face_find_in_list(lv_ft_info_t *info)
{
    ...
    FT_Face *pface = (FT_Face *)_lv_ll_get_head(&face_control.face_ll);
    while(pface) {
        face_info = (lv_face_info_t *)(*pface)->generic.data;
        if (strcmp(face_info->name, info->name) == 0) {
            return *pface;
        }
        pface = (FT_Face *)_lv_ll_get_next(&face_control.face_ll, pface);
    }
