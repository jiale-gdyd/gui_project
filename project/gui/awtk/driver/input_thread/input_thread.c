...
static int32_t map_key(uint16_t code)
{
    if ((custom_keys() != NULL) && (custom_keys_nr() > 0)) {
        const key_type_value_t *key_value = find_item_by_value(custom_keys(), custom_keys_nr(), (uint32_t)code);
        if (key_value != NULL) {
...
static ret_t input_dispatch_one_event(run_info_t *info)
{
    ...
    switch (e.type) {
        ...
        case EV_KEY: {
            if (e.code == BTN_LEFT || e.code == BTN_RIGHT || e.code == BTN_MIDDLE || e.code == BTN_TOUCH) {
