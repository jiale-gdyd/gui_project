...
int PackageApp1(IFD_S ifd0[], IFD_S ifd1[], int ifd0_len, int ifd1_len, char *app1_buf, char *thumbnail_buf, int thumbnail_len)
{
    int data_len = 0;
    int total_len = 0;
    int dir_start = 0;
    ...
    *(short *)(app1_buf + dir_start) = ifd0_len;
    ...
    for (int i = 0; i < ifd0_len; i++, dir_start += 12) {
        ...
        if (value_len > 4) {
            if (ifd0[i].format == 2) {
                memcpy(app1_buf + data_start, ifd0[i].value[0].asv, strlen(ifd0[i].value[0].asv));
            } else {
    ...
    *(short *)(app1_buf + dir_start) = ifd1_len;
    ...
    for (int i = 0; i < ifd1_len; i++, dir_start += 12) {
        ...
        if (ifd1[i].tag_no == 0x0201) {
            ...
            if (ifd1[i].format == 2) {
                memcpy(app1_buf + data_start, ifd1[i].value[0].asv, strlen(ifd1[i].value[0].asv));
            } else {
...
int PackageApp2(IFD_S mpfd[], int mpfd_len, int image_cnt, char *app2_buf, int thumbnail_len[])
{
    int data_len = 0;
    int dir_start = 0;
    int mpf_offset[3];
    ...
    for (int i = 0; i < mpfd_len; i++, dir_start += 12) {
        ...
        if (mpfd[i].tag_no == 0xB001) {
            ...
            for (int j = 0; j < image_cnt; j++) {
                if (j == 0) {
                    *(long *)(app2_buf + data_start + j * 16) = (1 << 31) | (0 << 30) | (1 << 29) | 0x030000;
                } else {
                    *(long *)(app2_buf + data_start + j * 16) = (0 << 31) | (1 << 30) | (0 << 29) | 0x010001;
                }
            ...
            if (mpfd[i].format == 2) {
                memcpy(app2_buf + data_start, mpfd[i].value[0].asv, strlen(mpfd[i].value[0].asv));
            } else {
