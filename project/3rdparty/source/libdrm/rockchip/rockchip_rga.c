...
int get_string_of_cmd(int index,char *buf)
{
    switch (index) {
        ...
        case SRC_Y_RGB_BASE_ADDR:
            strcpy(buf,"SRC_Y_RGB_BASE_ADDR");
            break;

        case SRC_CB_BASE_ADDR:
            strcpy(buf,"SRC_CB_BASE_ADDR   ");
            break;

        case SRC_CR_BASE_ADDR:
            strcpy(buf,"SRC_CR_BASE_ADDR   ");
            break;

        case SRC1_RGB_BASE_ADDR:
            strcpy(buf,"SRC1_RGB_BASE_ADDR ");
            break;

        case SRC_VIR_INFO:
            strcpy(buf,"SRC_VIR_INFO       ");
            break;

        case SRC_ACT_INFO:
            strcpy(buf,"SRC_ACT_INFO       ");
            break;

        case SRC_X_FACTOR:
            strcpy(buf,"SRC_X_FACTOR       ");
            break;

        case SRC_Y_FACTOR:
            strcpy(buf,"SRC_Y_FACTOR       ");
            break;

        case SRC_BG_COLOR:
            strcpy(buf,"SRC_BG_COLOR       ");
            break;

        case SRC_FG_COLOR:
            strcpy(buf,"SRC_FG_COLOR       ");
            break;

        case SRC_TR_COLOR0:
            strcpy(buf,"SRC_TR_COLOR0      ");
            break;

        case SRC_TR_COLOR1:
            strcpy(buf,"SRC_TR_COLOR1      ");
            break;
        ...
        case DST_Y_RGB_BASE_ADDR:
            strcpy(buf,"DST_Y_RGB_BASE_ADDR");
            break;

        case DST_CB_BASE_ADDR:
            strcpy(buf,"DST_CB_BASE_ADDR   ");
            break;

        case DST_CR_BASE_ADDR:
            strcpy(buf,"DST_CR_BASE_ADDR   ");
            break;

        case DST_VIR_INFO:
            strcpy(buf,"DST_VIR_INFO       ");
            break;

        case DST_ACT_INFO:
            strcpy(buf,"DST_ACT_INFO       ");
            break;

        case ALPHA_CTRL0:
            strcpy(buf,"ALPHA_CTRL0        ");
            break;

        case ALPHA_CTRL1:
            strcpy(buf,"ALPHA_CTRL1        ");
            break;

        case FADING_CTRL:
            strcpy(buf,"FADING_CTRL        ");
            break;
        ...
        case MASK_BASE:
            strcpy(buf,"MASK_BASE          ");
            break;

        case MMU_CTRL1:
            strcpy(buf,"MMU_CTRL1          ");
            break;

        case MMU_SRC_BASE:
            strcpy(buf,"MMU_SRC_BASE       ");
            break;

        case MMU_SRC1_BASE:
            strcpy(buf,"MMU_SRC1_BASE      ");
            break;

        case MMU_DST_BASE:
            strcpy(buf,"MMU_DST_BASE       ");
            break;

        case MMU_ELS_BASE:
            strcpy(buf,"MMU_ELS_BASE       ");
            break;
...
static struct rga_corners_addr_offset rga_get_addr_offset(struct rga_image *img, unsigned int x, unsigned int y, unsigned int w, unsigned int h)
{
...
static struct rga_addr_offset *rga_lookup_draw_pos(struct rga_corners_addr_offset *offsets, enum e_rga_src_rot_mode rotate_mode, enum e_rga_src_mirr_mode mirr_mode)
{
...
static int rga_add_cmd(struct rga_context *ctx, unsigned long cmd, unsigned long value)
{
    char buf[25];
    const char *fmt;
...
int rga_dump_context(struct rga_context ctx)
{
    ...
    fprintf(stderr, "********************frame start************************\n"
                ...
                "cmdlist_nr=%d\n",
                    ctx.fd, ctx.major, ctx.minor, ctx.cmd_nr, ctx.cmd, ctx.cmd_buf_nr, ctx.cmd_buf, ctx.cmdlist_nr);
...
static void rga_add_base_addr(struct rga_context *ctx, struct rga_image *img, enum rga_base_addr_reg reg)
{
    const unsigned long cmd = (reg == rga_dst) ? DST_Y_RGB_BASE_ADDR : SRC_Y_RGB_BASE_ADDR;
...
int rga_multiple_transform(struct rga_context *ctx, struct rga_image *src, struct rga_image *dst, unsigned int src_x, unsigned int src_y, unsigned int src_w, unsigned int src_h, unsigned int dst_x, unsigned int dst_y, unsigned int dst_w, unsigned int dst_h, unsigned int degree, unsigned int x_mirr, unsigned int y_mirr)
{
    ...
    if (src_info.data.rot_mode == RGA_SRC_ROT_MODE_90_DEGREE || src_info.data.rot_mode == RGA_SRC_ROT_MODE_270_DEGREE) {
    ...
    if (rga_src_color_is_yuv(src_info.data.format) && rga_dst_color_is_yuv(dst_info.data.format)) {
    ...
    src_vir_info.data.vir_stride = 0x3ff;
    ...
    dst_offset = rga_lookup_draw_pos(&offsets, src_info.data.rot_mode, src_info.data.mir_mode);
...
int rga_copy_with_rotate(struct rga_context *ctx, struct rga_image *src, struct rga_image *dst, unsigned int src_x, unsigned int src_y, unsigned int src_w, unsigned int src_h, unsigned int dst_x, unsigned int dst_y, unsigned int dst_w, unsigned int dst_h, unsigned int degree)
{
    ...
    return rga_multiple_transform(ctx, src, dst, src_x, src_y, src_w, src_h, dst_x, dst_y, dst_w, dst_h, degree, 0, 0);
}
...
{
    return rga_multiple_transform(ctx, src, dst, src_x, src_y, src_w, src_h, dst_x, dst_y, dst_w, dst_h, 0, 0, 0);
}

int rga_copy(struct rga_context *ctx, struct rga_image *src, struct rga_image *dst, unsigned int src_x, unsigned int src_y, unsigned int dst_x, unsigned dst_y, unsigned int w, unsigned int h)
{
