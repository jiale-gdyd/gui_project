...
static const char *modifier_to_string(uint64_t modifier)
{
    ...
    if (!modifier_name) {
        if (vendor_name) {
            snprintf(mod_string, sizeof(mod_string), "%s_%s", vendor_name, "UNKNOWN_MODIFIER");
        } else {
            snprintf(mod_string, sizeof(mod_string), "%s_%s", "UNKNOWN_VENDOR", "UNKNOWN_MODIFIER");
        }
    ...
    snprintf(mod_string, sizeof(mod_string), "%s_%s", vendor_name, modifier_name);
    free(modifier_name);
    free(vendor_name);
...
static void dump_prop(struct device *dev, drmModePropertyPtr prop, uint32_t prop_id, uint64_t value)
{
    ...
    if (drm_property_type_is(prop, DRM_MODE_PROP_ENUM)) {
        ...
        for (i = 0; i < prop->count_enums; i++) {
            printf(" %s=%"PRIu64, prop->enums[i].name, (uint64_t)prop->enums[i].value);
        }
        printf("\n");
    ...
    for (i = 0; i < dev->resources->count_crtcs; i++) {
        ...
        if (_crtc->props) {
            ...
            for (j = 0; j < _crtc->props->count_props; j++) {
                dump_prop(dev, _crtc->props_info[j], _crtc->props->props[j], _crtc->props->prop_values[j]);
            }
        } else {
    ...
    for (i = 0; i < dev->resources->count_planes; i++) {
        ...
        if (plane->props) {
            ...
            for (j = 0; j < plane->props->count_props; j++) {
                dump_prop(dev, plane->props_info[j], plane->props->props[j], plane->props->prop_values[j]);
            }
        } else {
            printf("  no properties found\n");
        }
...
#define get_resource(_res, __res, type, Type)                                                           \
    if (need_resource(type)) {                                                                          \
        do {                                                                                            \
            for (i = 0; i < (int)(_res)->count_##type##s; ++i) {                                        \
                ...
                (_res)->type##s[i].type = drmModeGet##Type(dev->fd, type##id);                          \
                if (!(_res)->type##s[i].type) {                                                         \
    ...
    if (need_resource(connector)) {
        for (i = 0; i < res->count_connectors; i++) {
            ...
            num = asprintf(&connector->name, "%s-%u", drmModeGetConnectorTypeName(conn->connector_type), conn->connector_type_id);
            if (num < 0) {
                goto error;
            }
        }
    ...
    if (need_resource(type)) {                                                                                              \
        do {                                                                                                                \
            for (i = 0; i < (int)(_res)->count_##type##s; ++i) {                                                            \
                ...
                unsigned int j;                                                                                             \
                obj->props = drmModeObjectGetProperties(dev->fd, obj->type->type##_id, DRM_MODE_OBJECT_##Type);             \
                if (!obj->props) {                                                                                          \
                    fprintf(stderr, "could not get %s %i properties: %s\n", #type, obj->type->type##_id, strerror(errno));  \
                    continue;                                                                                               \
                }                                                                                                           \
                ...
                for (j = 0; j < obj->props->count_props; ++j) {                                                             \
                    obj->props_info[j] = drmModeGetProperty(dev->fd, obj->props->props[j]);                                 \
                }                                                                                                           \
            }                                                                                                               \
        } while (0);                                                                                                        \
    }
...
static int pipe_find_crtc_and_mode(struct device *dev, struct pipe_arg *pipe)
{
    ...
    for (i = 0; i < (int)pipe->num_cons; i++) {
        ...
        if (mode == NULL) {
            if (pipe->vrefresh) {
                fprintf(stderr, "failed to find mode \"%s-%.2fHz\" for connector %s\n", pipe->mode_str, pipe->vrefresh, pipe->cons[i]);
            } else {
...
static void page_flip_handler(int fd, unsigned int frame, unsigned int sec, unsigned int usec, void *data)
{
    ...
    drmModePageFlip(fd, pipe->crtc_id, new_fb_id, DRM_MODE_PAGE_FLIP_EVENT, pipe);
    pipe->current_fb_id = new_fb_id;
    pipe->swap_count++;

    if (pipe->swap_count == 60) {
        gettimeofday(&end, NULL);
        t = end.tv_sec + end.tv_usec * 1e-6 - (pipe->start.tv_sec + pipe->start.tv_usec * 1e-6);
...
static bool add_property_optional(struct device *dev, uint32_t obj_id, const char *name, uint64_t value)
{
    struct property_arg p;
...
static int bo_fb_create(int fd, unsigned int fourcc, const uint32_t w, const uint32_t h, enum util_fill_pattern pat, struct bo **out_bo, unsigned int *out_fb_id)
{
    struct bo *bo;
    unsigned int fb_id;
...
static int atomic_set_plane(struct device *dev, struct plane_arg *p, int pattern, bool update)
{
    ...
    if (!plane_bo) {
        if (bo_fb_create(dev->fd, p->fourcc, p->w, p->h, pattern, &plane_bo, &p->fb_id)) {
            return -1;
        }
    }
...
static int set_plane(struct device *dev, struct plane_arg *p)
{
    ...
    for (i = 0; i < dev->resources->count_planes; i++) {
        ...
        if ((ovr->possible_crtcs & crtc_mask) && (ovr->crtc_id == 0 || ovr->crtc_id == p->crtc_id)) {
    ...
    if (bo_fb_create(dev->fd, p->fourcc, p->w, p->h, secondary_fill, &p->bo, &p->fb_id)) {
        return -1;
    }
    ...
    if (drmModeSetPlane(dev->fd, plane_id, p->crtc_id, p->fb_id, 0, crtc_x, crtc_y, crtc_w, crtc_h, 0, 0, p->w << 16, p->h << 16)) {
...
static void atomic_set_planes(struct device *dev, struct plane_arg *p, unsigned int count, bool update)
{
...
static void atomic_test_page_flip(struct device *dev, struct pipe_arg *pipe_args, struct plane_arg *plane_args, unsigned int plane_count)
{
    int ret;
    ...
    while (true) {
        ...
        if (pipe_args->swap_count == 60) {
            ...
            gettimeofday(&end, NULL);
            t = end.tv_sec + end.tv_usec * 1e-6 - (pipe_args->start.tv_sec + pipe_args->start.tv_usec * 1e-6);
...
static int pipe_attempt_connector(struct device *dev, drmModeConnector *con, struct pipe_arg *pipe)
{
    int i;
    char *con_str;
...
static void set_mode(struct device *dev, struct pipe_arg *pipes, unsigned int count)
{
    ...
    if (!dev->use_atomic) {
        ...
        if (bo_fb_create(dev->fd, pipes[0].fourcc, dev->mode.width, dev->mode.height, primary_fill, &dev->mode.bo, &dev->mode.fb_id)) {
            return;
        }
    }
    ...
    for (i = 0; i < count; i++) {
        ...
        printf("setting mode %s-%.2fHz on connectors ", pipe->mode->name, mode_vrefresh(pipe->mode));
        ...
        if (!dev->use_atomic) {
            ret = drmModeSetCrtc(dev->fd, pipe->crtc_id, dev->mode.fb_id, x, 0, pipe->con_ids, pipe->num_cons, pipe->mode);
...
static void set_cursors(struct device *dev, struct pipe_arg *pipes, unsigned int count)
{
    ...
    bo = bo_create(dev->fd, DRM_FORMAT_ARGB8888, cw, ch, handles, pitches, offsets, UTIL_PATTERN_PLAIN);
    if (bo == NULL) {
        return;
    }
    ...
    for (i = 0; i < count; i++) {
        struct pipe_arg *pipe = &pipes[i];
        ret = cursor_init(dev->fd, handles[0], pipe->crtc_id, pipe->mode->hdisplay, pipe->mode->vdisplay, cw, ch);
        if (ret) {
...
static void test_page_flip(struct device *dev, struct pipe_arg *pipes, unsigned int count)
{
    ...
    if (bo_fb_create(dev->fd, pipes[0].fourcc, dev->mode.width, dev->mode.height, UTIL_PATTERN_PLAIN, &other_bo, &other_fb_id)) {
        return;
    }
    ...
    for (i = 0; i < count; i++) {
        ...
        ret = drmModePageFlip(dev->fd, pipe->crtc_id, other_fb_id, DRM_MODE_PAGE_FLIP_EVENT, pipe);
        if (ret) {
...
static int parse_connector(struct pipe_arg *pipe, const char *arg)
{
    ...
    len = libdrm_min(sizeof pipe->mode_str - 1, (unsigned int)(p - arg));
...
int libdrm_modetest_main(int argc, char **argv)
{
    int c;
    int ret;
    unsigned int i;
