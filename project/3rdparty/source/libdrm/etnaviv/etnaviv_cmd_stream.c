...
static inline struct etna_cmd_stream_priv *etna_cmd_stream_priv(struct etna_cmd_stream *stream)
{
    return (struct etna_cmd_stream_priv *)stream;
}

drm_public struct etna_cmd_stream *etna_cmd_stream_new(struct etna_pipe *pipe, uint32_t size, void (*reset_notify)(struct etna_cmd_stream *stream, void *priv), void *priv)
{
...
static void flush(struct etna_cmd_stream *stream, int in_fence_fd, int *out_fence_fd)
{
    struct etna_cmd_stream_priv *priv = etna_cmd_stream_priv(stream);
    ...
    ret = drmCommandWriteRead(gpu->dev->fd, DRM_ETNAVIV_GEM_SUBMIT, &req, sizeof(req));
    if (ret) {
...
drm_public void etna_cmd_stream_flush2(struct etna_cmd_stream *stream, int in_fence_fd, int *out_fence_fd)
{
...
drm_public void etna_cmd_stream_reloc(struct etna_cmd_stream *stream, const struct etna_reloc *r)
{
    uint32_t addr = 0;
    struct etna_cmd_stream_priv *priv = etna_cmd_stream_priv(stream);
    ...
    reloc->submit_offset = stream->offset * 4;
    reloc->flags = 0;
