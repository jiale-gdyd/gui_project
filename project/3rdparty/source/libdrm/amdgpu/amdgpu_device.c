...
static void amdgpu_device_reference(struct amdgpu_device **dst, struct amdgpu_device *src)
{
...
drm_public int amdgpu_device_initialize(int fd, uint32_t *major_version, uint32_t *minor_version, amdgpu_device_handle *device_handle)
{
    int r;
    int flag_auth = 0;
    uint64_t start, max;
    ...
    *device_handle = NULL;
    ...
    if (version->version_major != 3) {
        fprintf(stderr, "%s: DRM version is %d.%d.%d but this driver is only compatible with 3.x.x.\n", __func__, version->version_major, version->version_minor, version->version_patchlevel);
    ...
    amdgpu_vamgr_init(&dev->vamgr_32, start, max, dev->dev_info.virtual_address_alignment);

    start = max;
    max = MAX2(dev->dev_info.virtual_address_max, 0x100000000ULL);
    amdgpu_vamgr_init(&dev->vamgr, start, max, dev->dev_info.virtual_address_alignment);

    start = dev->dev_info.high_va_offset;
    max = MIN2(dev->dev_info.high_va_max, (start & ~0xffffffffULL) + 0x100000000ULL);
    amdgpu_vamgr_init(&dev->vamgr_high_32, start, max, dev->dev_info.virtual_address_alignment);

    start = max;
    max = MAX2(dev->dev_info.high_va_max, (start & ~0xffffffffULL) + 0x100000000ULL);
    amdgpu_vamgr_init(&dev->vamgr_high, start, max, dev->dev_info.virtual_address_alignment);
...
drm_public int amdgpu_query_sw_info(amdgpu_device_handle dev, enum amdgpu_sw_info info, void *value)
{
