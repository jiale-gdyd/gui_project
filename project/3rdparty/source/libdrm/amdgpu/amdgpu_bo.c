...
static int amdgpu_bo_create(amdgpu_device_handle dev, uint64_t size, uint32_t handle, amdgpu_bo_handle *buf_handle)
{
    int r;
    struct amdgpu_bo *bo;
...
drm_public int amdgpu_bo_alloc(amdgpu_device_handle dev, struct amdgpu_bo_alloc_request *alloc_buffer, amdgpu_bo_handle *buf_handle)
{
    int r;
    ...
    r = amdgpu_bo_create(dev, alloc_buffer->alloc_size, args.out.handle, buf_handle);
...
drm_public int amdgpu_bo_set_metadata(amdgpu_bo_handle bo, struct amdgpu_bo_metadata *info)
{
...
drm_public int amdgpu_bo_query_info(amdgpu_bo_handle bo, struct amdgpu_bo_info *info)
{
    int r;
...
static int amdgpu_bo_export_flink(amdgpu_bo_handle bo)
{
    ...
    if (bo->dev->flink_fd != bo->dev->fd) {
        r = drmPrimeHandleToFD(bo->dev->fd, bo->handle, DRM_CLOEXEC, &dma_fd);
        if (!r) {
...
drm_public int amdgpu_bo_export(amdgpu_bo_handle bo, enum amdgpu_bo_handle_type type, uint32_t *shared_handle)
{
    int r;
    ...
    switch (type) {
        ...
        case amdgpu_bo_handle_type_dma_buf_fd:
            return drmPrimeHandleToFD(bo->dev->fd, bo->handle, DRM_CLOEXEC | DRM_RDWR, (int *)shared_handle);
    }

    return -EINVAL;
}

drm_public int amdgpu_bo_import(amdgpu_device_handle dev, enum amdgpu_bo_handle_type type, uint32_t shared_handle, struct amdgpu_bo_import_result *output)
{
    int r = 0;
    int dma_fd;
    ...
    switch (type) {
        case amdgpu_bo_handle_type_gem_flink_name:
            ...
            if (dev->flink_fd != dev->fd) {
                r = drmPrimeHandleToFD(dev->flink_fd, handle, DRM_CLOEXEC, &dma_fd);
                if (r) {
                    goto free_bo_handle;
                }
...
drm_public int amdgpu_bo_cpu_map(amdgpu_bo_handle bo, void **cpu)
{
    ...
    ptr = drm_mmap(NULL, bo->alloc_size, PROT_READ | PROT_WRITE, MAP_SHARED, bo->dev->fd, args.out.addr_ptr);
    if (ptr == MAP_FAILED) {
        pthread_mutex_unlock(&bo->cpu_access_mutex);
        return -errno;
    }
...
drm_public int amdgpu_query_buffer_size_alignment(amdgpu_device_handle dev, struct amdgpu_buffer_size_alignments *info)
{
...
drm_public int amdgpu_bo_wait_for_idle(amdgpu_bo_handle bo, uint64_t timeout_ns, bool *busy)
{
    int r;
    ...
    r = drmCommandWriteRead(bo->dev->fd, DRM_AMDGPU_GEM_WAIT_IDLE, &args, sizeof(args));
    if (r == 0) {
...
drm_public int amdgpu_find_bo_by_cpu_mapping(amdgpu_device_handle dev, void *cpu, uint64_t size, amdgpu_bo_handle *buf_handle, uint64_t *offset_in_bo)
{
    int r = 0;
    uint32_t i;
    struct amdgpu_bo *bo = NULL;
    ...
    for (i = 0; i < dev->bo_handles.max_key; i++) {
        ...
        if ((cpu >= bo->cpu_ptr) && (cpu < (void *)((uintptr_t)bo->cpu_ptr + bo->alloc_size))) {
            break;
        }
    }
...
drm_public int amdgpu_create_bo_from_user_mem(amdgpu_device_handle dev, void *cpu, uint64_t size, amdgpu_bo_handle *buf_handle)
{
    int r;
    struct drm_amdgpu_gem_userptr args;

    args.addr = (uintptr_t)cpu;
    args.flags = AMDGPU_GEM_USERPTR_ANONONLY | AMDGPU_GEM_USERPTR_REGISTER | AMDGPU_GEM_USERPTR_VALIDATE;
    args.size = size;
    r = drmCommandWriteRead(dev->fd, DRM_AMDGPU_GEM_USERPTR, &args, sizeof(args));
    if (r) {
        goto out;
    }
...
drm_public int amdgpu_bo_list_create_raw(amdgpu_device_handle dev, uint32_t number_of_buffers, struct drm_amdgpu_bo_list_entry *buffers, uint32_t *result)
{
    int r;
    ...
    r = drmCommandWriteRead(dev->fd, DRM_AMDGPU_BO_LIST, &args, sizeof(args));
    if (!r) {
...
drm_public int amdgpu_bo_list_destroy_raw(amdgpu_device_handle dev, uint32_t bo_list)
{
    union drm_amdgpu_bo_list args;
    ...
    return drmCommandWriteRead(dev->fd, DRM_AMDGPU_BO_LIST, &args, sizeof(args));
}

drm_public int amdgpu_bo_list_create(amdgpu_device_handle dev, uint32_t number_of_resources, amdgpu_bo_handle *resources, uint8_t *resource_prios, amdgpu_bo_list_handle *result)
{
    int r;
    unsigned i;
    union drm_amdgpu_bo_list args;
...
drm_public int amdgpu_bo_list_destroy(amdgpu_bo_list_handle list)
{
    ...
    r = drmCommandWriteRead(list->dev->fd, DRM_AMDGPU_BO_LIST, &args, sizeof(args));
    if (!r) {
        free(list);
    }
...
drm_public int amdgpu_bo_list_update(amdgpu_bo_list_handle handle, uint32_t number_of_resources, amdgpu_bo_handle *resources, uint8_t *resource_prios)
{
    int r;
    unsigned i;
    union drm_amdgpu_bo_list args;
    ...
    r = drmCommandWriteRead(handle->dev->fd, DRM_AMDGPU_BO_LIST, &args, sizeof(args));
    free(list);

    return r;
}

drm_public int amdgpu_bo_va_op(amdgpu_bo_handle bo, uint64_t offset, uint64_t size, uint64_t addr, uint64_t flags, uint32_t ops)
{
    amdgpu_device_handle dev = bo->dev;

    size = ALIGN(size, getpagesize());
    return amdgpu_bo_va_op_raw(dev, bo, offset, size, addr, AMDGPU_VM_PAGE_READABLE | AMDGPU_VM_PAGE_WRITEABLE | AMDGPU_VM_PAGE_EXECUTABLE, ops);
}

drm_public int amdgpu_bo_va_op_raw(amdgpu_device_handle dev, amdgpu_bo_handle bo, uint64_t offset, uint64_t size, uint64_t addr, uint64_t flags, uint32_t ops)
{
    int r;
    struct drm_amdgpu_gem_va va;

    if ((ops != AMDGPU_VA_OP_MAP) && (ops != AMDGPU_VA_OP_UNMAP) && (ops != AMDGPU_VA_OP_REPLACE) && (ops != AMDGPU_VA_OP_CLEAR)) {
        return -EINVAL;
    }
