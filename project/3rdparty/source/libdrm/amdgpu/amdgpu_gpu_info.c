...
drm_public int amdgpu_read_mm_registers(amdgpu_device_handle dev, unsigned dword_offset, unsigned count, uint32_t instance, uint32_t flags, uint32_t *values)
{
    struct drm_amdgpu_info request;
...
drm_public int amdgpu_query_hw_ip_count(amdgpu_device_handle dev, unsigned type, uint32_t *count)
{
    struct drm_amdgpu_info request;
...
drm_public int amdgpu_query_hw_ip_info(amdgpu_device_handle dev, unsigned type, unsigned ip_instance, struct drm_amdgpu_info_hw_ip *info)
{
    struct drm_amdgpu_info request;
...
drm_public int amdgpu_query_firmware_version(amdgpu_device_handle dev, unsigned fw_type, unsigned ip_instance, unsigned index, uint32_t *version, uint32_t *feature)
{
    int r;
    struct drm_amdgpu_info request;
    ...
    r = drmCommandWrite(dev->fd, DRM_AMDGPU_INFO, &request, sizeof(struct drm_amdgpu_info));
    if (r) {
        return r;
    }
...
drm_private int amdgpu_query_gpu_info_init(amdgpu_device_handle dev)
{
    ...
    r = amdgpu_query_info(dev, AMDGPU_INFO_DEV_INFO, sizeof(dev->dev_info), &dev->dev_info);
    if (r) {
        return r;
    }
    ...
    dev->info.num_shader_arrays_per_engine = dev->dev_info.num_shader_arrays_per_engine;
    ...
    if (dev->info.family_id < AMDGPU_FAMILY_AI) {
        for (i = 0; i < (int)dev->info.num_shader_engines; i++) {
            unsigned instance = (i << AMDGPU_INFO_MMR_SE_INDEX_SHIFT) | (AMDGPU_INFO_MMR_SH_INDEX_MASK << AMDGPU_INFO_MMR_SH_INDEX_SHIFT);

            r = amdgpu_read_mm_registers(dev, 0x263d, 1, instance, 0, &dev->info.backend_disable[i]);
            if (r) {
                return r;
            }

            dev->info.backend_disable[i] = (dev->info.backend_disable[i] >> 16) & 0xff;

            r = amdgpu_read_mm_registers(dev, 0xa0d4, 1, instance, 0, &dev->info.pa_sc_raster_cfg[i]);
            if (r) {
                return r;
            }

            if (dev->info.family_id >= AMDGPU_FAMILY_CI) {
                r = amdgpu_read_mm_registers(dev, 0xa0d5, 1, instance, 0, &dev->info.pa_sc_raster_cfg1[i]);
                if (r) {
                    return r;
                }
            }
    ...
    if (dev->info.family_id < AMDGPU_FAMILY_AI) {
        r = amdgpu_read_mm_registers(dev, 0x2644, 32, 0xffffffff, 0, dev->info.gb_tile_mode);
        if (r) {
            return r;
        }

        if (dev->info.family_id >= AMDGPU_FAMILY_CI) {
            r = amdgpu_read_mm_registers(dev, 0x2664, 16, 0xffffffff, 0, dev->info.gb_macro_tile_mode);
            if (r) {
                return r;
            }
        }

        r = amdgpu_read_mm_registers(dev, 0x9d8, 1, 0xffffffff, 0, &dev->info.mc_arb_ramcfg);
        if (r) {
            return r;
        }
    }
...
drm_public int amdgpu_query_gpu_info(amdgpu_device_handle dev,struct amdgpu_gpu_info *info)
{
    if (!dev || !info) {
        return -EINVAL;
    }
...
drm_public int amdgpu_query_heap_info(amdgpu_device_handle dev, uint32_t heap, uint32_t flags, struct amdgpu_heap_info *info)
{
    int r;
    struct drm_amdgpu_info_vram_gtt vram_gtt_info = {};

    r = amdgpu_query_info(dev, AMDGPU_INFO_VRAM_GTT, sizeof(vram_gtt_info), &vram_gtt_info);
    if (r) {
        return r;
    }
    ...
    switch (heap) {
        case AMDGPU_GEM_DOMAIN_VRAM:
            ...
            if (flags & AMDGPU_GEM_CREATE_CPU_ACCESS_REQUIRED) {
                r = amdgpu_query_info(dev, AMDGPU_INFO_VIS_VRAM_USAGE, sizeof(info->heap_usage), &info->heap_usage);
            } else {
                r = amdgpu_query_info(dev, AMDGPU_INFO_VRAM_USAGE, sizeof(info->heap_usage), &info->heap_usage);
            }
        ...
        case AMDGPU_GEM_DOMAIN_GTT:
            ...
            r = amdgpu_query_info(dev, AMDGPU_INFO_GTT_USAGE, sizeof(info->heap_usage), &info->heap_usage);
            if (r) {
                return r;
            }
            break;
...
drm_public int amdgpu_query_gds_info(amdgpu_device_handle dev, struct amdgpu_gds_resource_info *gds_info)
{
    int r;
    ...
    r = amdgpu_query_info(dev, AMDGPU_INFO_GDS_CONFIG, sizeof(gds_config), &gds_config);
    if (r) {
        return r;
    }
...
drm_public int amdgpu_query_sensor_info(amdgpu_device_handle dev, unsigned sensor_type, unsigned size, void *value)
{
    struct drm_amdgpu_info request;
...
drm_public int amdgpu_query_video_caps_info(amdgpu_device_handle dev, unsigned cap_type, unsigned size, void *value)
{
    struct drm_amdgpu_info request;
