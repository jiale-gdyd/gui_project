...
static bool pushbuf_kref_fits(struct nouveau_pushbuf *push, struct nouveau_bo *bo, uint32_t *domains)
{
    ...
    if ((*domains & NOUVEAU_GEM_DOMAIN_VRAM) && ((krec->vram_used + bo->size) <= dev->vram_limit)) {
    ...
    for (i = 0; i < krec->nr_buffer; i++, kref++) {
        ...
        if (!(kref->valid_domains & NOUVEAU_GEM_DOMAIN_VRAM) || ((krec->vram_used + kbo->size) > dev->vram_limit)) {
            continue;
        }
...
static struct drm_nouveau_gem_pushbuf_bo *pushbuf_kref(struct nouveau_pushbuf *push, struct nouveau_bo *bo, uint32_t flags)
{
    ...
    if (kref) {
        ...
        if ((kref->valid_domains & NOUVEAU_GEM_DOMAIN_GART) && (domains == NOUVEAU_GEM_DOMAIN_VRAM)) {
            if ((krec->vram_used + bo->size) > dev->vram_limit) {
                return NULL;
            }
        ...
        if ((krec->nr_buffer == NOUVEAU_GEM_MAX_BUFFERS) || !pushbuf_kref_fits(push, bo, &domains)) {
            return NULL;
        }
...
static void pushbuf_dump(struct nouveau_pushbuf_krec *krec, int krec_id, int chid)
{
    ...
    err("chid:[%d]: krec:[%d], pushes:[%d], bufs:[%d], relocs:[%d]\n", chid, krec_id, krec->nr_push, krec->nr_buffer, krec->nr_reloc);

    kref = krec->buffer;
    for (i = 0; i < krec->nr_buffer; i++, kref++) {
        bo = (void *)(uintptr_t)kref->user_priv;
        err("ch%d: buf %08x %08x %08x %08x %08x %p 0x%" PRIx64 " 0x%" PRIx64 "\n", chid, i, kref->handle, kref->valid_domains, kref->read_domains, kref->write_domains, bo->map, bo->offset, bo->size);
    }
    ...
    for (i = 0; i < krec->nr_push; i++, kpsh++) {
        ...
        err("ch%d: psh %s%08x %010llx %010llx\n", chid, bo->map ? "" : "(unmapped) ", kpsh->bo_index, (unsigned long long)kpsh->offset, (unsigned long long)(kpsh->offset + kpsh->length));
        if (!bo->map) {
            continue;
        }
...
static int pushbuf_flush(struct nouveau_pushbuf *push)
{
    int ret = 0, i;
    struct nouveau_bo *bo;
...
drm_public int nouveau_pushbuf_new(struct nouveau_client *client, struct nouveau_object *chan, int nr, uint32_t size, bool immediate, struct nouveau_pushbuf **ppush)
{
    int ret;
...
drm_public void nouveau_pushbuf_del(struct nouveau_pushbuf **ppush)
{
...
drm_public struct nouveau_bufctx *nouveau_pushbuf_bufctx(struct nouveau_pushbuf *push, struct nouveau_bufctx *ctx)
{
...
drm_public int nouveau_pushbuf_space(struct nouveau_pushbuf *push, uint32_t dwords, uint32_t relocs, uint32_t pushes)
{
    int ret = 0;
    bool flushed = false;
...
drm_public void nouveau_pushbuf_data(struct nouveau_pushbuf *push, struct nouveau_bo *bo, uint64_t offset, uint64_t length)
{
    struct drm_nouveau_gem_pushbuf_bo *kref;
    struct drm_nouveau_gem_pushbuf_push *kpsh;
    struct nouveau_pushbuf_priv *nvpb = nouveau_pushbuf(push);
...
drm_public int nouveau_pushbuf_refn(struct nouveau_pushbuf *push, struct nouveau_pushbuf_refn *refs, int nr)
{
    return pushbuf_refn(push, true, refs, nr);
}

drm_public void nouveau_pushbuf_reloc(struct nouveau_pushbuf *push, struct nouveau_bo *bo, uint32_t data, uint32_t flags, uint32_t vor, uint32_t tor)
{
    *push->cur = pushbuf_krel(push, bo, data, flags, vor, tor);
    push->cur++;
}

drm_public int nouveau_pushbuf_validate(struct nouveau_pushbuf *push)
{
    return pushbuf_validate(push, true);
}

drm_public uint32_t nouveau_pushbuf_refd(struct nouveau_pushbuf *push, struct nouveau_bo *bo)
{
    uint32_t flags = 0;
    struct drm_nouveau_gem_pushbuf_bo *kref;
...
drm_public int nouveau_pushbuf_kick(struct nouveau_pushbuf *push, struct nouveau_object *chan)
{
    if (!push->channel) {
...
drm_public bool nouveau_check_dead_channel(struct nouveau_drm *drm, struct nouveau_object *chan)
{
    int ret;
    struct drm_nouveau_gem_pushbuf req = {};
    struct nouveau_fifo *fifo = chan->data;
