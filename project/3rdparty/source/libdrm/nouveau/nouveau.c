...
#include <libdrm/drm/nouveau_drm.h>
...
static int nouveau_object_ioctl(struct nouveau_object *obj, void *data, uint32_t size)
{
    int ret = -ENOSYS;
    uint32_t argc = size;
...
drm_public int nouveau_object_mthd(struct nouveau_object *obj, uint32_t mthd, void *data, uint32_t size)
{
    struct nouveau_drm *drm = nouveau_drm(obj);
    struct {
...
drm_public void nouveau_object_sclass_put(struct nouveau_sclass **psclass)
{
    free(*psclass);
    *psclass = NULL;
}

drm_public int nouveau_object_sclass_get(struct nouveau_object *obj, struct nouveau_sclass **psclass)
{
    struct nouveau_drm *drm = nouveau_drm(obj);
    struct {
        struct nvif_ioctl_v0        ioctl;
...
drm_public int nouveau_object_mclass(struct nouveau_object *obj, const struct nouveau_mclass *mclass)
{
    int cnt, i, j;
    int ret = -ENODEV;
    struct nouveau_sclass *sclass;
    ...
    for (i = 0; ret < 0 && mclass[i].oclass; i++) {
        for (j = 0; j < cnt; j++) {
            if ((mclass[i].oclass  == sclass[j].oclass) && (mclass[i].version >= sclass[j].minver) && (mclass[i].version <= sclass[j].maxver)) {
                ret = i;
                break;
            }
        }
...
static int nouveau_object_init(struct nouveau_object *parent, uint32_t handle, int32_t oclass, void *data, uint32_t size, struct nouveau_object *obj)
{
...
drm_public int nouveau_object_new(struct nouveau_object *parent, uint64_t handle, uint32_t oclass, void *data, uint32_t length, struct nouveau_object **pobj)
{
    int ret;
...
drm_public void nouveau_object_del(struct nouveau_object **pobj)
{
...
drm_public void nouveau_drm_del(struct nouveau_drm **pdrm)
{
    free(*pdrm);
    *pdrm = NULL;
}

drm_public int nouveau_drm_new(int fd, struct nouveau_drm **pdrm)
{
    drmVersionPtr ver;
    ...
    *pdrm = drm;
    drm->version = (ver->version_major << 24) | (ver->version_minor << 8) | ver->version_patchlevel;
...
drm_public int nouveau_device_open_existing(struct nouveau_device **pdev, int close, int fd, drm_context_t ctx)
{
    return -EACCES;
}

drm_public int nouveau_device_new(struct nouveau_object *parent, int32_t oclass, void *data, uint32_t size, struct nouveau_device **pdev)
{
    ...
    if ((oclass != NV_DEVICE) || nvif_unpack(ret, &data, &size, args->v0, 0, 0, false)) {
        return ret;
    }
    ...
    if (drm->nvif) {
        ret = nouveau_object_init(parent, 0, oclass, args, argc, &dev->object);
        if (ret) {
            goto done;
        }

        info.version = 0;
        ret = nouveau_object_mthd(&dev->object, NV_DEVICE_V0_INFO, &info, sizeof(info));
        if (ret) {
            goto done;
        }
...
drm_public int nouveau_device_wrap(int fd, int close, struct nouveau_device **pdev)
{
    int ret;
    struct nouveau_drm *drm;
    struct nouveau_device_priv *nvdev;
    ...
    ret = nouveau_device_new(&drm->client, NV_DEVICE, &(struct nv_device_v0) { .device = ~0ULL, }, sizeof(struct nv_device_v0), pdev);
    if (ret) {
        nouveau_drm_del(&drm);
        return ret;
    }
...
drm_public int nouveau_device_open(const char *busid, struct nouveau_device **pdev)
{
...
drm_public void nouveau_device_del(struct nouveau_device **pdev)
{
...
drm_public int nouveau_getparam(struct nouveau_device *dev, uint64_t param, uint64_t *value)
{
    struct nouveau_drm *drm = nouveau_drm(&dev->object);
    struct drm_nouveau_getparam r = { .param = param };
    int fd = drm->fd, ret = drmCommandWriteRead(fd, DRM_NOUVEAU_GETPARAM, &r, sizeof(r));
    *value = r.value;

    return ret;
}

drm_public int nouveau_setparam(struct nouveau_device *dev, uint64_t param, uint64_t value)
{
    struct nouveau_drm *drm = nouveau_drm(&dev->object);
...
drm_public int nouveau_client_new(struct nouveau_device *dev, struct nouveau_client **pclient)
{
    uint32_t *clients;
...
drm_public void nouveau_client_del(struct nouveau_client **pclient)
{
    struct nouveau_device_priv *nvdev;
...
drm_public int nouveau_bo_new(struct nouveau_device *dev, uint32_t flags, uint32_t align, uint64_t size, union nouveau_bo_config *config, struct nouveau_bo **pbo)
{
    int ret;
...
static int nouveau_bo_wrap_locked(struct nouveau_device *dev, uint32_t handle, struct nouveau_bo **pbo, int name)
{
    int ret;
...
drm_public int nouveau_bo_wrap(struct nouveau_device *dev, uint32_t handle, struct nouveau_bo **pbo)
{
    int ret;
    struct nouveau_device_priv *nvdev = nouveau_device(dev);
...
drm_public int nouveau_bo_name_ref(struct nouveau_device *dev, uint32_t name, struct nouveau_bo **pbo)
{
    int ret;
    struct nouveau_bo_priv *nvbo;
    struct nouveau_drm *drm = nouveau_drm(&dev->object);
...
drm_public int nouveau_bo_name_get(struct nouveau_bo *bo, uint32_t *name)
{
...
drm_public void nouveau_bo_ref(struct nouveau_bo *bo, struct nouveau_bo **pref)
{
...
drm_public int nouveau_bo_prime_handle_ref(struct nouveau_device *dev, int prime_fd, struct nouveau_bo **bo)
{
    int ret;
    unsigned int handle;
    struct nouveau_drm *drm = nouveau_drm(&dev->object);
...
drm_public int nouveau_bo_set_prime(struct nouveau_bo *bo, int *prime_fd)
{
    int ret;
    struct nouveau_drm *drm = nouveau_drm(&bo->device->object);
    struct nouveau_bo_priv *nvbo = nouveau_bo(bo);
...
drm_public int nouveau_bo_wait(struct nouveau_bo *bo, uint32_t access, struct nouveau_client *client)
{
    int ret = 0;
...
drm_public int nouveau_bo_map(struct nouveau_bo *bo, uint32_t access, struct nouveau_client *client)
{
    struct nouveau_drm *drm = nouveau_drm(&bo->device->object);
    struct nouveau_bo_priv *nvbo = nouveau_bo(bo);

    if (bo->map == NULL) {
        bo->map = drm_mmap(0, bo->size, PROT_READ | PROT_WRITE, MAP_SHARED, drm->fd, nvbo->map_handle);
