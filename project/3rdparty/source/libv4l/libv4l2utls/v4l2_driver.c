...
int v4l2_setget_std(struct v4l2_driver *drv, enum v4l2_direction dir, v4l2_std_id *id)
{
    ...
    if (dir & V4L2_SET) {
        ...
        if (ret < 0) {
            ret = errno;
            sprintf (s, "while trying to set STD to %08x", (unsigned int) *id);
            perror(s);
        }
    }
    ...
    if (dir == V4L2_SET_GET) {
        if (*id & s_id) {
            if (*id != s_id) {
                printf ("Warning: Received a std subset (%08x std) while trying to adjust to %08x\n", (unsigned int) s_id,(unsigned int) *id);
            }
        } else {
            fprintf (stderr,"Error: Received %08x std while trying to adjust to %08x\n", (unsigned int) s_id,(unsigned int) *id);
        }
    }
...
int v4l2_gettryset_fmt_cap(struct v4l2_driver *drv, enum v4l2_direction dir, struct v4l2_format *fmt,uint32_t width, uint32_t height, uint32_t pixelformat, enum v4l2_field field)
{
    ...
    if (dir & (V4L2_TRY | V4L2_SET)) {
        ...
        if (pix->pixelformat != pixelformat) {
            fprintf(stderr, "Error: asked for format %d, received %d", pixelformat, pix->pixelformat);
        }
...
int v4l2_mmap_bufs(struct v4l2_driver *drv, unsigned int num_buffers)
{
    ...
    if (drv->debug) {
        printf("REQBUFS: count=%d, type=%s,  memory=%s\n", drv->reqbuf.count, prt_names(drv->reqbuf.type, v4l2_type_names), prt_names(drv->reqbuf.memory, v4l2_memory_names));
    }
    ...
    for (drv->n_bufs = 0; drv->n_bufs < drv->reqbuf.count; drv->n_bufs++) {
        ...
        drv->bufs[drv->n_bufs].start = mmap(NULL, p->length, PROT_READ | PROT_WRITE, MAP_SHARED, drv->fd, p->m.offset);
