...
void v4l2_log_ioctl(unsigned long int request, void *arg, int result)
{
    ...
    if ((_IOC_TYPE(request) == 'V') && (_IOC_NR(request) < ARRAY_SIZE(v4l2_ioctls))) {
        ...
        snprintf(buf, sizeof(buf), "unknown request: %c %d", (int)_IOC_TYPE(request), (int)_IOC_NR(request));
        ioctl_str = buf;
    }
    ...
    switch (request) {
        ...
        case VIDIOC_TRY_FMT: {
            ...
            if (fmt->type == V4L2_BUF_TYPE_VIDEO_CAPTURE) {
                fprintf(v4l2_log_file, "  pixelformat: %c%c%c%c %ux%u\n", pixfmt & 0xff, (pixfmt >> 8) & 0xff, (pixfmt >> 16) & 0xff, pixfmt >> 24, fmt->fmt.pix.width, fmt->fmt.pix.height);
                fprintf(v4l2_log_file, "  field: %d bytesperline: %d imagesize: %d\n", (int)fmt->fmt.pix.field, (int)fmt->fmt.pix.bytesperline, (int)fmt->fmt.pix.sizeimage);
        ...
        case VIDIOC_ENUM_FRAMESIZES: {
            ...
            fprintf(v4l2_log_file, "  index: %u pixelformat: %c%c%c%c\n", frmsize->index, pixfmt & 0xff, (pixfmt >> 8) & 0xff, (pixfmt >> 16) & 0xff, pixfmt >> 24);
            switch (frmsize->type) {
        ...
        case VIDIOC_ENUM_FRAMEINTERVALS: {
            ...
            fprintf(v4l2_log_file, "  index: %u pixelformat: %c%c%c%c %ux%u:\n", frmival->index, pixfmt & 0xff, (pixfmt >> 8) & 0xff, (pixfmt >> 16) & 0xff, pixfmt >> 24, frmival->width, frmival->height);
            switch (frmival->type) {
