...
#define RGB2Y(r, g, b, y)       (y) = ((8453 * (r) + 16594 * (g) + 3223 * (b) + 524288) >> 15)
...
void v4lconvert_rgb24_to_yuv420(const unsigned char *src, unsigned char *dest, const struct v4l2_format *src_fmt, int bgr, int yvu, int bpp)
{
    ...
    for (y = 0; y < src_fmt->fmt.pix.height / 2; y++) {
        for (x = 0; x < src_fmt->fmt.pix.width / 2; x++) {
            ...
            avg_src[0] = (src[0] + src[bpp] + src[src_fmt->fmt.pix.bytesperline] + src[src_fmt->fmt.pix.bytesperline + bpp]) / 4;
            avg_src[1] = (src[1] + src[bpp + 1] + src[src_fmt->fmt.pix.bytesperline + 1] + src[src_fmt->fmt.pix.bytesperline + bpp + 1]) / 4;
            avg_src[2] = (src[2] + src[bpp + 2] + src[src_fmt->fmt.pix.bytesperline + 2] + src[src_fmt->fmt.pix.bytesperline + bpp + 2]) / 4;
...
void v4lconvert_yuv420_to_bgr24(const unsigned char *src, unsigned char *dest, int width, int height, int stride, int yvu)
{
    int i, j;
...
void v4lconvert_yuv420_to_rgb24(const unsigned char *src, unsigned char *dest, int width, int height, int stride, int yvu)
{
    int i, j;
    const unsigned char *ysrc = src;
    const unsigned char *usrc, *vsrc;
...
void v4lconvert_yuyv_to_bgr24(const unsigned char *src, unsigned char *dest, int width, int height, int stride)
{
    int j;
...
void v4lconvert_yuyv_to_rgb24(const unsigned char *src, unsigned char *dest, int width, int height, int stride)
{
    int j;
...
void v4lconvert_yuyv_to_yuv420(const unsigned char *src, unsigned char *dest, int width, int height, int stride, int yvu)
{
    int i, j;
    ...
    for (i = 0; i < height; i += 2) {
        for (j = 0; j + 1 < width; j += 2) {
            *udest++ = ((int) src[0] + src1[0]) / 2;
            *vdest++ = ((int) src[2] + src1[2]) / 2;
            src += 4;
            src1 += 4;
        }
...
void v4lconvert_yvyu_to_bgr24(const unsigned char *src, unsigned char *dest, int width, int height, int stride)
{
    int j;
...
void v4lconvert_yvyu_to_rgb24(const unsigned char *src, unsigned char *dest, int width, int height, int stride)
{
    int j;
...
void v4lconvert_uyvy_to_bgr24(const unsigned char *src, unsigned char *dest, int width, int height, int stride)
{
    int j;
...
void v4lconvert_uyvy_to_rgb24(const unsigned char *src, unsigned char *dest, int width, int height, int stride)
{
    int j;
...
void v4lconvert_uyvy_to_yuv420(const unsigned char *src, unsigned char *dest, int width, int height, int stride, int yvu)
{
    int i, j;
    const unsigned char *src1;
    unsigned char *udest, *vdest;
...
static inline void convert_packed_to_16bit(const uint8_t *raw, uint16_t *unpacked, int vw, int unpacked_len)
{
    int bitsIn = 0;
    uint32_t buffer = 0;
    int mask = (1 << vw) - 1;
