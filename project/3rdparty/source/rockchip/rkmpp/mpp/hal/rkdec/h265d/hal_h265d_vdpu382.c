...
static MPP_RET hal_h265d_vdpu382_init(void *hal, MppHalCfg *cfg)
{
...

    {
        ...
        for (i = 0; i < max_cnt; i++) {
            reg_cxt->g_buf[i].hw_regs = mpp_calloc_size(void, sizeof(Vdpu382H265dRegSet));
...
static RK_S32 hal_h265d_v382_output_pps_packet(void *hal, void *dxva)
{
    ...
    RK_S32 width, height;
    HalH265dCtx *reg_cxt = ( HalH265dCtx *)hal;
    Vdpu382H265dRegSet *hw_reg = (Vdpu382H265dRegSet*)(reg_cxt->hw_regs);
                                      ...
                                      RK_S32 width, RK_S32 height)
{
    ...
    if (reg_cxt->num_row_tiles != num_tiles ||
        ...
        reg_cxt->rcb_buf_size = vdpu382_get_rcb_buf_size((Vdpu382RcbInfo*)reg_cxt->rcb_info, width, height);
        h265d_refine_rcb_size((Vdpu382RcbInfo*)reg_cxt->rcb_info, hw_regs, width, height, dxva_cxt);
...
static MPP_RET hal_h265d_vdpu382_gen_regs(void *hal,  HalTaskInfo *syn)
{
    ...
    /* output pps */
    hw_regs = (Vdpu382H265dRegSet*)reg_cxt->hw_regs;
...

    {
        ...
        if (MPP_FRAME_FMT_IS_HDR(mpp_frame_get_fmt(mframe)) && reg_cxt->cfg->base.enable_hdr_meta)
...
static MPP_RET hal_h265d_vdpu382_start(void *hal, HalTaskInfo *task)
{
    ...
    if (reg_cxt->fast_mode) {
        ...
        hw_regs = ( Vdpu382H265dRegSet *)reg_cxt->g_buf[index].hw_regs;
    } else {
