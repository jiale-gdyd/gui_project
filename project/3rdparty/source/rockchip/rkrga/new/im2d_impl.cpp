...
IM_API static IM_STATUS rga_get_context(void)
{
    if (rgaCtx == NULL) {
        ...
        if (rgaCtx == NULL) {
            rga_error("The current RockchipRga singleton is destroyed. Please check if RkRgaInit/RkRgaDeInit are called, if so, please disable them.");
...
int rga_version_table_check_minimum_range(rga_version_t &version, const rga_version_bind_table_entry_t *table, int table_size, int index)
{
...
{
    rga_error("Failed to get the version binding table of librga, current version: librga:[%s], header:[%s]", user_version.str, header_version.str);
    return IM_STATUS_ERROR_VERSION;
}
...
{
    rga_error("Failed to get the version binding table of header file, current version: librga:[%s], header:[%s]", user_version.str, header_version.str);
    return IM_STATUS_ERROR_VERSION;
}
...
{
    rga_error("The current referenced header_version is %s, but the running librga's version(%s) is too old, "
...
{
    rga_error("The current librga.so's verison is %s, but the referenced header_version(%s) is too old, "
...
{
    rga_error("Failed to get the version binding table of librga, current version: librga:[%s], driver:[%s]", user_version.str, driver_version.str);
    return IM_STATUS_ERROR_VERSION;
}
...
{
    rga_error("Failed to get the version binding table of rga_driver, current version: librga:[%s], driver:[%s]", user_version.str, driver_version.str);
    return IM_STATUS_ERROR_VERSION;
}
...
{
    rga_error("The librga must to be updated to version %s at least. "
            "You can try to update the SDK or update librga.so and header files "
            "through github(https://github.com/airockchip/librga/). "
...
{
    rga_error("The driver may be compatible, "
...
static int rga_version_check(rga_version_t &current_version, rga_version_t &minimum_version, const rga_version_bind_table_entry_t *table, int table_size, const rga_version_check_ops_t *ops)
{
    int ret;
    ...
    if (current_bind_index < 0)
        return ops->get_current_index_failed ? ops->get_current_index_failed(current_version, minimum_version) : rga_version_get_current_index_failed_default(current_version, minimum_version);

    switch (rga_version_table_check_minimum_range(minimum_version, table, table_size, current_bind_index)) {
        case 0:
            ops->witnin_minimun_range ? ops->witnin_minimun_range(current_version, minimum_version) : rga_version_witnin_minimun_range_default(current_version, minimum_version);
            return 0;

        case -1:
            ops->below_minimun_range ? ops->below_minimun_range(current_version, minimum_version, &(table[current_bind_index])) : rga_version_below_minimun_range_default(current_version, minimum_version, &(table[current_bind_index]));
            return -1;

        case 1:
            least_index = rga_version_table_get_minimum_index(minimum_version, table, table_size);
            if (least_index < 0) {
                ops->get_minimum_index_failed ? ops->get_minimum_index_failed(current_version, minimum_version) : rga_version_get_minimum_index_failed_default(current_version, minimum_version);
                return 1;
            }

            ops->above_minimun_range ? ops->above_minimun_range(current_version, minimum_version, &(table[least_index])) : rga_version_above_minimun_range_default(current_version, minimum_version, &(table[least_index]));
            return 1;

        default:
            rga_error("This shouldn't happen!");
            return IM_STATUS_FAILED;
    }
}
...
static IM_STATUS rga_yuv_legality_check(const char *name, rga_buffer_t info, im_rect rect)
{
    ...
    {
        rga_warn("%s, Error yuv not align to 2, rect:[x,y,w,h] = [%d, %d, %d, %d], wstride:[%d], hstride:[%d], format:[0x%X(%s)]",
            name, rect.x, rect.y, info.width, info.height, info.wstride, info.hstride, info.format, translate_format_str(info.format));
...
{
    if (NULL == dstinfo) {
        rga_error("Invaild dstinfo, dst structure address is NULL!");
        return IM_STATUS_INVALID_PARAM;
    }
    ...
    if (dst.handle > 0) {
        ...
        rga_error("Invaild dst buffer, no address available in dst buffer, phy_addr:[%ld], fd:[%d], vir_addr:[%ld], handle:[%d]", (unsigned long)dst.phy_addr, dst.fd, (unsigned long)dst.vir_addr, dst.handle);
        return IM_STATUS_INVALID_PARAM;
    }
...
{
    if (NULL == srcinfo) {
        rga_error("Invaild srcinfo, src structure address is NULL.");
        return IM_STATUS_INVALID_PARAM;
    }
    ...
    if (src.handle > 0) {
        ...
        rga_error("Invaild src buffer, no address available in src buffer, phy_addr:[%ld], fd:[%d], vir_addr:[%ld], handle:[%d]", (unsigned long)src.phy_addr, src.fd, (unsigned long)src.vir_addr, src.handle);
        return IM_STATUS_INVALID_PARAM;
    }
...
TRY_TO_COMPATIBLE:
    ...
    if (rga_version == IM_RGA_HW_VERSION_RGA_V_ERR_INDEX) {
        rga_error("Can not get the correct RGA version, please check the driver, version:[%s]", rgaCtx->mHwVersions.version[0].str);
        return IM_STATUS_FAILED;
    }
...
IM_STATUS rga_check_header(rga_version_t header_version)
{
    ...
    ret = rga_version_check(user_version, header_version, user_header_bind_table, table_size, &rga_version_check_user_header_ops);
    switch (ret) {
        case 0:
            return IM_STATUS_SUCCESS;
...
IM_STATUS rga_check_driver(rga_version_t &driver_version)
{
    ...
    ret =  rga_version_check(user_version, driver_version, user_driver_bind_table, table_size, &rga_version_check_user_driver_ops);
    switch (ret) {
        case 0:
        case -1:
            return IM_STATUS_SUCCESS;
...
IM_STATUS rga_check_info(const char *name, const rga_buffer_t info, const im_rect rect, int resolution_usage)
{
    if ((info.width <= 0) || (info.height <= 0) || (info.format < 0)) {
        rga_warn("Illegal %s, the parameter cannot be negative or 0, width:[%d], height:[%d], format:[0x%X(%s)]", name, info.width, info.height, info.format, translate_format_str(info.format));
        return IM_STATUS_ILLEGAL_PARAM;
    }

    if ((info.width < 2) || (info.height < 2)) {
        rga_warn("Hardware limitation %s, unsupported operation of images smaller than 2 pixels, width:[%d], height:[%d]", name, info.width, info.height);
        return IM_STATUS_ILLEGAL_PARAM;
    }

    if ((info.wstride < info.width) || (info.hstride < info.height)) {
        rga_warn("Invaild %s, Virtual width or height is less than actual width and height, wstride:[%d], width:[%d], hstride:[%d], height:[%d]", name, info.wstride, info.width, info.hstride, info.height);
        return IM_STATUS_INVALID_PARAM;
    }

    if ((rect.width < 0) || (rect.height < 0) || (rect.x < 0) || (rect.y < 0)) {
        rga_warn("Illegal %s rect, the parameter cannot be negative, rect:[x,y,w,h] = [%d, %d, %d, %d]", name, rect.x, rect.y, rect.width, rect.height);
        return IM_STATUS_ILLEGAL_PARAM;
    }

    if (((rect.width > 0) && (rect.width < 2)) || ((rect.height > 0) && (rect.height < 2)) || ((rect.x > 0) && (rect.x < 2)) || ((rect.y > 0) && (rect.y < 2))) {
        rga_warn("Hardware limitation %s rect, unsupported operation of images smaller than 2 pixels, rect:[x,y,w,h] = [%d, %d, %d, %d]", name, rect.x, rect.y, rect.width, rect.height);
        return IM_STATUS_INVALID_PARAM;
    }

    if ((rect.width + rect.x > info.wstride) || (rect.height + rect.y > info.hstride)) {
        rga_warn("Invaild %s rect, the sum of width and height of rect needs to be less than wstride or hstride, rect:[x,y,w,h] = [%d, %d, %d, %d], wstride:[%d], hstride:[%d]", name, rect.x, rect.y, rect.width, rect.height, info.wstride, info.hstride);
        return IM_STATUS_INVALID_PARAM;
    }

    if ((info.width > resolution_usage) || (info.height > resolution_usage)) {
        rga_warn("Unsupported %s to input resolution more than %d, width:[%d], height:[%d]", name, resolution_usage, info.width, info.height);
        return IM_STATUS_NOT_SUPPORTED;
    } else if (((rect.width > 0) && (rect.width > resolution_usage)) || ((rect.height > 0) && (rect.height > resolution_usage))) {
        rga_warn("Unsupported %s rect to output resolution more than %d, rect:[x,y,w,h] = [%d, %d, %d, %d]", name, resolution_usage, rect.x, rect.y, rect.width, rect.height);
        return IM_STATUS_NOT_SUPPORTED;
    }
...
IM_STATUS rga_check_limit(rga_buffer_t src, rga_buffer_t dst, int scale_usage, int mode_usage)
{
    ...
    if (((src_width >> (int)(log(scale_usage)/log(2))) > dst_width) || ((src_height >> (int)(log(scale_usage)/log(2))) > dst_height)) {
        rga_warn("Unsupported to scaling less than 1/%d ~ %d times, src:[w,h] = [%d, %d], dst:[w,h] = [%d, %d]", scale_usage, scale_usage, src.width, src.height, dst.width, dst.height);
        return IM_STATUS_NOT_SUPPORTED;
    }

    if (((dst_width >> (int)(log(scale_usage)/log(2))) > src_width) || ((dst_height >> (int)(log(scale_usage)/log(2))) > src_height)) {
        rga_warn("Unsupported to scaling more than 1/%d ~ %d times, src:[w,h] = [%d, %d], dst:[w,h] = [%d, %d]", scale_usage, scale_usage, src.width, src.height, dst.width, dst.height);
        return IM_STATUS_NOT_SUPPORTED;
    }
...
IM_STATUS rga_check_format(const char *name, rga_buffer_t info, im_rect rect, int format_usage, int mode_usgae)
{
        ...
        || format == RK_FORMAT_BGR_565)
    {
        if (~format_usage & IM_RGA_SUPPORT_FORMAT_RGB) {
            rga_warn("%s unsupported RGB format, format:[0x%X(%s)], str:[%s]", name, info.format, translate_format_str(info.format), querystring((strcmp("dst", name) == 0) ? RGA_OUTPUT_FORMAT : RGA_INPUT_FORMAT));
            return IM_STATUS_NOT_SUPPORTED;
        }
        ...
        || format == RK_FORMAT_ABGR_5551)
    {
        if (~format_usage & IM_RGA_SUPPORT_FORMAT_RGB_OTHER) {
            rga_warn("%s unsupported RGBA 4444/5551 format, format:[0x%X(%s)], str:[%s]", name, info.format, translate_format_str(info.format), querystring((strcmp("dst", name) == 0) ? RGA_OUTPUT_FORMAT : RGA_INPUT_FORMAT));
            return IM_STATUS_NOT_SUPPORTED;
        }
        ...
        || format == RK_FORMAT_BPP8)
    {
        if ((~format_usage & IM_RGA_SUPPORT_FORMAT_BPP) && !(mode_usgae & IM_COLOR_PALETTE)) {
            rga_warn("%s unsupported BPP format, format:[0x%X(%s)], str:[%s]", name, info.format, translate_format_str(info.format), querystring((strcmp("dst", name) == 0) ? RGA_OUTPUT_FORMAT : RGA_INPUT_FORMAT));
            return IM_STATUS_NOT_SUPPORTED;
        }
        ...
        if (~format_usage & IM_RGA_SUPPORT_FORMAT_YUV_420_SEMI_PLANNER_8_BIT) {
            rga_warn("%s unsupported YUV420 semi-planner 8bit format, format:[0x%X(%s)], str:[%s]", name, info.format, translate_format_str(info.format), querystring((strcmp("dst", name) == 0) ? RGA_OUTPUT_FORMAT : RGA_INPUT_FORMAT));
            return IM_STATUS_NOT_SUPPORTED;
        }
        ...
        if (~format_usage & IM_RGA_SUPPORT_FORMAT_YUV_420_PLANNER_8_BIT) {
            rga_warn("%s unsupported YUV420 planner 8bit format, format:[0x%X(%s)], str:[%s]", name, info.format, translate_format_str(info.format), querystring((strcmp("dst", name) == 0) ? RGA_OUTPUT_FORMAT : RGA_INPUT_FORMAT));
            return IM_STATUS_NOT_SUPPORTED;
        }
        ...
        if (format_usage & IM_RGA_SUPPORT_FORMAT_YUV_422_SEMI_PLANNER_8_BIT) {
            rga_warn("%s unsupported YUV422 semi-planner 8bit format, format:[0x%X(%s)], str:[%s]", name, info.format, translate_format_str(info.format), querystring((strcmp("dst", name) == 0) ? RGA_OUTPUT_FORMAT : RGA_INPUT_FORMAT));
            return IM_STATUS_NOT_SUPPORTED;
        }
        ...
        if (format_usage & IM_RGA_SUPPORT_FORMAT_YUV_422_PLANNER_8_BIT) {
            rga_warn("%s unsupported YUV422 planner 8bit format, format:[0x%X(%s)], str:[%s]", name, info.format, translate_format_str(info.format), querystring((strcmp("dst", name) == 0) ? RGA_OUTPUT_FORMAT : RGA_INPUT_FORMAT));
            return IM_STATUS_NOT_SUPPORTED;
        }
        ...
        if (format_usage & IM_RGA_SUPPORT_FORMAT_YUV_420_SEMI_PLANNER_10_BIT) {
            rga_warn("%s unsupported YUV420 semi-planner 10bit format, format:[0x%X(%s)], str:[%s]", name, info.format, translate_format_str(info.format), querystring((strcmp("dst", name) == 0) ? RGA_OUTPUT_FORMAT : RGA_INPUT_FORMAT));
            return IM_STATUS_NOT_SUPPORTED;
        }
        ...
        if (format_usage & IM_RGA_SUPPORT_FORMAT_YUV_422_SEMI_PLANNER_10_BIT) {
            rga_warn("%s unsupported YUV422 semi-planner 10bit format, format:[0x%X(%s)], str:[%s]", name, info.format, translate_format_str(info.format), querystring((strcmp("dst", name) == 0) ? RGA_OUTPUT_FORMAT : RGA_INPUT_FORMAT));
            return IM_STATUS_NOT_SUPPORTED;
        }
        ...
        || format == RK_FORMAT_VYUY_420)
    {
        if (~format_usage & IM_RGA_SUPPORT_FORMAT_YUYV_420) {
            rga_warn("%s unsupported YUYV format, format:[0x%X(%s)], str:[%s]", name, info.format, translate_format_str(info.format), querystring((strcmp("dst", name) == 0) ? RGA_OUTPUT_FORMAT : RGA_INPUT_FORMAT));
            return IM_STATUS_NOT_SUPPORTED;
        }
        ...
        || format == RK_FORMAT_VYUY_422)
    {
        ...
        if (format_usage & IM_RGA_SUPPORT_FORMAT_YUV_400) {
            rga_warn("%s unsupported YUV400 format, format:[0x%X(%s)], str:[%s]", name, info.format, translate_format_str(info.format), querystring((strcmp("dst", name) == 0) ? RGA_OUTPUT_FORMAT : RGA_INPUT_FORMAT));
            return IM_STATUS_NOT_SUPPORTED;
        }
        ...
        if (format_usage & IM_RGA_SUPPORT_FORMAT_Y4) {
            rga_warn("%s unsupported Y4/Y1 format, format:[0x%X(%s)], str:[%s]", name, info.format, translate_format_str(info.format), querystring((strcmp("dst", name) == 0) ? RGA_OUTPUT_FORMAT : RGA_INPUT_FORMAT));
            return IM_STATUS_NOT_SUPPORTED;
        }
        ...
        if (format_usage & IM_RGA_SUPPORT_FORMAT_RGBA2BPP) {
            rga_warn("%s unsupported rgba2bpp format, format:[0x%X(%s)], str:[%s]", name, info.format, translate_format_str(info.format), querystring((strcmp("dst", name) == 0) ? RGA_OUTPUT_FORMAT : RGA_INPUT_FORMAT));
            return IM_STATUS_NOT_SUPPORTED;
        }
    } else {
        rga_warn("%s unsupported this format, format:[0x%X(%s)], str:[%s]", name, info.format, translate_format_str(info.format), querystring((strcmp("dst", name) == 0) ? RGA_OUTPUT_FORMAT : RGA_INPUT_FORMAT));
        return IM_STATUS_NOT_SUPPORTED;
    }
...
IM_STATUS rga_check_align(const char *name, rga_buffer_t info, int byte_stride)
{
    ...
    if (bit_stride % (byte_stride * 8) == 0) {
        ...
        rga_warn("%s unsupport width stride %d, %s width stride should be %d aligned", name, info.wstride, translate_format_str(info.format), align);
        return IM_STATUS_NOT_SUPPORTED;
    }
...
IM_STATUS rga_check_blend(rga_buffer_t src, rga_buffer_t pat, rga_buffer_t dst, int pat_enable, int mode_usage)
{
    ...
    switch (mode_usage & IM_ALPHA_BLEND_MASK) {
        ...
        case IM_ALPHA_BLEND_SRC_OVER:
            if (!NormalRgaFormatHasAlpha(src_fmt)) {
                rga_warn("Blend mode 'src_over' unsupported src format without alpha, format:[src,src1,dst] = [0x%x(%s), 0x%x(%s), 0x%x(%s)]",
                    src_fmt, translate_format_str(src_fmt), pat_fmt, translate_format_str(pat_fmt), dst_fmt, translate_format_str(dst_fmt));
                return IM_STATUS_NOT_SUPPORTED;
            }
            break;
        ...
        case IM_ALPHA_BLEND_DST_OVER:
            if (pat_enable) {
                if (!NormalRgaFormatHasAlpha(pat_fmt)) {
                    rga_warn("Blend mode 'dst_over' unsupported pat format without alpha, format:[src,src1,dst] = [0x%x(%s), 0x%x(%s), 0x%x(%s)]",
                        src_fmt, translate_format_str(src_fmt), pat_fmt, translate_format_str(pat_fmt), dst_fmt, translate_format_str(dst_fmt));
                    return IM_STATUS_NOT_SUPPORTED;
                }
            } else {
                if (!NormalRgaFormatHasAlpha(dst_fmt)) {
                    rga_warn("Blend mode 'dst_over' unsupported dst format without alpha, format:[src,src1,dst] = [0x%x(%s), 0x%x(%s), 0x%x(%s)]",
                        src_fmt, translate_format_str(src_fmt), pat_fmt, translate_format_str(pat_fmt), dst_fmt, translate_format_str(dst_fmt));
                    return IM_STATUS_NOT_SUPPORTED;
                }
            }
        ...
        default:
            if (!(NormalRgaFormatHasAlpha(src_fmt) || NormalRgaFormatHasAlpha(dst_fmt))) {
                rga_warn("Blend mode unsupported format without alpha, format:[src,src1,dst] = [0x%x(%s), 0x%x(%s), 0x%x(%s)]",
                    src_fmt, translate_format_str(src_fmt), pat_fmt, translate_format_str(pat_fmt), dst_fmt, translate_format_str(dst_fmt));
                return IM_STATUS_NOT_SUPPORTED;
            }
            break;
    ...
    if (pat_enable && ((pat.width != dst.width) || (pat.height != dst.height))) {
        rga_warn("In the three-channel mode Alapha blend, the width and height of the src1 channel must be equal to the dst channel, src1:[w,h] = [%d, %d], dst:[w,h] = [%d, %d]", pat.width, pat.height, dst.width, dst.height);
        return IM_STATUS_NOT_SUPPORTED;
    }
...
IM_STATUS rga_check_rotate(int mode_usage, rga_info_table_entry &table)
{
    if (table.version & (IM_RGA_HW_VERSION_RGA_1 | IM_RGA_HW_VERSION_RGA_1_PLUS)) {
        if (mode_usage & IM_HAL_TRANSFORM_FLIP_H_V) {
            rga_warn("RGA1/RGA1_PLUS cannot support H_V mirror.");
            return IM_STATUS_NOT_SUPPORTED;
        }
        ...
        {
            rga_warn("RGA1/RGA1_PLUS cannot support rotate with mirror.");
            return IM_STATUS_NOT_SUPPORTED;
        }
    }
...
IM_STATUS rga_check_feature(rga_buffer_t src, rga_buffer_t pat, rga_buffer_t dst, int pat_enable, int mode_usage, int feature_usage)
{
    if ((mode_usage & IM_COLOR_FILL) && (feature_usage & IM_RGA_SUPPORT_FEATURE_COLOR_FILL)) {
        rga_warn("The platform does not support color fill featrue. %s", querystring(RGA_FEATURE));
        return IM_STATUS_NOT_SUPPORTED;
    }

    if ((mode_usage & IM_COLOR_PALETTE) && (feature_usage & IM_RGA_SUPPORT_FEATURE_COLOR_PALETTE)) {
        rga_warn("The platform does not support color palette featrue. %s", querystring(RGA_FEATURE));
        return IM_STATUS_NOT_SUPPORTED;
    }

    if ((mode_usage & IM_ROP) && (feature_usage & IM_RGA_SUPPORT_FEATURE_ROP)) {
        rga_warn("The platform does not support ROP featrue. %s", querystring(RGA_FEATURE));
        return IM_STATUS_NOT_SUPPORTED;
    }

    if ((mode_usage & IM_NN_QUANTIZE) && (feature_usage & IM_RGA_SUPPORT_FEATURE_QUANTIZE)) {
        rga_warn("The platform does not support quantize featrue. %s", querystring(RGA_FEATURE));
        return IM_STATUS_NOT_SUPPORTED;
    }

    if ((pat_enable ? (pat.color_space_mode & IM_RGB_TO_YUV_MASK) : 0) && (feature_usage & IM_RGA_SUPPORT_FEATURE_SRC1_R2Y_CSC)) {
        rga_warn("The platform does not support src1 channel RGB2YUV color space convert featrue. %s", querystring(RGA_FEATURE));
        return IM_STATUS_NOT_SUPPORTED;
    }
    ...
    {
        rga_warn("The platform does not support dst channel full color space convert(Y2Y/Y2R) featrue. %s", querystring(RGA_FEATURE));
        return IM_STATUS_NOT_SUPPORTED;
    }

    if ((mode_usage & IM_MOSAIC) && (feature_usage & IM_RGA_SUPPORT_FEATURE_MOSAIC)) {
        rga_warn("The platform does not support mosaic featrue. %s", querystring(RGA_FEATURE));
        return IM_STATUS_NOT_SUPPORTED;
    }

    if ((mode_usage & IM_OSD) && (feature_usage & IM_RGA_SUPPORT_FEATURE_OSD)) {
        rga_warn("The platform does not support osd featrue. %s", querystring(RGA_FEATURE));
        return IM_STATUS_NOT_SUPPORTED;
    }

    if ((mode_usage & IM_PRE_INTR) && (feature_usage & IM_RGA_SUPPORT_FEATURE_PRE_INTR)) {
        rga_warn("The platform does not support pre_intr featrue. %s", querystring(RGA_FEATURE));
        return IM_STATUS_NOT_SUPPORTED;
    }
...
IM_STATUS rga_check(const rga_buffer_t src, const rga_buffer_t dst, const rga_buffer_t pat, const im_rect src_rect, const im_rect dst_rect, const im_rect pat_rect, int mode_usage)
{
    bool pat_enable = 0;
    ...
    if (pat_enable) {
        if (rga_info.version & (IM_RGA_HW_VERSION_RGA_1 | IM_RGA_HW_VERSION_RGA_1_PLUS)) {
            rga_warn("RGA1/RGA1_PLUS cannot support src1.");
            return IM_STATUS_NOT_SUPPORTED;
        }
...
IM_STATUS rga_check_external(rga_buffer_t src, rga_buffer_t dst, rga_buffer_t pat, im_rect src_rect, im_rect dst_rect, im_rect pat_rect, int mode_usage)
{
    int ret;
    int format;
    ...
    if (format == RK_FORMAT_UNKNOWN) {
        rga_warn("Invaild src format:[0x%X]", src.format);
        return IM_STATUS_NOT_SUPPORTED;
    }
    src.format = format;
    ...
    if (format == RK_FORMAT_UNKNOWN) {
        rga_warn("Invaild dst format:[0x%X]", dst.format);
        return IM_STATUS_NOT_SUPPORTED;
    }
    dst.format = format;
    ...
    if (rga_is_buffer_valid(pat)) {
        ...
        if (format == RK_FORMAT_UNKNOWN) {
            rga_warn("Invaild pat format:[0x%X]", pat.format);
            return IM_STATUS_NOT_SUPPORTED;
        }
        pat.format = format;
    }
...
IM_API IM_STATUS rga_import_buffers(struct rga_buffer_pool *buffer_pool)
{
    ...
    if (buffer_pool == NULL) {
        rga_warn("buffer pool is null!");
        return IM_STATUS_FAILED;
    }

    ret = ioctl(rgaCtx->rgaFd, RGA_IOC_IMPORT_BUFFER, buffer_pool);
    if (ret < 0) {
        rga_warn("RGA_IOC_IMPORT_BUFFER fail! %s", strerror(errno));
        return IM_STATUS_FAILED;
    }
...
IM_API rga_buffer_handle_t rga_import_buffer(uint64_t memory, int type, im_handle_param_t *param)
{
    ...
    if (format == RK_FORMAT_UNKNOWN) {
        rga_warn("Invaild format:[0x%X]", buffers[0].memory_info.format);
        return IM_STATUS_NOT_SUPPORTED;
    }
...
IM_API IM_STATUS rga_release_buffers(struct rga_buffer_pool *buffer_pool)
{
    ...
    if (buffer_pool == NULL) {
        rga_error("buffer pool is null!");
        return IM_STATUS_FAILED;
    }

    ret = ioctl(rgaCtx->rgaFd, RGA_IOC_RELEASE_BUFFER, buffer_pool);
    if (ret < 0) {
        rga_error("RGA_IOC_RELEASE_BUFFER fail! %s", strerror(errno));
        return IM_STATUS_FAILED;
    }
...
static void rga_dump_opt(int log_level, im_opt_t &opt, int usage)
{
    ...
    if (usage & IM_ALPHA_COLORKEY_MASK) {
        IM_LOG(log_level, "color_key[min,max] = [0x%x, 0x%x] ", opt.colorkey_range.min, opt.colorkey_range.max);
    }
...
static IM_STATUS rga_task_submit(im_job_handle_t job_handle, rga_buffer_t src, rga_buffer_t dst, rga_buffer_t pat, im_rect srect, im_rect drect, im_rect prect, int acquire_fence_fd, int *release_fence_fd, im_opt_t *opt_ptr, int usage)
{
    int ret;
    int format;
    im_opt_t opt;
    ...
    if ((dst.color_space_mode & IM_YUV_TO_RGB_MASK) && (dst.color_space_mode & IM_RGB_TO_YUV_MASK)) {
            ...
            && NormalRgaIsYuvFormat(dst.format))
        {
            ...
            rga_warn("Not yuv + rgb -> yuv does not need for color_sapce_mode R2Y & Y2R, please fix, src_fromat:[0x%x(%s)], src1_format:[0x%X(%s)], dst_format:[0x%X(%s)]",
                src.format, translate_format_str(src.format), pat.format, translate_format_str(pat.format), dst.format, translate_format_str(dst.format));
            return IM_STATUS_ILLEGAL_PARAM;
        }
        ...
        {
            dstinfo.color_space_mode = dst.color_space_mode;
        } else if (NormalRgaIsYuvFormat(src.format) && NormalRgaIsRgbFormat(dst.format)) {
            dstinfo.color_space_mode = dst.color_space_mode;
        } else {
            rga_warn("Not yuv to rgb does not need for color_sapce_mode, please fix, src_fromat:0x%X(%s)], src1_format:[0x%X(%s)], dst_format:[0x%X(%s)]",
                src.format, translate_format_str(src.format), pat.format, rga_is_buffer_valid(pat) ? translate_format_str(pat.format) : "none", dst.format, translate_format_str(dst.format));
            return IM_STATUS_ILLEGAL_PARAM;
        }
        ...
        {
            dstinfo.color_space_mode = dst.color_space_mode;
        } else if (NormalRgaIsRgbFormat(src.format) && NormalRgaIsYuvFormat(dst.format)) {
            dstinfo.color_space_mode = dst.color_space_mode;
        } else {
            rga_warn("Not rgb to yuv does not need for color_sapce_mode, please fix, src_fromat:0x%X(%s)], src1_format:[0x%X(%s)], dst_format:[0x%X(%s)]",
                src.format, translate_format_str(src.format), pat.format, rga_is_buffer_valid(pat) ? translate_format_str(pat.format) : "none", dst.format, translate_format_str(dst.format));
            return IM_STATUS_ILLEGAL_PARAM;
        }
        ...
        if ((src.color_space_mode == IM_RGB_FULL) && (dst.color_space_mode == IM_YUV_BT709_FULL_RANGE)) {
            ...
            rga_warn("Unsupported full csc mode! src_csm:[0x%X], dst_csm:[0x%X]", src.color_space_mode, dst.color_space_mode);
            return IM_STATUS_NOT_SUPPORTED;
        }
    }
    ...
    if (usage & IM_ASYNC) {
        if (release_fence_fd == NULL) {
            rga_warn("Async mode release_fence_fd cannot be NULL!");
            return IM_STATUS_ILLEGAL_PARAM;
        }
    ...
    if (ret) {
        rga_error("Failed to call RockChipRga interface, please use 'dmesg' command to view driver error log.");
        ...
        rga_error("acquir_fence[%d], release_fence_ptr[0x%lX], usage[0x%X]", acquire_fence_fd, (unsigned long)release_fence_fd, usage);
        return IM_STATUS_FAILED;
    }
...
IM_STATUS rga_task_submit(im_job_handle_t job_handle, rga_buffer_t src, rga_buffer_t dst, rga_buffer_t pat, im_rect srect, im_rect drect, im_rect prect, im_opt_t *opt_ptr, int usage)
    {
...
im_job_handle_t rga_job_create(uint32_t flags)
{
    ...
    if (g_im2d_job_manager.job_map.count(job_handle) != 0) {
        rga_error("job_map error! handle:[%d] already exists:[%lu]", job_handle, (unsigned long)g_im2d_job_manager.job_map.count(job_handle));
...
IM_STATUS rga_job_submit(im_job_handle_t job_handle, int sync_mode, int acquire_fence_fd, int *release_fence_fd)
{
    ...
    g_im2d_job_manager.mutex.lock();
    if (g_im2d_job_manager.job_map.count(job_handle) == 0) {
        rga_error("job_handle:[%d] is illegal", job_handle);
        g_im2d_job_manager.mutex.unlock();
