...
static rga_buffer_t wrapbuffer_physicaladdr(void *phy_addr, int width, int height, int format, int wstride, int hstride)
{
    rga_buffer_t buffer;
...
#undef wrapbuffer_fd
static rga_buffer_t wrapbuffer_fd(int fd, int width, int height, int format, int wstride, int hstride)
{
    rga_buffer_t buffer;
...
IM_API IM_STATUS imsync(int fence_fd)
{
    ...
    if (fence_fd <= 0) {
        rga_error("illegal fence_fd:[%d]", fence_fd);
    ...
    switch (name) {
        case IM_CONFIG_SCHEDULER_CORE:
            if (value & IM_SCHEDULER_MASK) {
                ...
                rga_error("IM2D: It's not legal rga_core:[0x%lX], it needs to be a 'IM_SCHEDULER_CORE'.", (unsigned long)value);
                return IM_STATUS_ILLEGAL_PARAM;
            }
            break;
        ...
        case IM_CONFIG_PRIORITY:
            if (value <= 6) {
                ...
                rga_error("IM2D: It's not legal priority:[0x%lX], it needs to be a 'int', and it should be in the range of 0~6.", (unsigned long)value);
                return IM_STATUS_ILLEGAL_PARAM;
            }
            break;
        ...
        case IM_CONFIG_CHECK:
            if ((value == false) || (value == true)) {
                ...
                rga_error("IM2D: It's not legal check config:[0x%lX], it needs to be a 'bool'.", (unsigned long)value);
                return IM_STATUS_ILLEGAL_PARAM;
            }
            break;
...
IM_API IM_STATUS imcopy(const rga_buffer_t src, rga_buffer_t dst, int sync, int *release_fence_fd)
{
    ...
    if ((src.width != dst.width) || (src.height != dst.height)) {
        rga_warn("imcopy cannot support scale, src:[w,h] = [%d, %d], dst:[w,h] = [%d, %d]", src.width, src.height, dst.width, dst.height);
...
IM_API IM_STATUS imresize(const rga_buffer_t src, rga_buffer_t dst, double fx, double fy, int interpolation, int sync, int *release_fence_fd)
{
    ...
    if ((fx > 0) || (fy > 0)) {
        ...
        if (format == RK_FORMAT_UNKNOWN) {
            rga_error("Invaild dst format:[0x%X]", dst.format);
            return IM_STATUS_NOT_SUPPORTED;
        }
...
IM_API IM_STATUS imtranslate(const rga_buffer_t src, rga_buffer_t dst, int x, int y, int sync, int *release_fence_fd)
{
    ...
    if ((src.width != dst.width) || (src.height != dst.height)) {
        rga_warn("The width and height of src and dst need to be equal, src:[w,h] = [%d, %d], dst:[w,h] = [%d, %d]", src.width, src.height, dst.width, dst.height);
        return IM_STATUS_INVALID_PARAM;
    }
...
IM_API IM_STATUS imosd(const rga_buffer_t osd,const rga_buffer_t dst, const im_rect osd_rect, im_osd_t *osd_info, int sync, int *release_fence_fd)
{
    im_opt_t opt;
    int usage = 0;
    im_rect tmp_rect;
...
IM_STATUS immakeBorder(rga_buffer_t src, rga_buffer_t dst, int top, int bottom, int left, int right, int border_type, int value, int sync, int acquir_fence_fd, int *release_fence_fd)
{
    bool reflect;
    IM_STATUS ret;
    ...
    if (((src.width + left + right) != dst.width) || ((src.height + top + bottom) != dst.height)) {
        rga_warn("The width/height of dst must be equal to the width/height after making the border! src:[w,h] = [%d, %d], dst:[w,h] = [%d, %d], [t,b,l,r] = [%d, %d, %d, %d]",
    ...
    if (border_type == IM_BORDER_CONSTANT) {
        ...
        switch (border_type) {
            ...
            default:
                rga_warn("unknown border type:[0x%X]", border_type);
...
IM_STATUS improcess_ctx(rga_buffer_t src, rga_buffer_t dst, rga_buffer_t pat, im_rect srect, im_rect drect, im_rect prect, int acquire_fence_fd, int *release_fence_fd, im_opt_t *opt_ptr, int usage, im_ctx_id_t ctx_id)
{
...
IM_API IM_STATUS imcheck_t(rga_buffer_t src, rga_buffer_t dst, rga_buffer_t pat, im_rect src_rect, im_rect dst_rect, im_rect pat_rect, int mode_usage)
{
...
IM_API IM_STATUS imosd(const rga_buffer_t osd,const rga_buffer_t dst, const im_rect osd_rect, im_osd_t *osd_info, int sync)
{
