...
int checkRectForRga(rga_rect_t rect)
{
    if ((rect.xoffset < 0) || (rect.yoffset < 0)) {
        rga_error("Error offset XxY:[%d,%d]", rect.xoffset, rect.yoffset);
        return -EINVAL;
    }

    if ((rect.width < 2) || (rect.height < 2)) {
        rga_error("Error act WxH:[%d,%d]", rect.width, rect.height);
        return -EINVAL;
    }

    if ((rect.xoffset + rect.width) > rect.wstride) {
        rga_error("Error XxWxWS:[%d,%d,%d]", rect.xoffset, rect.width, rect.wstride);
        return -EINVAL;
    }

    if ((rect.yoffset + rect.height) > rect.hstride) {
        rga_error("Error YxHxHS:[%d,%d,%d]", rect.yoffset, rect.height, rect.hstride);
        return -EINVAL;
    }
...
{
    rga_info("render_mode:[%d] rotate_mode:[%d] in_fence_fd:[%d] handle_flag:[0x%X]", rgaReg.render_mode, rgaReg.rotate_mode, rgaReg.in_fence_fd, rgaReg.handle_flag);
    ...
    rga_info("ROP:[%lx,%x,%x], LUT:[%lx]", (unsigned long)rgaReg.rop_mask_addr, rgaReg.alpha_rop_flag, rgaReg.rop_code, (unsigned long)rgaReg.LUT_addr);

    rga_info("color:[%x,%x,%x,%x,%x]", rgaReg.color_key_max, rgaReg.color_key_min, rgaReg.fg_color, rgaReg.bg_color, rgaReg.color_fill_mode);
    rga_info("MMU:[%d,%lx,%x]", rgaReg.mmu_info.mmu_en, (unsigned long)rgaReg.mmu_info.base_addr, rgaReg.mmu_info.mmu_flag);

    rga_info("mode[%d,%d,%d,%d,%d]", rgaReg.palette_mode, rgaReg.yuv2rgb_mode, rgaReg.endian_mode, rgaReg.src_trans_mode,rgaReg.scale_mode);
...
static inline void NormalRgaCompatModeConvertRga2Mmu(rga2_mmu_t *mmu, MMU *orig_mmu)
{
    ...
    mmu->base_addr = (arch_type_t)orig_mmu->base_addr;
...
void NormalRgaCompatModeConvertRga2(rga2_req *req, rga_req *orig_req)
{
    ...
    req->rop_mask_addr = (arch_type_t)orig_req->rop_mask_addr;
    req->LUT_addr = (arch_type_t)orig_req->LUT_addr;
