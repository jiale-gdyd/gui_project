...
{
    int sock = rtsp::setupStreamSock(port, makeNonBlocking);
    if (sock > 0) {
        fSock = sock;
        fPort = port;
    }
...
{
    int sock = rtsp::setupDatagramSock(port, makeNonBlocking);
    if (sock > 0) {
        fSock = sock;
        fPort = port;
    }
...
{
    int sock = rtsp::setupServerSock(port, makeNonBlocking);
    if (sock > 0) {
        fSock = sock;
        fPort = port;
    }
...
{
    int sock = rtsp::setupClientSock(serverSock, makeNonBlocking, fClientAddr);
    if (sock > 0) {
        fSock = sock;
...
{
    MUTEX_LOCK(&fMutex);
    int err = rtsp::writeSocket(fSock, buffer, bufferSize); 
    MUTEX_UNLOCK(&fMutex);
...
{
    MUTEX_LOCK(&fMutex);
    int err = rtsp::writeSocket(fSock, buffer, bufferSize, toAddress);
    MUTEX_UNLOCK(&fMutex);
...
{
    MUTEX_LOCK(&fMutex);
    int err = rtsp::sendRTPOverTCP(fSock, buffer, len, streamChannelId);
    MUTEX_UNLOCK(&fMutex);
