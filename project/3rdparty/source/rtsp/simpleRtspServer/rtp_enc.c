...
int rtp_enc_h265(rtp_enc_t *e, const uint8_t *frame, int len, uint64_t ts, uint8_t *packets[], int pktsizs[])
{
    ...
    while ((len > 0) && packets[count] && (pktsizs[count] > RTPHDR_SIZE)) {
        ...
        if ((count == 0) && (len <= (pktsiz - RTPHDR_SIZE))) {
            ...
            packets[count][RTPHDR_SIZE + 0] = (nalhdr[0] & 0x81) | (49 << 1);
            packets[count][RTPHDR_SIZE + 1] = (nalhdr[1]);
            packets[count][RTPHDR_SIZE + 2] = ((nalhdr[0] >> 1) & 0x3f);
            if (count == 0) {
                packets[count][RTPHDR_SIZE + 2] |= 0x80;
            }

            if (mark) {
                packets[count][RTPHDR_SIZE + 2] |= 0x40;
