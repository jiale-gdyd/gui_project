...
static int rtsp_msg_parse_uri(const char *line, rtsp_msg_uri_s *uri)
{
    ...
    if (uri->scheme == RTSP_MSG_URI_SCHEME_BUTT) {
        simple_rtsp_server_error("parse scheme failed, line:[%s]", line);
        return -1;
    }
    uri->port = 0;
    uri->ipaddr[0] = 0;
    ...
    if ((*p != '/') || (*(p + 1) != '/')) {
        simple_rtsp_server_error("parse ip failed, line:[%s]", line);
        return -1;
    }
    p += 2;
    ...
    if (*q == ':') {
        if (sscanf(q + 1, "%u", &tmp) != 1) {
            simple_rtsp_server_error("parse uri port failed, line:[%s]", line);
            return -1;
        }
...
{
    if (uri->port) {
        snprintf(line, size, "%s//%s:%u%s", rtsp_msg_int2str(rtsp_msg_uri_scheme_tbl, ARRAY_SIZE(rtsp_msg_uri_scheme_tbl), uri->scheme), uri->ipaddr, uri->port, uri->abspath);
    } else {
        snprintf(line, size, "%s//%s%s", rtsp_msg_int2str(rtsp_msg_uri_scheme_tbl, ARRAY_SIZE(rtsp_msg_uri_scheme_tbl), uri->scheme), uri->ipaddr, uri->abspath);
    }

    return strlen(line);
}

static int rtsp_msg_parse_startline(rtsp_msg_t *msg, const char *line)
{
    ...
    if (ret != RTSP_MSG_METHOD_BUTT) {
        ...
        if (ret == RTSP_MSG_VERSION_BUTT) {
            simple_rtsp_server_error("parse version failed, line:[%s]", line);
            return -1;
        }
    ...
    if (ret != RTSP_MSG_VERSION_BUTT) {
        ...
        if (sscanf(p, "%d", &ret) != 1) {
            simple_rtsp_server_error("parse status-code failed, line:[%s]", line);
            return -1;
        }
    ...
    if (*p != '$') {
        simple_rtsp_server_error("parse startline failed: %s\n", line);
        return -1;
    }
...
static int rtsp_msg_build_startline(const rtsp_msg_t *msg, char *line, int size)
{
    int ret;
    char *p = line;

    if (msg->type == RTSP_MSG_TYPE_REQUEST) {
        snprintf(line, size, "%s ", rtsp_msg_int2str(rtsp_msg_method_tbl, ARRAY_SIZE(rtsp_msg_method_tbl), msg->hdrs.startline.reqline.method));
        ret = strlen(p);
        p += ret;
    ...
    if (msg->type == RTSP_MSG_TYPE_RESPONSE) {
        snprintf(p, size, "%s %u %s\r\n", rtsp_msg_int2str(rtsp_msg_version_tbl, ARRAY_SIZE(rtsp_msg_version_tbl), msg->hdrs.startline.resline.version), msg->hdrs.startline.resline.status_code,
...
static int rtsp_msg_parse_transport(rtsp_msg_t *msg, const char *line)
{
    const char *p;
    unsigned int tmp;
    ...
    if (!hdrs->transport) {
        simple_rtsp_server_error("rtsp_mem_alloc for %s failed", "rtsp_msg_transport_s");
        return -1;
    }

    p = strstr(line, "RTP/AVP");
    if (!p) {
        simple_rtsp_server_error("parse transport failed, line:[%s]", line);
        rtsp_mem_free(hdrs->transport);
        hdrs->transport = NULL;
        return -1;
    }
...
static int rtsp_msg_build_transport(const rtsp_msg_t *msg, char *line, int size)
{
...
#define TRANSPORT_BUILD_STEP() \
    ...
    if (size <= 1) {           \
        ...
        if (hdrs->transport->flags & RTSP_MSG_TRANSPORT_FLAG_SERVER_PORT) {
            snprintf(p, size, ";server_port=%u-%u", hdrs->transport->server_port, hdrs->transport->server_port + 1);
            TRANSPORT_BUILD_STEP();
        }
...
static int rtsp_msg_parse_range(rtsp_msg_t *msg, const char *line)
{
    return 0;
}

static int rtsp_msg_build_range(const rtsp_msg_t *msg, char *line, int size)
{
    return 0;
}

static int rtsp_msg_parse_authorization(rtsp_msg_t *msg, const char *line)
{
    return 0;
}

static int rtsp_msg_build_authorization(const rtsp_msg_t *msg, char *line, int size)
{
    return 0;
}

static int rtsp_msg_parse_rtp_info(rtsp_msg_t *msg, const char *line)
{
    return 0;
}

static int rtsp_msg_build_rtp_info(const rtsp_msg_t *msg, char *line, int size)
{
    return 0;
}

#define DEFINE_PARSE_BUILD_LINK_CSEQ(_name, _type, _param, _fmt)                   \
    static int rtsp_msg_parse_##_name(rtsp_msg_t *msg, const char *line)           \
    {                                                                              \
        rtsp_msg_hdr_s *hdrs = &msg->hdrs;                                         \
        if (hdrs->_name) {                                                         \
        ...
        if (!hdrs->_name) {                                                        \
            simple_rtsp_server_error("rtsp_mem_alloc for %s failed\n", #_type);    \
            return -1;                                                             \
        }                                                                          \
        if (sscanf(line, _fmt, &hdrs->_name->_param) != 1) {                       \
            rtsp_mem_free(hdrs->_name);                                            \
            hdrs->_name = NULL;                                                    \
            simple_rtsp_server_error("parse %s failed, line:[%s]", #_name, line);  \
            return -1;                                                             \
        }                                                                          \
        return 0;                                                                  \
    }                                                                              \
                                                                                   \
    static int rtsp_msg_build_##_name(const rtsp_msg_t *msg, char *line, int size) \
    {                                                                              \
        if (msg->hdrs._name) {                                                     \
...
typedef int (*rtsp_msg_line_parser)(rtsp_msg_t *msg, const char *line);
...
void rtsp_msg_free(rtsp_msg_t *msg)
{
    ...
    if (msg->hdrs.rtp_info) {
        rtsp_mem_free(msg->hdrs.rtp_info);
    }
    ...
    while ((p = rtsp_msg_hdr_next_line(p, NULL, 0))) {
        ...
        if (strncmp(p, "Content-Length", 14) == 0) {
            if (sscanf(p, "Content-Length: %d", &content_len) != 1) {
                simple_rtsp_server_error("parse Content-Length failed. line: %s", p);
                return -1;
            }
        }
    }
...
int rtsp_msg_parse_from_array(rtsp_msg_t *msg, const void *data, int size)
{
    ...
    while ((p = rtsp_msg_hdr_next_line(p, line, sizeof(line)))) {
        ...
        if (!parser) {
            simple_rtsp_server_warn("unknown line:[%s]", line);
            continue;
        }

        ret = (*parser)(msg, line);
        if (ret < 0) {
            simple_rtsp_server_error("parse failed, line:[%s]", line);
            break;
        }
    }
    ...
    if (msg->hdrs.content_length) {
        ...
        if (!msg->body.body) {
            simple_rtsp_server_error("set body failed");
            rtsp_msg_free(msg);
            return -1;
        }
    }
    ...
    if (ret != size) {
        simple_rtsp_server_warn("frame size is %d. but real used %d", size, ret);
    }
...
int rtsp_msg_set_request(rtsp_msg_t *msg, rtsp_msg_method_e mt, const char *ipaddr, const char *abspath)
{
    ...
    strncpy(msg->hdrs.startline.reqline.uri.ipaddr, ipaddr, sizeof(msg->hdrs.startline.reqline.uri.ipaddr) - 1);
    strncpy(msg->hdrs.startline.reqline.uri.abspath, abspath, sizeof(msg->hdrs.startline.reqline.uri.abspath) - 1);
