#include <rkrga/rga.h>
#include <rkrga/rgadbg.h>
#include <rkrga/NormalRga.h>
#include <rkrga/NormalRgaContext.h>
...
int checkRectForRga(rga_rect_t rect)
{
    if (rect.xoffset < 0 || rect.yoffset < 0) {
        rga_error("error offset:[%d,%d]", rect.xoffset, rect.yoffset);
        return -EINVAL;
    }

    if (rect.width < 2 || rect.height < 2) {
        rga_error("error act:[%d,%d]", rect.width, rect.height);
        return -EINVAL;
    }

    if (rect.xoffset + rect.width > rect.wstride) {
        rga_error("error ws:[%d,%d,%d]", rect.xoffset, rect.width, rect.wstride);
        return -EINVAL;
    }

    if (rect.yoffset + rect.height > rect.hstride) {
        rga_error("error hs:[%d,%d,%d]", rect.yoffset, rect.height, rect.hstride);
        return -EINVAL;
    }
    ...
    {
        rga_error("error yuv not align to 2");
        return -EINVAL;
    }
...
#endif
{
    ...
    msg->mmu_info.mmu_flag = ((page_size & 0x3) << 4) | ((cmd_flush & 0x1) << 3) | ((dst_flush & 0x1) << 2) | ((src_flush & 0x1) << 1) | mmu_en;
...
int NormalRgaFullColorSpaceConvert(struct rga_req *msg, int color_space_mode)
{
    ...
    static full_csc_float_t default_csc_float_table[] = {
        { { 0.299, 0.587, 0.114, 0 }, { -0.169, -0.331, 0.5, 128 }, { 0.5, -0.419, -0.081, 128 } },
        { { 0.213, 0.715, 0.072, 0 }, { -0.115, -0.385, 0.5, 128 }, { 0.5, -0.454, -0.046, 128 } },
        { { -0.1826, 0.8588, -0.1014, 52.3554 }, { 0.1007, -0.0004, 0.8948, 0.5781 }, { 0.9005, 0, 0.0653, 4.3855 } },
        { { 0.1916, 1, 0.0993, -37.2476 }, { -0.1106, 0, 0.9895, 15.4669 }, { 0.9827, 0.0002, -0.0723, 11.4231 } },
        { { 0.1685, 0.8588, 0.0872, -16.7232 }, { -0.0971, 0, 0.8695, 29.1335 }, { 0.8638, 0, -0.0637, 25.5824 } },
        { { 0.1955, 1, 0.1019, -38.0729 }, { -0.1104, 0, 0.9899, 15.4218 }, { 0.9836, 0, -0.0716, 11.2587 } },
    };
    ...
    switch (color_space_mode) {
        ...
        default :
            rga_error("not support full csc mode:[%X]", color_space_mode);
            return -1;
    }
...
{
#if defined(__arm64__) || defined(__aarch64__)
    rga_error("render_mode:[%d], rotate_mode:[%d]", rgaReg.render_mode, rgaReg.rotate_mode);
    rga_error("src:[%lx,%lx,%lx], x-y:[%d,%d], w-h:[%d,%d], vw-vh:[%d,%d], f:[%d]", rgaReg.src.yrgb_addr, rgaReg.src.uv_addr, rgaReg.src.v_addr, rgaReg.src.x_offset, rgaReg.src.y_offset, rgaReg.src.act_w, rgaReg.src.act_h, rgaReg.src.vir_w, rgaReg.src.vir_h, rgaReg.src.format);
    rga_error("dst:[%lx,%lx,%lx], x-y:[%d,%d], w-h:[%d,%d], vw-vh:[%d,%d], f:[%d]", rgaReg.dst.yrgb_addr, rgaReg.dst.uv_addr, rgaReg.dst.v_addr, rgaReg.dst.x_offset, rgaReg.dst.y_offset, rgaReg.dst.act_w, rgaReg.dst.act_h, rgaReg.dst.vir_w, rgaReg.dst.vir_h, rgaReg.dst.format);
    rga_error("pat:[%lx,%lx,%lx], x-y:[%d,%d], w-h:[%d,%d], vw-vh:[%d,%d], f:[%d]", rgaReg.pat.yrgb_addr, rgaReg.pat.uv_addr, rgaReg.pat.v_addr, rgaReg.pat.x_offset, rgaReg.pat.y_offset, rgaReg.pat.act_w, rgaReg.pat.act_h, rgaReg.pat.vir_w, rgaReg.pat.vir_h, rgaReg.pat.format);
    rga_error("ROP:[%lx,%x,%x], LUT:[%lx]", rgaReg.rop_mask_addr, rgaReg.alpha_rop_flag, rgaReg.rop_code, rgaReg.LUT_addr);

    rga_error("color:[%x,%x,%x,%x,%x]", rgaReg.color_key_max, rgaReg.color_key_min, rgaReg.fg_color, rgaReg.bg_color, rgaReg.color_fill_mode);
    rga_error("MMU:[%d,%lx,%x]", rgaReg.mmu_info.mmu_en, rgaReg.mmu_info.base_addr, rgaReg.mmu_info.mmu_flag);

    rga_error("mode[%d,%d,%d,%d]", rgaReg.palette_mode, rgaReg.yuv2rgb_mode, rgaReg.endian_mode, rgaReg.src_trans_mode);

    rga_error("Full CSC : en[%d]", rgaReg.full_csc.flag);
    ...
    rga_error("gr_color_x:[%x, %x, %x]", rgaReg.gr_color.gr_x_r, rgaReg.gr_color.gr_x_g, rgaReg.gr_color.gr_x_b);
    rga_error("gr_color_x:[%x, %x, %x]", rgaReg.gr_color.gr_y_r, rgaReg.gr_color.gr_y_g, rgaReg.gr_color.gr_y_b);
#else
    rga_error("render_mode:[%d], rotate_mode:[%d]", rgaReg.render_mode, rgaReg.rotate_mode);
    rga_error("src:[%x,%x,%x], x-y:[%d,%d], w-h:[%d,%d], vw-vh:[%d,%d], f:[%d]", rgaReg.src.yrgb_addr, rgaReg.src.uv_addr, rgaReg.src.v_addr, rgaReg.src.x_offset, rgaReg.src.y_offset, rgaReg.src.act_w, rgaReg.src.act_h, rgaReg.src.vir_w, rgaReg.src.vir_h, rgaReg.src.format);
    rga_error("dst:[%x,%x,%x], x-y:[%d,%d], w-h:[%d,%d], vw-vh:[%d,%d], f:[%d]", rgaReg.dst.yrgb_addr, rgaReg.dst.uv_addr, rgaReg.dst.v_addr, rgaReg.dst.x_offset, rgaReg.dst.y_offset, rgaReg.dst.act_w, rgaReg.dst.act_h, rgaReg.dst.vir_w, rgaReg.dst.vir_h, rgaReg.dst.format);
    rga_error("pat:[%x,%x,%x], x-y:[%d,%d], w-h:[%d,%d], vw-vh:[%d,%d], f:[%d]", rgaReg.pat.yrgb_addr, rgaReg.pat.uv_addr, rgaReg.pat.v_addr, rgaReg.pat.x_offset, rgaReg.pat.y_offset, rgaReg.pat.act_w, rgaReg.pat.act_h, rgaReg.pat.vir_w, rgaReg.pat.vir_h, rgaReg.pat.format);
    rga_error("ROP:[%x,%x,%x], LUT:[%x]", rgaReg.rop_mask_addr, rgaReg.alpha_rop_flag, rgaReg.rop_code, rgaReg.LUT_addr);

    rga_error("color:[%x,%x,%x,%x,%x]", rgaReg.color_key_max, rgaReg.color_key_min, rgaReg.fg_color, rgaReg.bg_color, rgaReg.color_fill_mode);
    rga_error("MMU:[%d,%x,%x]", rgaReg.mmu_info.mmu_en, rgaReg.mmu_info.base_addr, rgaReg.mmu_info.mmu_flag);

    rga_error("mode:[%d,%d,%d,%d,%d]", rgaReg.palette_mode, rgaReg.yuv2rgb_mode, rgaReg.endian_mode, rgaReg.src_trans_mode,rgaReg.scale_mode);

    rga_error("Full CSC : EN:[%d] FACTOR:[%d, %d, %d, %d], [%d, %d, %d, %d], [%d, %d, %d, %d]",
