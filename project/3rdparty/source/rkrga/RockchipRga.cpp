...
#include <rkrga/version.h>
#include <rkrga/NormalRga.h>
#include <rkrga/RockchipRga.h>
...
RGA_SINGLETON_STATIC_INSTANCE(RockchipRga)
...
#if defined(CONFIG_LIBDRM)
        int ret = drmPrimeHandleToFD(bo_info->fd, bo_info->handle, DRM_CLOEXEC | DRM_RDWR, fd);
        return ret;
#else
        return -1;
#endif
    ...
    int RockchipRga::RkRgaLogOutUserPara(rga_info *rgaInfo) {
        ...
        rga_error("fd-vir-phy-hnd-format[%d, %p, %p, %p, %d]", rgaInfo->fd, rgaInfo->virAddr, rgaInfo->phyAddr, (void*)rgaInfo->hnd, rgaInfo->format);
        rga_error("rect[%d, %d, %d, %d, %d, %d, %d, %d]", rgaInfo->rect.xoffset, rgaInfo->rect.yoffset, rgaInfo->rect.width, rgaInfo->rect.height, rgaInfo->rect.wstride, rgaInfo->rect.hstride, rgaInfo->rect.format, rgaInfo->rect.size);
        rga_error("f-blend-size-rotation-col-log-mmu[%d, %x, %d, %d, %d, %d, %d]", rgaInfo->format, rgaInfo->blend, rgaInfo->bufferSize, rgaInfo->rotation, rgaInfo->color, rgaInfo->testLog, rgaInfo->mmuFlag);
