#ifndef RTSP_INTERNAL_RTSP_CLIENT_H
#define RTSP_INTERNAL_RTSP_CLIENT_H
...
public:
    ...
    const uint8_t *videoExtraData() {
        return fVideoExtraData;
    }

    unsigned videoExtraDataSize() {
        return fVideoExtraDataSize;
    }

    const uint8_t *audioExtraData() {
        return fAudioExtraData;
    }

    unsigned audioExtraDataSize() {
        return fAudioExtraDataSize;
    }
    ...
    unsigned lastResponseCode() {
        return fLastResponseCode;
    }

protected:
    char *sendOptionsCmd(const char *url, char *username = NULL, char *password = NULL, Authenticator *authenticator = NULL);

    char *describeURL(const char *url, Authenticator *authenticator = NULL, bool allowKasennaProtocol = false);
    char *describeWithPassword(char const *url, char const *username, char const *password, bool allowKasennaProtocol = false);

    bool setupMediaSubsession(MediaSubsession &subsession, bool streamOutgoing = false, bool streamUsingTCP = false, bool forceMulticastOnUnspecified = false);

    bool playMediaSession(MediaSession &session, bool response, double start = 0.0f, double end = -1.0f, float scale = 1.0f);
    bool pauseMediaSession(MediaSession &session);

    bool setMediaSessionParameter(MediaSession &session, char const *parameterName, char const *parameterValue);
    bool getMediaSessionParameter(MediaSession &session, char const *parameterName, char *&parameterValue);
...
protected:
    ...
    bool getResponse(char const *tag, unsigned &bytesRead, unsigned &responseCode, char *&firstLine, char *&nextLineStart, bool checkFor200Response = true);
    ...
    bool parseTransportResponse(char const *line, char *&serverAddressStr, unsigned short &serverPortNum, unsigned char &rtpChannelId, unsigned char &rtcpChannelId);
