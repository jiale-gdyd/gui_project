#ifndef RKRGA_IM2D_IMPL_H
#define RKRGA_IM2D_IMPL_H
...
#define RGA_GET_API_VERSION(v)                      {(((v) >> 24) & 0xff), (((v) >> 16) & 0xff), (((v) >> 8) & 0xff), {0}}

#define RGA_SET_VERSION(major, minor, revision)     (struct rga_version_t) { (major), (minor), (revision), RGA_VERSION_STR(major) "." RGA_VERSION_STR(minor) "." RGA_VERSION_STR(revision) }
#define RGA_SET_CURRENT_API_VERSION                 (RGA_SET_VERSION(RGA_API_MAJOR_VERSION, RGA_API_MINOR_VERSION, RGA_API_REVISION_VERSION))
...
IM_STATUS rga_check_external(const rga_buffer_t src, const rga_buffer_t dst, const rga_buffer_t pat, const im_rect src_rect, const im_rect dst_rect, const im_rect pat_rect, int mode_usage);
